...
\begin{document}
	...
	\begin{table}[H]
		...
		\begin{tabularx}{\textwidth}{@{}l l l X@{}}
			...
			4.1 & 30/08/2026 & Equipo MundiPets & Cierre de trazabilidad de la Entrega 4 (2B): se redactan las 12 historias de usuario y criterios de aceptación pendientes (HU-16 a HU-27, RF de prioridad \emph{Should}/\emph{Could}), quedando las 27 historias del sistema completas; se incorpora la Sección~\ref{sec:casos-prueba} (Casos de prueba, CP-01 a CP-61) para los 61 elementos vigentes; el Apéndice~D se reconstruye como matriz de trazabilidad única, incorporando las columnas Horizontal, Clase, Proceso, Estado, Caso de Prueba y Estado de la traza junto a las ya existentes, de modo que un solo apéndice cierra los tres tipos de trazabilidad exigidos (hacia atrás, hacia adelante y horizontal) sin dividirlos en apéndices separados; se corrige el enlace de caso de uso de RF-11, RNF-01, RD-04 y RNF-16 (CU-12, Configurar privacidad médica, en lugar de CU-06, que corresponde a la evaluación de compatibilidad de cruza); y se depura la redacción de las Secciones~\ref{sec:kano} y \ref{sec:amenazas} para reflejar el estado de cierre de esta entrega. \\
			\bottomrule
		\end{tabularx}
	\end{table}
...
trazados como TR-51 y TR-52, con su evidencia correspondiente (EV-24 y
...
de la traza}. Esta única tabla cierra los tres tipos de trazabilidad
exigidos, sin dividirlos en columnas ni en apéndices separados:
...
\begin{itemize}[leftmargin=*]
	...
	observación de campo y Encuesta para el cuestionario digital) o con el
	artículo de la LOPDP cuando aplica --- de qué persona o de qué obligación
	legal concreta proviene cada requisito, en lugar de limitarse al
	identificador interno de evidencia (EV-XX).
	...
	deriva o con los que se relaciona, además de su fuente primaria de
	evidencia --- por ejemplo, RNF-13 (explicabilidad de la compatibilidad
	para cruza) declara como fuente tanto la evidencia de campo que motivó la
	explicabilidad como \enquote{también RF-06, RNF-06}, el requisito y la
	métrica de los que depende horizontalmente.
\end{itemize}

La matriz alcanza las \textbf{61 filas} (TR-01 a TR-61), por encima del
mínimo de 40 exigido: 27 filas para los requisitos funcionales, 16 para los
no funcionales, 9 para las restricciones de diseño y 9 para los requisitos
legales, estas últimas trazadas en sentido inverso --- Ley~$\rightarrow$~Requisito --- ya que para un requisito legal la pregunta
relevante es qué elemento concreto lo implementa, no de qué participante
proviene. Se versiona además, con columnas adicionales de detalle
(Ley, Artículo, ID-EV, Objetivo, Stakeholder, Mockup y componente de
software), como \texttt{04\_Trazabilidad/matriz\_trazabilidad.csv} en el
repositorio.

\paragraph{Descripción de las columnas.}
\emph{Fuente}: código de participante (Apéndice~B.1) u obligación legal
(norma y artículo de la LOPDP) de la que proviene el requisito, seguido,
cuando corresponde, de los requisitos del mismo nivel de los que también
se deriva (trazabilidad horizontal). \emph{Requisito}: identificador RF,
RNF, RD o RL de la fila. \emph{Caso de Uso}: caso de uso que realiza el
requisito (Sección~\ref{sec:cu-detallado}), o su designación transversal
cuando no corresponde a un caso de uso discreto; para los requisitos
legales, el caso de uso indicado es el del RF o RNF que materializa la
obligación. \emph{Clase}: clase del diagrama de clases refinado
(Sección~\ref{sec:clases-refinado}) que materializa el requisito.
\emph{Proceso}: módulo funcional de la Sección~\ref{sec:funciones} al que
pertenece (P1 Gestión de perfiles de mascotas, P2 Búsqueda y filtrado, P3
Publicación de disponibilidad, P4 Validación de información médica, P5
Comunicación entre usuarios, P6 Gestión de mascotas rescatadas, P7 Reporte
de conversaciones sospechosas, P8 Apoyo basado en inteligencia artificial).
Este proyecto no modela con Diagrama de Flujo de Datos, por lo que esta
columna usa la agrupación funcional ya definida en la Sección~\ref{sec:funciones} como unidad de proceso de negocio. \emph{Estado}: estado
concreto de la máquina de estados (Sección~\ref{sec:estados}) que el
requisito activa o consume --- es un dato del \textbf{dominio del sistema}
(por ejemplo, una mascota queda en el estado \emph{Registrada}), y no debe
confundirse con la columna \emph{Estado de la traza}. \emph{Criterio}:
escenario Gherkin (CA-XX) definido junto a la historia de usuario
correspondiente (Sección~\ref{sec:historias}). \emph{Caso de Prueba}: ficha
CP-XX de la Sección~\ref{sec:casos-prueba}. \emph{Historia}: historia de
usuario (HU-XX) que el requisito utiliza; cuando un requisito no funcional
reutiliza la historia de un requisito funcional del que depende de forma
exclusiva --- no transversal ---, se indica entre paréntesis de cuál la
hereda, en lugar de generar una historia propia sin base real.
\emph{Estado de la traza}: a diferencia de la columna \emph{Estado}, esta
no es un dato del sistema sino un \textbf{juicio de calidad sobre la fila
misma} de la matriz: certifica si la cadena de trazabilidad de ese
requisito quedó completa, es transversal por diseño, o --- de existir ---
rota o huérfana (véase el diagnóstico posterior a la tabla).
...
\begin{landscape}
	\footnotesize
	\begin{longtable}{@{}p{0.8cm} p{4.4cm} p{1.3cm} p{1.7cm} p{1.9cm} p{1.0cm} p{1.7cm} p{0.9cm} p{0.9cm} p{1.75cm} p{2.2cm}@{}}
		...
		\endlastfoot
			...
			TR-01 & LOPDP, Art. 7 y 8; PROP-02, VET-01, VET-03, OBS-01; también RF-12, RF-13 & RF-01 & CU-01 & Mascota & P1 & Registrada & CA-01 & CP-01 & HU-01 & Completa \\ \midrule
			TR-02 & LOPDP, Art. 14; PROP-01, VET-02; también RF-14, RF-24 & RF-02 & CU-02 & HistorialMedico & P1 & --- & CA-02 & CP-02 & HU-02 & Completa \\ \midrule
			TR-03 & LOPDP, Art. 12 y 13; VET-02, PROP-03, OBS-01; también RF-04, RF-11 & RF-03 & CU-10 & Mascota & P2 & --- & CA-03 & CP-03 & HU-03 & Completa \\ \midrule
			TR-04 & Encuesta, OBS-01; también RF-03 & RF-04 & CU-11 & Mascota, Publicacion & P2 & --- & CA-16 & CP-04 & HU-16 & Completa \\ \midrule
			TR-05 & PROP-01, PROP-02, VET-01, OBS-01; también RF-12, RF-13, RF-18 & RF-05 & CU-04 & SolicitudAdopcion & P3 & Pendiente & CA-04 & CP-05 & HU-04 & Completa \\ \midrule
			TR-06 & LOPDP, Art. 20; VET-02, VET-03, PROP-03, OBS-01; también RF-16, RF-19, RF-23 & RF-06 & CU-13 & RegistroCompatibilidad & P8 & --- & CA-05 & CP-06 & HU-05 & Completa \\ \midrule
			TR-07 & LOPDP, Art. 10 lit. e); PROP-02, Encuesta, OBS-01; también RF-05, RF-08, RNF-15 & RF-07 & CU-08 & Mensaje & P5 & --- & CA-17 & CP-07 & HU-17 & Completa \\ \midrule
			TR-08 & PROP-01, VET-01, VET-03; también RF-13, RF-19, RF-27 & RF-08 & CU-05 & SolicitudCruza & P3 & Pendiente & CA-06 & CP-08 & HU-06 & Completa \\ \midrule
			TR-09 & LOPDP, Art. 25 y 26; VET-01, VET-03, Encuesta; también RF-02, RF-26 & RF-09 & CU-09 & HistorialMedico, Veterinaria & P4 & PendienteDeValidacion & CA-07 & CP-09 & HU-07 & Completa \\ \midrule
			TR-10 & PROP-02, VET-02, PROP-03; también RF-14, RD-08 & RF-10 & CU-12 & Mascota & P1 & --- & CA-18 & CP-10 & HU-18 & Completa \\ \midrule
			TR-11 & LOPDP, Art. 12, 13 y 10 lit. e); PROP-02, VET-03, PROP-03, Encuesta, OBS-01; también RF-03, RD-02 & RF-11 & CU-12 & Usuario & P1 & --- & CA-08 & CP-11 & HU-08 & Completa \\ \midrule
			TR-12 & LOPDP, Art. 7 y 8; PROP-01, VET-01, Encuesta, OBS-01; también RF-01, RL-01 & RF-12 & (Transversal a Onboarding) & Usuario & P1 & --- & CA-09 & CP-12 & HU-09 & Completa \\ \midrule
			TR-13 & PROP-01, PROP-02, Encuesta, OBS-01; también RF-01, RF-05, RF-08 & RF-13 & CU-04 & Publicacion & P3 & Publicada & CA-10 & CP-13 & HU-10 & Completa \\ \midrule
			TR-14 & LOPDP, Art. 14; VET-01, VET-03, Encuesta; también RF-02, RF-24 & RF-14 & CU-03 & HistorialMedico & P1 & PendienteDeValidacion & CA-11 & CP-14 & HU-11 & Completa \\ \midrule
			TR-15 & PROP-01, PROP-02, VET-02, PROP-03; también RF-05, RF-08, RF-27 & RF-15 & CU-07 & SolicitudAdopcion, SolicitudCruza & P2 & --- & CA-19 & CP-15 & HU-19 & Completa \\ \midrule
			TR-16 & PROP-01, VET-01, VET-03, Encuesta; también RF-06, RF-19 & RF-16 & CU-02 & AntecedenteGenetico & P1 & --- & CA-12 & CP-16 & HU-12 & Completa \\ \midrule
			TR-17 & LOPDP, Art. 20; PROP-01, VET-01, Encuesta, OBS-01; también RNF-07, RNF-14 & RF-17 & (Transversal a Publicaciones) & Publicacion & P8 & --- & CA-20 & CP-17 & HU-20 & Completa \\ \midrule
			TR-18 & LOPDP, Art. 10 lit. j) y 25; VET-03, PROP-03, Encuesta, OBS-01; también RD-04, RL-05, RL-07 & RNF-01 & CU-12 & --- & P1 & --- & --- & CP-28 & --- & Completa (transversal) \\ \midrule
			TR-19 & PROP-01, PROP-02, VET-01, VET-02, VET-03, PROP-03, Encuesta; también RNF-03 & RNF-02 & (Transversal) & --- & Transversal & --- & --- & CP-29 & --- & Completa (transversal) \\ \midrule
			TR-20 & PROP-01, PROP-02, VET-01, VET-02, VET-03, PROP-03, Encuesta; también RNF-02, RNF-12 & RNF-03 & CU-11 & --- & Transversal & --- & --- & CP-30 & --- & Completa (transversal) \\ \midrule
			TR-21 & PROP-01, PROP-02, VET-01, VET-02, VET-03, PROP-03, Encuesta, OBS-01 & RNF-04 & (Transversal / UI) & --- & Transversal & --- & --- & CP-31 & --- & Completa (transversal) \\ \midrule
			TR-22 & LOPDP, Art. 25 y 26; VET-01, VET-03, Encuesta; también RD-04 & RNF-05 & CU-02 & --- & P1 & --- & --- & CP-32 & --- & Completa (transversal) \\ \midrule
			TR-23 & LOPDP, Art. 20; PROP-01, VET-01, VET-03, Encuesta; también RF-06, RNF-13 & RNF-06 & CU-13 & --- & P8 & --- & CA-05 (heredado de RF-06) & CP-33 & HU-05 (heredada de RF-06) & Completa \\ \midrule
			TR-24 & LOPDP, Art. 20; PROP-01, VET-01, VET-03, Encuesta; también RF-17, RNF-14 & RNF-07 & (Transversal a Media) & --- & P8 & --- & CA-20 (heredado de RF-17) & CP-34 & HU-20 (heredada de RF-17) & Completa \\ \midrule
			TR-25 & PROP-01, PROP-02, VET-01, VET-02, VET-03, PROP-03, Encuesta & RNF-08 & CU-01 & --- & P1 & --- & --- & CP-35 & --- & Completa (transversal) \\ \midrule
			TR-26 & LOPDP, Art. 20; PROP-01, VET-01, VET-03, Encuesta; también RF-06, RF-17, RF-07 & RD-01 & CU-13 & --- & P8 & --- & --- & CP-44 & --- & Completa (transversal) \\ \midrule
			TR-27 & LOPDP, Art. 39; PROP-01, PROP-02, VET-01, VET-02, VET-03, PROP-03, Encuesta; también RF-11, RL-05, RL-08 & RD-02 & (Transversal a Seguridad) & Usuario & Transversal & --- & --- & CP-45 & --- & Completa (transversal) \\ \midrule
			TR-28 & LOPDP, Art. 25 y 26; VET-01, VET-03, Encuesta; también RF-09, RF-19 & RD-03 & CU-09 & --- & P4 & --- & --- & CP-46 & --- & Completa (transversal) \\ \midrule
			TR-29 & LOPDP, Art. 25, 26 y 30; PROP-02, VET-03, PROP-03, Encuesta; también RNF-01, RL-05, RL-07 & RD-04 & CU-12 & --- & P1 & --- & --- & CP-47 & --- & Completa (transversal) \\ \midrule
			TR-30 & Equipo desarrollador; también RNF-10 & RD-05 & (Transversal a Diseño) & --- & Transversal & --- & --- & CP-48 & --- & Completa (transversal) \\ \midrule
			TR-31 & PROP-06, VET-05; también RF-05, RF-22 & RF-18 & CU-04 & SolicitudAdopcion & P3 & EnEntrevista/EnVisita/EnPeriodoDePrueba & CA-13 & CP-18 & HU-13 & Completa \\ \midrule
			TR-32 & LOPDP, Art. 12; PROP-06; también RF-06, RF-16 & RF-19 & CU-09 & CertificadoVeterinario & P4 & PendienteDeValidacion & CA-21 & CP-19 & HU-21 & Completa \\ \midrule
			TR-33 & PROP-08; también RF-07 & RF-20 & CU-08 & Mensaje & P5 & --- & CA-27 & CP-20 & HU-27 & Completa (Could) \\ \midrule
			TR-34 & VET-05; también RF-14, RF-25 & RF-21 & CU-01 & Microchip & P1 & --- & CA-22 & CP-21 & HU-22 & Completa \\ \midrule
			TR-35 & PROP-05, VET-05; también RF-18 & RF-22 & CU-07 & SolicitudAdopcion & P3 & EnSeguimiento & CA-14 & CP-22 & HU-14 & Completa \\ \midrule
			TR-36 & PROP-04, PROP-07, VET-04, PROP-11; también RF-06, RF-16 & RF-23 & CU-06 & AlertaRiesgo & P7 & --- & CA-15 & CP-23 & HU-15 & Completa \\ \midrule
			TR-37 & LOPDP, Art. 4; VET-04, VET-05; también RF-02, RF-14 & RF-24 & CU-03 & HistorialMedico & P1 & PendienteDeValidacion & CA-23 & CP-24 & HU-23 & Completa \\ \midrule
			TR-38 & PROP-07, VET-05; también RF-21 & RF-25 & CU-04 & AvisoExtraviado & P3 & Extraviada & CA-24 & CP-25 & HU-24 & Completa \\ \midrule
			TR-39 & LOPDP, Art. 9; PROP-07, VET-05; también RF-11, RL-05, RL-08 & RNF-16 & CU-12 & --- & P1 & --- & CA-08 & CP-43 & HU-08 (heredada de RF-11) & Completa \\ \midrule
			TR-40 & LOPDP, Art. 30--32; PROP-01, PROP-02, VET-01, VET-02, VET-03, PROP-03, Encuesta, OBS-01, PROP-04, PROP-05, PROP-06, PROP-07, PROP-08, PROP-09, VET-04, VET-05, PROP-10, PROP-11, VET-06, PROP-12, VET-07 & RD-09 & (Transversal a Datos) & --- & Transversal & --- & --- & CP-52 & --- & Completa (transversal) \\ \midrule
			TR-41 & PROP-04, PROP-11; también RF-10, RD-08 & RNF-09 & CU-12 & --- & P1 & --- & CA-18 (heredado de RF-10) & CP-36 & HU-18 (heredada de RF-10) & Completa \\ \midrule
			TR-42 & Equipo desarrollador; también RD-05 & RNF-10 & (Transversal a Diseño) & --- & Transversal & --- & --- & CP-37 & --- & Completa (transversal) \\ \midrule
			TR-43 & Encuesta, OBS-01 & RNF-11 & (Transversal a UI) & --- & Transversal & --- & --- & CP-38 & --- & Completa (transversal) \\ \midrule
			TR-44 & Encuesta, OBS-01; también RNF-03 & RNF-12 & (Transversal) & --- & Transversal & --- & --- & CP-39 & --- & Completa (transversal) \\ \midrule
			TR-45 & LOPDP, Art. 20; PROP-06, PROP-08, VET-04, VET-05; también RF-06, RNF-06, RL-06 & RNF-13 & CU-13 & --- & P8 & --- & CA-05 & CP-40 & HU-05 (heredada de RF-06) & Completa \\ \midrule
			TR-46 & LOPDP, Art. 20; PROP-06, PROP-08; también RF-17, RNF-07, RL-06 & RNF-14 & (Transversal a Media) & --- & P8 & --- & CA-20 (heredado de RF-17) & CP-41 & HU-20 (heredada de RF-17) & Completa \\ \midrule
			TR-47 & LOPDP, Art. 20; PROP-08, PROP-11; también RF-07, RL-06 & RNF-15 & CU-08 & --- & P5 & --- & CA-17 (heredado de RF-07) & CP-42 & HU-17 (heredada de RF-07) & Completa \\ \midrule
			TR-48 & LOPDP, Art. 15; PROP-11; también RL-04 & RD-06 & (Transversal a Datos) & --- & Transversal & --- & --- & CP-49 & --- & Completa (transversal) \\ \midrule
			TR-49 & LOPDP, Art. 43 y 46; PROP-07, PROP-11; también RL-09 & RD-07 & (Transversal a Seguridad) & --- & Transversal & --- & --- & CP-50 & --- & Completa (transversal) \\ \midrule
			TR-50 & PROP-04, PROP-11; también RF-10, RNF-09 & RD-08 & CU-12 & --- & P1 & --- & --- & CP-51 & --- & Completa (transversal) \\ \midrule
			TR-51 & VET-07; también RF-09 & RF-26 & CU-09 & HistorialMedico, Veterinaria & P4 & Validado & CA-25 & CP-26 & HU-25 & Completa \\ \midrule
			TR-52 & PROP-12; también RF-08, RF-15 & RF-27 & CU-05 & SolicitudCruza & P3 & Completada & CA-26 & CP-27 & HU-26 & Completa \\ \midrule
...
\textbf{requisito huérfano} aquel que carece de fuente identificable (ya
verificado como cero casos: las 61 filas declaran al menos un participante,
una fuente de evidencia agregada o un artículo de la LOPDP) o de todo
artefacto posterior que lo realice, y como \textbf{cadena rota} aquella en
la que un artefacto de diseño --- un caso de uso o una clase --- existe sin
que ningún requisito lo respalde. La revisión de las 61 filas de la
Tabla~\ref{tab:matriz_trazabilidad_completa} contra el diagrama de casos de
uso (Sección~\ref{sec:uml}) y el diagrama de clases refinado
(Sección~\ref{sec:clases-refinado}) no encontró ningún caso de ninguna de
las dos patologías. La Tabla~\ref{tab:huerfanos} documenta este resultado
siguiendo el formato exigido para reportar huérfanos y cadenas rotas cuando
existan.
...
\begin{table}[H]
	...
	\begin{tabularx}{\textwidth}{@{}l l X@{}}
		...
		\midrule
		Requisito huérfano (sin fuente) & 0 de 61 & No aplica; las 61 filas declaran al menos una fuente (participante, evidencia agregada o artículo de la LOPDP). \\
		\addlinespace
		Requisito huérfano (sin realización) & 0 de 61 & No aplica; las 61 filas declaran al menos un caso de uso o designación transversal, un proceso funcional y un caso de prueba. \\
		\addlinespace
		...
		\addlinespace
		Cadena rota (clase sin requisito) & 0 de 14\footnotemark & No aplica; las 14 clases de dominio del diagrama de clases refinado están referenciadas desde al menos una fila de la matriz. \\
		\bottomrule
	\end{tabularx}
\end{table}
\footnotetext{Las cuatro clases restantes del diagrama --- \texttt{PropietarioMascota}, \texttt{Adoptante}, \texttt{InteresadoEnCruza} y \texttt{RefugioAnimal} --- son especializaciones de \texttt{Usuario} que representan roles y se alcanzan a través del actor de cada caso de uso (Sección~\ref{sec:cu-detallado}), no como clase de dominio realizada por un requisito individual; ninguna de las cuatro existe sin un requisito que la origine, por lo que no constituyen una cadena rota.}
