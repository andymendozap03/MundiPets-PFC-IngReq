...
\end{table}
	...
	\begin{longtable}{@{}p{1.6cm} p{4.8cm} p{1.6cm} p{5.2cm}@{}}
		...
		\hline
		\hyperref[tab:RNF-10]{RNF-10} & Facilidad de mantenimiento del código & Should & Importante para sostenibilidad técnica, pero no bloquea la operación funcional del MVP. \\
		\hline
...
\end{table}
	...
	de protección de datos declaradas en \hyperref[tab:RD-04]{RD-04}, \hyperref[tab:RD-05]{RD-05} y \hyperref[tab:RD-06]{RD-06}, que
	...
	\hyperref[tab:RF-12]{RF-12}, se cubre en modo simulado (selección de rol de demostración en
	...
	es de 14 de 15 (93,3\,\%). El MVP cubre además la mayor parte de los
	requisitos \emph{Should} y \emph{Could}: de los 12 requisitos de esas dos
	prioridades, cubre 11 (91,7\,\%), quedando pendiente únicamente \hyperref[tab:RF-20]{RF-20}
	(\emph{Could}, coordinar encuentros de socialización supervisados). En
	conjunto, el MVP cubre \textbf{26 de los 27 RF vigentes (96,3\,\%)}. El
	único requisito no implementado, \hyperref[tab:RF-20]{RF-20}, se documenta explícitamente en la
	...
	\begin{longtable}{@{}p{1.5cm} p{5.2cm} p{1.7cm} p{3.0cm} p{3.4cm}@{}}
		...
		\hline
		\hyperref[tab:RF-10]{RF-10} & Gestionar recordatorios de controles preventivos & Should & Cubierto (nuevo) & \texttt{dashboard.html} (módulo de alertas y simulación de WhatsApp) \\
		\hline
		...
		\hline
		\hyperref[tab:RF-15]{RF-15} & Consultar el historial de procesos de adopción y cruza & Could & Cubierto (nuevo) & \texttt{dashboard.html} (sección expandible de solicitudes finalizadas) \\
		\hline
		...
		\hline
		\hyperref[tab:RF-17]{RF-17} & Validar imágenes & Should & Cubierto (nuevo, simulado) & \texttt{pet-add.html} (analizador interactivo de IA en el paso 2 del asistente) \\
		\hline
		\hyperref[tab:RF-18]{RF-18} & Gestionar el flujo de solicitud de adopción por etapas & Must & Cubierto & \texttt{request.html} \\
		\hline
		\hyperref[tab:RF-19]{RF-19} & Validar certificados veterinarios antes de habilitar la cruza & Should & Cubierto (nuevo) & \texttt{pet-profile.html} (bloqueo de la opción de cruza sin certificado validado) \\
		\hline
		\hyperref[tab:RF-20]{RF-20} & Coordinar encuentros de socialización supervisados & Could & No implementado & --- \\
		\hline
		\hyperref[tab:RF-21]{RF-21} & Registrar el identificador de microchip de la mascota & Should & Cubierto (nuevo) & \texttt{pet-add.html}, \texttt{pet-profile.html}, \texttt{pet-detail.html}, \texttt{explore.html} \\
		\hline
		...
		\hline
		\hyperref[tab:RF-24]{RF-24} & Registrar trazabilidad de cepa, lote y aplicador de cada vacuna & Should & Cubierto (nuevo) & \texttt{pet-add.html}, \texttt{pet-profile.html} (campos del carnet referencial) \\
		\hline
		\hyperref[tab:RF-25]{RF-25} & Publicar aviso de mascota extraviada & Could & Cubierto (nuevo) & \texttt{pet-profile.html}, \texttt{explore.html} (alerta destacada y contacto directo) \\
		\hline
		\hyperref[tab:RF-26]{RF-26} & Habilitar validación médica por segunda opinión veterinaria & Should & Cubierto (nuevo) & \texttt{vet-panel.html}, \texttt{pet-profile.html} (módulo de doble validación y constancia) \\
		\hline
		\hyperref[tab:RF-27]{RF-27} & Controlar y alertar sobre solicitudes repetitivas de cruza de una misma mascota & Should & Cubierto (nuevo) & \texttt{compatibility.html} (alerta automática al superar el umbral semestral de cruzas) \\
	\end{longtable}
	
	\textbf{Resumen:} 26 de 27 RF cubiertos (96,3\,\% de cobertura total y
	100\,\% de los requisitos \emph{Debe tener}). El único requisito que
	permanece sin cubrir es \hyperref[tab:RF-20]{RF-20}.
	...
	escritorio, en coherencia con el requisito de portabilidad \hyperref[tab:RNF-11]{RNF-11}. La capa
	de lógica de sesión, implementada en \texttt{js/auth.js}, simula la
	verificación de identidad exigida por \hyperref[tab:RF-12]{RF-12} mediante la selección de un rol
	...
	restricción de arquitectura modular \hyperref[tab:RD-05]{RD-05} en su expresión mínima viable.
	...
	\begin{longtable}{@{}p{4.2cm} p{10.6cm}@{}}
		...
		\hline
		\texttt{register.html} & Formulario de creación de cuenta y verificación de identidad simulada (\hyperref[tab:RF-12]{RF-12}). \\
		\hline
		...
		\hline
		\texttt{js/db.js} & Capa de acceso a datos sobre \texttt{localStorage}: define las colecciones del dominio (usuarios, mascotas, historiales médicos, antecedentes genéticos, solicitudes, mensajes, evaluaciones de compatibilidad y seguimientos), el conjunto de datos ficticios de arranque y las operaciones CRUD genéricas. Estas mismas ocho colecciones se ampliaron con los campos necesarios para soportar \hyperref[tab:RF-10]{RF-10}, \hyperref[tab:RF-15]{RF-15}, \hyperref[tab:RF-17]{RF-17}, \hyperref[tab:RF-19]{RF-19}, \hyperref[tab:RF-21]{RF-21}, \hyperref[tab:RF-24]{RF-24}, \hyperref[tab:RF-25]{RF-25}, \hyperref[tab:RF-26]{RF-26} y \hyperref[tab:RF-27]{RF-27}, sin requerir colecciones adicionales. \\
		\hline
		\texttt{js/auth.js} & Gestión de la sesión activa mediante selección de rol, sin autenticación por contraseña, simulando de forma declarada el resultado de \hyperref[tab:RF-12]{RF-12}. \\
		\hline
...
transcrito manualmente para este documento.
	...
	\begin{table}[H]
		...
		\begin{tabularx}{\linewidth}{|C{2.5cm}|X|C{2.8cm}|}
			...
			\hline
			Semanas 14--16 (03--23 ago.) & Ejecución del componente empírico, desarrollo del MVP (ampliado a 26 de 27 RF cubiertos), segunda ronda de \emph{walkthrough} con PROP-12 y VET-07 (EV-23, EV-24), revisión integral del documento. & Completado \\
			\hline
...
del volumen \texttt{.7z} los contiene.
	...
	{\small
		\begin{longtable}{@{}>{\raggedright\arraybackslash}p{1.4cm} >{\raggedright\arraybackslash}p{1.6cm} >{\raggedright\arraybackslash}p{3.2cm} >{\raggedright\arraybackslash}p{1.8cm} >{\raggedright\arraybackslash}p{6.5cm}@{}}
			...
			EV-04 & VET-02 & Veterinario & 17/05/2026 & \texttt{2026-05-17\_Veterinario\_VET-02\_TranscripcionEntrevista.txt} \\
	...
	\begin{landscape}
		\begin{longtable}{@{}p{1.2cm} p{5.2cm} p{1.0cm} p{1.3cm} p{1.3cm} p{1.1cm} p{6.5cm}@{}}
			...
			\hyperref[tab:RNF-10]{RNF-10} & Facilidad de mantenimiento del código & RNF & Should & --- & --- & Importante para sostenibilidad técnica, pero no bloquea la operación funcional del MVP. \\ \midrule
...
\end{landscape}
	...
	corregida de \texttt{01\_ERS/ERS\_SRS\_2B\_v2\_0.tex} y su PDF compilado.
	...
	\begin{table}[H]
		...
		\begin{tabularx}{\textwidth}{@{}p{4cm} X@{}}
			...
			Archivo etiquetado & \texttt{01\_ERS/ERS\_SRS\_2B\_v2\_0.tex} y su PDF compilado \\
...
\begin{longtable}{@{}p{2.6cm} p{7.4cm} p{4.0cm}@{}}
	...
	Morales Sánchez Gary Alejandro (Documentador) & Redacción y consolidación del documento ERS/SRS, gestión de la evidencia de campo (transcripciones, actas, anonimización) y estructura del repositorio. & Commits sobre \texttt{02\_Evidencias/} y \texttt{01\_ERS/ERS\_SRS\_2B\_v2\_0.tex} \\
	\midrule
