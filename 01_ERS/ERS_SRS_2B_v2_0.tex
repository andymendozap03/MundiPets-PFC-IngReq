...
\begin{document}
	...
	dos requisitos funcionales adicionales --- RF-26 y RF-27 --- derivados de
...
Complementando la exactitud (RNF-06, RNF-07) y la explicabilidad (RNF-13,
RNF-14, RNF-15) ya especificadas, esta sección define, para cada uno de los
...
\begin{table}[H]
	...
	\begin{tabularx}{\textwidth}{@{}p{2.3cm} X p{2.6cm}@{}}
		...
		\midrule
		\emph{AI compatibility} (RF-06) & Diferencia de tasa de coincidencia con la evaluación veterinaria de referencia entre razas con más de 30 casos en el conjunto de evaluación y razas con menos de 30 casos. & 10 puntos porcentuales \\
		\addlinespace
		\emph{AI image validation} (RF-17) & Diferencia de tasa de clasificación correcta entre fotografías tomadas en interiores y en exteriores, condición que afecta la iluminación y la nitidez de la imagen. & 10 puntos porcentuales \\
		\addlinespace
...
\begin{table}[H]
	...
	\begin{tabularx}{\textwidth}{@{}p{2.3cm} p{2.0cm} X@{}}
		...
		\midrule
		\emph{AI compatibility} (RF-06) & Riesgo limitado & Influye en una decisión con efecto sobre el bienestar animal (proceso de cruza) y está sujeto al derecho a explicación del Art.~20 LOPDP (RL-06), lo que exige las obligaciones de transparencia ya cubiertas por RNF-13. \\
		\addlinespace
		\emph{AI image validation} (RF-17) & Riesgo mínimo/limitado & Su salida (aceptar o rechazar una fotografía) no afecta la salud ni la seguridad de personas o animales; la obligación de transparencia ya está cubierta por RNF-14. \\
		\addlinespace
...
otras partes del sistema (RF-09, CU-09).
...
\begin{table}[H]
	...
	\begin{tabularx}{\textwidth}{@{}p{2.0cm} X p{4.3cm}@{}}
		...
		\midrule
		\emph{AI compatibility} & Coincidencia con la evaluación veterinaria (RNF-06), medida mensualmente sobre una muestra auditada de recomendaciones emitidas. & Caída sostenida por debajo del 85\,\% durante dos meses consecutivos: reentrenar con los casos auditados del período; si no se recupera el umbral en el siguiente ciclo, retirar temporalmente el componente y operar en modo manual (evaluación veterinaria directa). \\
		\addlinespace
		\emph{AI image validation} & Tasa de clasificación correcta (RNF-07), medida mensualmente sobre una muestra de imágenes revisadas manualmente como control de calidad. & Caída sostenida por debajo del 95\,\% durante dos meses consecutivos: reentrenar incorporando los casos mal clasificados detectados; si no se recupera, derivar temporalmente las imágenes a revisión manual antes de la publicación. \\
		\addlinespace
...
\end{table}
	...
	Las restricciones RD-06 a RD-09 derivan del análisis legal de la
	...
	RF-26 y RF-27 se apoyan en los actores, clases, pasos de control y
	...
	\paragraph{Cobertura de RF-21, RF-24 y RF-25 en el diagrama de clases.}
	La clase \texttt{Pet} incluye los atributos
	\texttt{-microchipId~:~string} y \texttt{-microchipDate~:~datetime}
	(RF-21); la clase \texttt{PetVaccine} incluye los atributos
	\texttt{-batchNumber~:~string} y \texttt{-administeredBy~:~string}
	(RF-24); y la clase \texttt{LostPetAlert}
	(RF-25) incluye los atributos \texttt{-alertId~:~long},
	...
	de gestión de solicitudes descrito en CU-07 y en la máquina de estados de
	...
	enviada (RF-05, RF-18), complementando la publicación cubierta por CU-04.
	El adoptante consulta el perfil (CU-10) y envía la solicitud, que el
	...
	de la gestión de solicitud de cruza descrita en el flujo alterno de CU-05.
	...
	información descrita en RF-11. El propietario consulta las opciones de
	...
	preventivo (RF-10). El controlador (\texttt{CareRecommendationController})
	...
	solicitud de adopción (CU-07), cuyo patrón es análogo para cruza, y en la
	...
	uso (flujo alterno 3.1 de la ficha de CU-06) considera inviable.
	...
	configuración de privacidad (RF-11). Tras seleccionar los datos a
	...
	cuidado preventivo asociada a RF-10. El sistema obtiene los datos de la
	...
	vigentes: 27 requisitos funcionales (RF-01 a RF-27),
	...
	\ref{sec:restricciones}. RF-26 y RF-27 se incorporan a partir de la nueva
	evidencia recolectada de PROP-12 y VET-07. El alcance de RF-06, RF-07,
	RF-10, RNF-02 y RNF-03 quedó acotado tras la revisión de calidad de
	requisitos (Sección~\ref{sec:trazabilidad}), y el de RNF-15 se redefinió
	para ser coherente con RF-07; dos propuestas de ampliación de alcance no se
	...
	limitaciones de alcance reconocidas en RD-06 y RL-01, respectivamente.
...
sus 61 filas (TR-01 a TR-61) y sus diez columnas, se presenta en el
\textbf{Apéndice D}.
...
\end{table}
	...
	prioridades, cubre 11 (91,7\,\%), quedando pendiente únicamente RF-20
	...
	único requisito no implementado, RF-20, se documenta explícitamente en la
	...
	El RF-17 (validación de imágenes) cuenta con una representación funcional,
	...
	funcionales (RF-18 a RF-25). Dos sesiones adicionales combinan entrevista
...
\subsubsection*{Acta de entrevista 7 --- PROP-04 (EV-09)}
	...
	el origen directo de RF-26 y RF-27) además de confirmar códigos ya
...
de usuario (HU-01 a HU-27) y por cada caso de prueba (CP-01 a CP-61),
...
\begin{table}[H]
	...
	\begin{tabularx}{\textwidth}{@{}p{3.0cm} X@{}}
		...
		\addlinespace
		Generación de código de apoyo & Apoyo en la generación de fragmentos de código de scripts de análisis y utilidades de verificación, revisados y ejecutados por el equipo antes de su incorporación al repositorio. \\
		\addlinespace
