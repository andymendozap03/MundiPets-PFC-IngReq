...
\begin{table}[H]
	...
	\small
	\begin{tabularx}{\textwidth}{@{}p{2.3cm} X p{2.6cm}@{}}
		\toprule
		\textbf{Componente} & \textbf{Métrica y subgrupos comparados} & \textbf{Brecha máxima tolerada} \\
		\midrule
		\emph{AI compatibility} (\hyperref[tab:RF-06]{RF-06}) & Diferencia de tasa de coincidencia con la evaluación veterinaria de referencia entre razas con más de 30 casos en el conjunto de evaluación y razas con menos de 30 casos. & 10 puntos porcentuales \\
		\addlinespace
		\emph{AI image validation} (\hyperref[tab:RF-17]{RF-17}) & Diferencia de tasa de clasificación correcta entre fotografías tomadas en interiores y en exteriores, condición que afecta la iluminación y la nitidez de la imagen. & 10 puntos porcentuales \\
		\addlinespace
		Moderación de mensajes (\hyperref[tab:RF-07]{RF-07}) & Diferencia de tasa de falsos bloqueos entre mensajes de longitud breve (menos de 15 palabras) y mensajes de longitud extensa, dado que los mensajes breves ofrecen menos contexto para la clasificación. & 10 puntos porcentuales \\
		\bottomrule
	\end{tabularx}
\end{table}
...
\end{landscape}
	...
	incluyendo la carpeta de defensa (\texttt{09\_Defensa/}), el archivo raíz
	...
	\begin{lstlisting}[style=csharp,caption={Estructura de carpetas del repositorio MundiPets-PFC-IngReq.},label={lst:estructura_repo}]
		...
		|-- .gitattributes
		|-- .gitignore
		|-- .gitmodules                    % declara el submodulo 05_MVP/codigo
		|-- CHANGELOG.md
		|-- checksums.sha256
		|-- CITATION.cff
		|-- LICENSE
		...
		|   `-- codigo/                    % submodulo Git del MVP_MundiPets
		|
		|-- 06_Experimento/
		...
		|   |   |-- transcripciones/       % copia anonimizada para el deposito
