...
\begin{table}[H]
	...
	\begin{tabularx}{\textwidth}{@{}p{2.3cm} p{2.0cm} X@{}}
		...
		\addlinespace
		\emph{AI image validation} (\hyperref[tab:RF-17]{RF-17}) & Riesgo mínimo/limitado & Su salida (aceptar o rechazar una fotografía) no afecta la salud ni la seguridad de personas o animales; la obligación de transparencia ya está cubierta por \hyperref[tab:RNF-14]{RNF-14}. \\
		\addlinespace
		Moderación de mensajes (\hyperref[tab:RF-07]{RF-07}) & Riesgo mínimo/limitado & Su salida (bloquear o permitir un mensaje) es revisable por los propios usuarios mediante el canal de comunicación directa; la obligación de transparencia ya está cubierta por \hyperref[tab:RNF-15]{RNF-15}. \\
		\bottomrule
	\end{tabularx}
\end{table}
...
\end{landscape}
	...
	\begin{lstlisting}[style=csharp,caption={Estructura de carpetas del repositorio MundiPets-PFC-IngReq.},label={lst:estructura_repo}]
		...
		|   `-- figuras/                   % diagramas i*, UML, mockups (MU-01
		|                                   % a MU-08) y capturas de pantalla
		...
		|   |       `-- evidencias_restringidas.7z.001 ... .275
		...
		|   |   |-- power_calculation/     % power calculation del perfil
		|   |   |                          % dominante (n=47, potencia=91,86%)
		|   |   `-- Respuestas/            % 61 respuestas en PDF + resumen .xlsx
		|   |-- Documentos_Organizacion/   % 5 documentos clinicos de referencia
		|   |-- Fotos_Entorno/             % B.5, observacion OBS-01 (20 archivos)
		...
		|   `-- Validacion_Walkthrough/    % B.7, actas EV-19 a EV-24 (6 archivos)
		|
		|-- 03_Modelado/
		...
		|   |                              % .pdf/.drawio
		|   `-- Mockups/                   % \hyperref[fig:mu01]{MU-01} a \hyperref[fig:mu08]{MU-08}
		|
		|-- 04_Trazabilidad/
		...
		|   `-- codigo/                    % codigo fuente del MVP, incorporado
		|                                   % con git subtree (no es submodulo;
		|                                   % conserva el historial real de
		|                                   % commits del desarrollo del MVP)
		|
		|-- 06_Experimento/
