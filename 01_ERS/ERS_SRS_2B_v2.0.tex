...
\begin{document}
	...
	\begin{titlepage}
		...
		\url{https://github.com/gleiston-guerrero/MundiPets-PFC-IngReq.git}\\[6pt]
		\textbf{DOI del paquete de datos (Zenodo):}\\
		\url{https://doi.org/10.5281/zenodo.22218780}
	...
	\begin{table}[H]
		...
		\begin{tabularx}{\textwidth}{@{}l l l X@{}}
			...
			4.0 & 30/08/2026 & Equipo MundiPets & Versión completa del ERS/SRS. Incluye una revisión de calidad de los requisitos funcionales conforme a ISO/IEC/IEEE 29148:2018 y la incorporación de dos requisitos funcionales adicionales, \hyperref[tab:RF-26]{RF-26} y \hyperref[tab:RF-27]{RF-27}, a partir de nueva evidencia recolectada (participantes PROP-12 y VET-07). Incorpora las 27 historias de usuario y sus criterios de aceptación, la sección de casos de prueba (\hyperref[tab:CP-01]{CP-01} a \hyperref[cp:61]{CP-61}) para los 61 elementos vigentes, y la matriz de trazabilidad final del Apéndice~D en formato de diez columnas (Fuente, Requisito, Caso de Uso, Clase, Proceso, Estado, Criterio, Caso de Prueba, Historia, Estado de la traza), ordenada por tipo de requisito (RF, RNF, RD, RL), con la columna Fuente identificando el código de participante del Apéndice~B.1 (PROP-XX, VET-XX, OBS-01, Encuesta) y la referencia a la LOPDP reservada a los requisitos legales. El diagrama de clases refinado incluye los atributos y la clase \texttt{LostPetAlert} necesarios para \hyperref[tab:RF-21]{RF-21}, \hyperref[tab:RF-24]{RF-24} y \hyperref[tab:RF-25]{RF-25}, y el caso de uso de \hyperref[tab:RF-11]{RF-11}, \hyperref[tab:RNF-01]{RNF-01}, \hyperref[tab:RD-04]{RD-04} y \hyperref[tab:RNF-16]{RNF-16} es \hyperref[tab:CU-12]{CU-12} (Configurar privacidad médica). \\
			\bottomrule
		\end{tabularx}
	\end{table}
	...
	Este documento incorpora una revisión de calidad de los
	requisitos funcionales conforme a ISO/IEC/IEEE 29148:2018 (Sección~\ref{sec:rf}),
	dos requisitos funcionales adicionales --- \hyperref[tab:RF-26]{RF-26} y \hyperref[tab:RF-27]{RF-27} --- derivados de
	evidencia de campo, y la sesión de miembro-verificación
	(\emph{member checking}, Apéndice~B.9) como evidencia terminal
	adicional a la ronda de \emph{walkthrough}. El componente empírico
	(Sección~\ref{sec:empirico}) valida mediante un protocolo experimental
	registrado y ejecutado con rigor metodológico aspectos concretos del
	sistema especificado, y sirve de base directa al manuscrito de
	publicación científica derivado del proyecto.
	...
	El alcance incluye además un Producto Mínimo Viable
	(MVP, Sección~\ref{sec:mvp}) que demuestra de forma ejecutable un subconjunto
	de los requisitos \emph{Debe tener}, y el componente empírico
	...
	conceptos empleados en el documento, incluyendo la sesión de
	miembro-verificación (\emph{member checking}), técnica de validación
	cualitativa incorporada como evidencia terminal adicional en el
	Apéndice~B.9; la priorización avanzada (Kano, WSJF); las historias de
	usuario (Connextra, INVEST, Gherkin); el marco de preguntas de
	investigación PICOC; los principios FAIR de gestión de datos; la
	explicabilidad de sistemas de IA; el lenguaje de modelado organizacional
	i*; el marco legal LOPDP; el repositorio OSF; el identificador DOI; y la
	saturación temática.
	...
	(\hyperref[tab:RF-18]{RF-18} a \hyperref[tab:RF-25]{RF-25}) se ubican dentro de los procesos ya delimitados y no
	...
	directamente con \hyperref[tab:RNF-05]{RNF-05} (integridad de la información médica) y con el
	...
	Estas metas blandas se conectan con \hyperref[tab:RNF-01]{RNF-01} (protección de la información) y
	\hyperref[tab:RNF-04]{RNF-04} (facilidad de uso de la plataforma).
	...
	conecta con \hyperref[tab:RNF-06]{RNF-06} (exactitud del componente de inteligencia artificial de
	...
	\paragraph{\hyperref[fig:mu01]{MU-01}: Crear cuenta}
	...
	\textbf{RF asociados:} \hyperref[tab:RF-12]{RF-12}.
	
	\begin{figure}[H]
		\centering
		\includegraphics[width=0.55\textwidth]{{figuras/MU-01_Crear_cuenta}.png}
		\caption{Mockup MU-01: Crear cuenta.}
		\label{fig:mu01}
	\end{figure}
	
	\paragraph{\hyperref[fig:mu02]{MU-02}: Registrar mascota}
	...
	\textbf{RF asociados:} \hyperref[tab:RF-01]{RF-01}.
	
	\begin{figure}[H]
		\centering
		\includegraphics[width=0.62\textwidth]{{figuras/MU-02_Registrar_mascota}.png}
		\caption{Mockup MU-02: Registrar mascota.}
		\label{fig:mu02}
	\end{figure}
	
	\paragraph{\hyperref[fig:mu03]{MU-03}: Agenda veterinaria y validación documental}
	...
	\textbf{RF asociados:} \hyperref[tab:RF-09]{RF-09}, \hyperref[tab:RF-14]{RF-14}.
	
	\begin{figure}[H]
		\centering
		\includegraphics[width=0.75\textwidth]{{figuras/MU-03_Agenda_Veterinaria_y_Validacion_Documental}.png}
		\caption{Mockup MU-03: Agenda veterinaria y validación documental.}
		\label{fig:mu03}
	\end{figure}
	
	\paragraph{\hyperref[fig:mu04]{MU-04}: Feed de publicaciones (búsqueda y filtrado)}
	...
	\textbf{RF asociados:} \hyperref[tab:RF-03]{RF-03}, \hyperref[tab:RF-04]{RF-04}.
	
	\begin{figure}[H]
		\centering
		\includegraphics[width=0.75\textwidth]{{figuras/MU-04_Feed_de_publicaciones_(Busqueda_y_filtrado)}.png}
		\caption{Mockup MU-04: Feed de publicaciones (búsqueda y filtrado).}
		\label{fig:mu04}
	\end{figure}
	
	\paragraph{\hyperref[fig:mu05]{MU-05}: Solicitud de adopción o cruza}
	...
	indicador de etapa del flujo de adopción por etapas (\hyperref[tab:RF-18]{RF-18}), que muestra en
	...
	post-adopción (\hyperref[tab:RF-22]{RF-22}), que reutiliza el mismo hilo de mensajería para el
	contacto entre propietario original y adoptante tras completarse la
	adopción.
	
	\textbf{RF asociados:} \hyperref[tab:RF-05]{RF-05}, \hyperref[tab:RF-07]{RF-07}, \hyperref[tab:RF-08]{RF-08}, \hyperref[tab:RF-18]{RF-18}, \hyperref[tab:RF-22]{RF-22}.
	
	\begin{figure}[H]
		\centering
		\includegraphics[width=0.75\textwidth]{{figuras/MU-05_Solicitud_de_adopcion_o_cruza}.png}
		\caption{Mockup MU-05: Solicitud de adopción o cruza.}
		\label{fig:mu05}
	\end{figure}
	
	\paragraph{\hyperref[fig:mu06]{MU-06}: Perfil de mascota}
	...
	\textbf{RF asociados:} \hyperref[tab:RF-01]{RF-01}, \hyperref[tab:RF-02]{RF-02}, \hyperref[tab:RF-03]{RF-03}, \hyperref[tab:RF-13]{RF-13}, \hyperref[tab:RF-16]{RF-16}.
	
	\begin{figure}[H]
		\centering
		\includegraphics[width=0.42\textwidth]{{figuras/MU-06_Perfil_de_mascota}.png}
		\caption{Mockup MU-06: Perfil de mascota.}
		\label{fig:mu06}
	\end{figure}
	
	\paragraph{\hyperref[fig:mu07]{MU-07}: Evaluación de compatibilidad}
	...
	riesgo exigida por \hyperref[tab:RF-23]{RF-23}, sin requerir una pantalla propia adicional.
	
	\textbf{RF asociados:} \hyperref[tab:RF-06]{RF-06}, \hyperref[tab:RF-23]{RF-23}.
	
	\begin{figure}[H]
		\centering
		\includegraphics[width=0.68\textwidth]{{figuras/MU-07_Evaluacion_de_compatibilidad}.png}
		\caption{Mockup MU-07: Evaluación de compatibilidad.}
		\label{fig:mu07}
	\end{figure}
	
	\paragraph{\hyperref[fig:mu08]{MU-08}: Configuración de privacidad de la mascota}
	...
	\textbf{RF asociados:} \hyperref[tab:RF-11]{RF-11}.
	
	\begin{figure}[H]
		\centering
		\includegraphics[width=0.48\textwidth]{{figuras/MU-08_Configuracion_de_privacidad_de_la_mascota}.png}
	...
	{\footnotesize
		\begin{longtable}{@{}L{1.5cm} L{2.0cm} L{2.3cm} L{3.2cm} L{2.0cm} L{2.9cm}@{}}
			...
			\bottomrule
			\endlastfoot
			\hyperref[fig:mu01]{MU-01} & Crear cuenta & Propietario / Adoptante / Interesado en cruza / Veterinaria & Landing $\rightarrow$ \enquote{Crear cuenta} $\rightarrow$ Formulario de registro $\rightarrow$ Verificación de identidad & \hyperref[tab:RF-12]{RF-12} & Formaliza el registro y la verificación de identidad exigida antes de participar en procesos de adopción o cruza. \\
			\addlinespace
			\hyperref[fig:mu02]{MU-02} & Registrar mascota & Propietario de mascota & Dashboard $\rightarrow$ Mis Mascotas $\rightarrow$ \enquote{Agregar mascota} $\rightarrow$ Formulario de datos generales & \hyperref[tab:RF-01]{RF-01} & Captura los datos básicos y fotográficos que dan origen al perfil de la mascota dentro de la plataforma. \\
			\addlinespace
			\hyperref[fig:mu03]{MU-03} & Agenda veterinaria y validación documental & Veterinaria & Dashboard $\rightarrow$ Validaciones $\rightarrow$ Agenda de revisiones $\rightarrow$ Detalle del documento & \hyperref[tab:RF-09]{RF-09}, \hyperref[tab:RF-14]{RF-14} & Organiza cronológicamente la revisión de documentos sanitarios y del carnet de vacunación por parte de un profesional autorizado. \\
			\addlinespace
			\hyperref[fig:mu04]{MU-04} & Feed de publicaciones (búsqueda y filtrado) & Adoptante / Interesado en cruza & Menú principal $\rightarrow$ Explorar Mascotas $\rightarrow$ Filtros dinámicos & \hyperref[tab:RF-03]{RF-03}, \hyperref[tab:RF-04]{RF-04} & Presenta el catálogo de mascotas disponibles mediante tarjetas visuales y filtros de búsqueda combinables. \\
			\addlinespace
			\hyperref[fig:mu05]{MU-05} & Solicitud de adopción o cruza & Adoptante / Interesado en cruza & Feed de publicaciones $\rightarrow$ Tarjeta de mascota $\rightarrow$ Botón \enquote{Solicitar} $\rightarrow$ Formulario de solicitud & \hyperref[tab:RF-05]{RF-05}, \hyperref[tab:RF-07]{RF-07}, \hyperref[tab:RF-08]{RF-08}, \hyperref[tab:RF-18]{RF-18}, \hyperref[tab:RF-22]{RF-22} & Formaliza el interés sobre una mascota, recopila la justificación del solicitante, abre el canal de mensajería con el propietario y aloja, en secciones secundarias, el indicador de etapa del flujo de adopción y el seguimiento post-adopción. \\
			\addlinespace
			\hyperref[fig:mu06]{MU-06} & Perfil de mascota & Propietario / Adoptante / Interesado en cruza & Feed de publicaciones $\rightarrow$ Tarjeta de mascota $\rightarrow$ Ver perfil completo & \hyperref[tab:RF-01]{RF-01}, \hyperref[tab:RF-02]{RF-02}, \hyperref[tab:RF-03]{RF-03}, \hyperref[tab:RF-13]{RF-13}, \hyperref[tab:RF-16]{RF-16} & Centraliza la identidad, el historial médico referencial y los antecedentes genéticos de la mascota para sustentar decisiones de adopción o cruza. \\
			\addlinespace
			\hyperref[fig:mu07]{MU-07} & Evaluación de compatibilidad & Propietario de mascota / Interesado en cruza & Perfil de mascota $\rightarrow$ \enquote{Evaluar compatibilidad} $\rightarrow$ Selección de segunda mascota $\rightarrow$ Resultado & \hyperref[tab:RF-06]{RF-06}, \hyperref[tab:RF-23]{RF-23} & Presenta el resultado comparativo generado por el componente de inteligencia artificial junto con su explicación causal y, cuando corresponde, la alerta de riesgo superpuesta al resultado, sin sustituir la evaluación veterinaria. \\
			\addlinespace
			\hyperref[fig:mu08]{MU-08} & Configuración de privacidad de la mascota & Propietario de mascota & Dashboard $\rightarrow$ Configuración $\rightarrow$ Privacidad de mi mascota & \hyperref[tab:RF-11]{RF-11} & Permite al propietario definir qué información sensible (ubicación, contacto, afecciones específicas) es visible para otros usuarios. \\
		\end{longtable}
	}
	...
	restantes --- \hyperref[tab:RF-18]{RF-18} (flujo de adopción por etapas), \hyperref[tab:RF-22]{RF-22} (seguimiento
	post-adopción) y \hyperref[tab:RF-23]{RF-23} (alertas de riesgo) --- asociados formalmente a un
	...
	\hyperref[tab:RF-18]{RF-18} y \hyperref[tab:RF-22]{RF-22} quedan asociados a \hyperref[fig:mu05]{MU-05}, donde se resuelven como una
	sección secundaria que muestra la etapa de la solicitud (\hyperref[tab:RF-18]{RF-18}) y el hilo
	de mensajería reutilizado para el seguimiento (\hyperref[tab:RF-22]{RF-22}); y \hyperref[tab:RF-23]{RF-23} queda
	asociado a \hyperref[fig:mu07]{MU-07}, donde se despliega como una alerta contextual
	...
	\emph{Should} y \emph{Could} (\hyperref[tab:RF-04]{RF-04}, \hyperref[tab:RF-07]{RF-07}, \hyperref[tab:RF-10]{RF-10}, \hyperref[tab:RF-15]{RF-15}, \hyperref[tab:RF-17]{RF-17}, \hyperref[tab:RF-19]{RF-19} a
	\hyperref[tab:RF-21]{RF-21} y \hyperref[tab:RF-24]{RF-24} a \hyperref[tab:RF-27]{RF-27}) se apoyan de la misma manera sobre las pantallas ya
	...
	Sección~\ref{sec:entorno} y con la portabilidad exigida por \hyperref[tab:RNF-11]{RNF-11}. Las
	...
	\begin{itemize}[leftmargin=*]
		...
		navegador para capturar o adjuntar las fotografías de la mascota (\hyperref[tab:RF-01]{RF-01})
		y los documentos veterinarios digitalizados (\hyperref[tab:RF-14]{RF-14}). El sistema no accede
		...
		se almacena ni se muestra, conforme a \hyperref[tab:RNF-16]{RNF-16} y a la configuración de
		privacidad de \hyperref[tab:RF-11]{RF-11}.
		...
		computador de escritorio (\hyperref[tab:RNF-04]{RNF-04}, \hyperref[tab:RNF-11]{RNF-11}).
		...
		horizontal exigida por \hyperref[tab:RNF-12]{RNF-12}.
	\end{itemize}
	...
	{\footnotesize
		\begin{longtable}{@{}L{3.2cm} L{4.5cm} L{3.4cm} L{3.4cm}@{}}
			...
			\bottomrule
			\endlastfoot
			Navegador web del cliente & Renderizado de la interfaz de usuario y consumo de los servicios de la aplicación & HTTPS sobre HTML5, CSS3 y JavaScript; respuestas en JSON & \hyperref[tab:RNF-04]{RNF-04}, \hyperref[tab:RNF-11]{RNF-11} \\
			\addlinespace
			Componente de evaluación de compatibilidad (IA) & Generar el resultado orientativo de cruza y su explicación causal & API interna REST/JSON expuesta por el módulo de IA (Sección~\ref{sec:componentes}) & \hyperref[tab:RF-06]{RF-06}, \hyperref[tab:RNF-06]{RNF-06}, \hyperref[tab:RNF-13]{RNF-13}, \hyperref[tab:RD-01]{RD-01} \\
			\addlinespace
			Componente de validación de imágenes (IA) & Verificar que la fotografía corresponde a una mascota y es apropiada & API interna REST/JSON & \hyperref[tab:RF-17]{RF-17}, \hyperref[tab:RNF-07]{RNF-07}, \hyperref[tab:RNF-14]{RNF-14} \\
			\addlinespace
			Componente de moderación de mensajes (IA) & Clasificar el contenido de los mensajes antes de su entrega & API interna REST/JSON & \hyperref[tab:RF-07]{RF-07}, \hyperref[tab:RNF-15]{RNF-15} \\
			\addlinespace
			WhatsApp Business API & Envío de recordatorios de controles preventivos y notificaciones & API REST del proveedor sobre HTTPS, con canal de respaldo \emph{in-app} & \hyperref[tab:RF-10]{RF-10}, \hyperref[tab:RNF-09]{RNF-09}, \hyperref[tab:RD-08]{RD-08} \\
			\addlinespace
			Servicio de correo electrónico & Confirmación de cuenta, recuperación de contraseña y avisos del sistema & SMTP o API del proveedor sobre canal cifrado & \hyperref[tab:RF-12]{RF-12}, \hyperref[tab:RNF-09]{RNF-09} \\
			\addlinespace
			Sistema gestor de base de datos & Persistencia de la información transaccional del sistema & Conexión cifrada mediante el controlador del SGBD seleccionado & \hyperref[tab:RNF-01]{RNF-01}, \hyperref[tab:RNF-05]{RNF-05}, \hyperref[tab:RNF-12]{RNF-12} \\
			\addlinespace
			Almacenamiento de archivos multimedia & Persistencia de fotografías de mascotas y documentos veterinarios & API de almacenamiento de objetos sobre HTTPS, con URL firmadas de acceso temporal & \hyperref[tab:RF-01]{RF-01}, \hyperref[tab:RF-14]{RF-14}, \hyperref[tab:RNF-01]{RNF-01}, \hyperref[tab:RD-04]{RD-04} \\
		\end{longtable}
	}
	...
	diseño \hyperref[tab:RD-04]{RD-04}. Cuando un servicio externo no se encuentre disponible, el
	...
	sin interrumpir el resto de la operación, según lo previsto en \hyperref[tab:RD-08]{RD-08}.
	...
	\begin{atributos}{RF-01}{RF-01 --- Registrar mascota.}
		Identificador & \hyperref[tab:RF-01]{RF-01} \\
		Nombre & Registrar mascota \\
	...
	\begin{atributos}{RF-02}{RF-02 --- Gestionar historial médico de la mascota.}
		Identificador & \hyperref[tab:RF-02]{RF-02} \\
	...
	\begin{atributos}{RF-03}{RF-03 --- Consultar perfil completo de una mascota.}
		Identificador & \hyperref[tab:RF-03]{RF-03} \\
	...
	\begin{atributos}{RF-04}{RF-04 --- Buscar y filtrar mascotas.}
		Identificador & \hyperref[tab:RF-04]{RF-04} \\
	...
	\begin{atributos}{RF-05}{RF-05 --- Gestionar solicitudes de adopción.}
		Identificador & \hyperref[tab:RF-05]{RF-05} \\
		...
		Postcondiciones & La solicitud queda registrada como aceptada o rechazada en su revisión inicial; si es aceptada, continúa su ciclo de vida conforme al flujo por etapas de \hyperref[tab:RF-18]{RF-18}. \\
		Prioridad (MoSCoW) & Must \\
	...
	\begin{atributos}{RF-06}{RF-06 --- Evaluar compatibilidad entre mascotas para cruza.}
		Identificador & \hyperref[tab:RF-06]{RF-06} \\
		...
		Salidas & Veredicto de compatibilidad en tres niveles discretos --- \emph{compatible}, \emph{compatible con reservas} o \emph{no recomendado}---, acompañado de la explicación exigida por \hyperref[tab:RNF-13]{RNF-13} y de una advertencia de que el resultado es orientativo y no sustituye la evaluación de un veterinario. \\
	...
	\begin{atributos}{RF-07}{RF-07 --- Sistema de mensajería entre usuarios.}
		Identificador & \hyperref[tab:RF-07]{RF-07} \\
	...
	\begin{atributos}{RF-08}{RF-08 --- Gestionar solicitudes de cruza.}
		Identificador & \hyperref[tab:RF-08]{RF-08} \\
	...
	\begin{atributos}{RF-09}{RF-09 --- Validar la información médica de una mascota.}
		Identificador & \hyperref[tab:RF-09]{RF-09} \\
	...
	\begin{atributos}{RF-10}{RF-10 --- Gestionar recordatorios de controles preventivos.}
		Identificador & \hyperref[tab:RF-10]{RF-10} \\
	...
	\begin{atributos}{RF-11}{RF-11 --- Administrar la privacidad de la información.}
		Identificador & \hyperref[tab:RF-11]{RF-11} \\
	...
	\begin{atributos}{RF-12}{RF-12 --- Verificar la identidad de los usuarios.}
		Identificador & \hyperref[tab:RF-12]{RF-12} \\
	...
	\begin{atributos}{RF-13}{RF-13 --- Registrar publicaciones de mascotas.}
		Identificador & \hyperref[tab:RF-13]{RF-13} \\
	...
	\begin{atributos}{RF-14}{RF-14 --- Gestionar carnet de vacunación.}
		Identificador & \hyperref[tab:RF-14]{RF-14} \\
	...
	\begin{atributos}{RF-15}{RF-15 --- Consultar el historial de procesos de adopción y cruza.}
		Identificador & \hyperref[tab:RF-15]{RF-15} \\
	...
	\begin{atributos}{RF-16}{RF-16 --- Gestionar antecedentes genéticos y parentesco de la mascota.}
		Identificador & \hyperref[tab:RF-16]{RF-16} \\
	...
	\begin{atributos}{RF-17}{RF-17 --- Validar imágenes.}
		Identificador & \hyperref[tab:RF-17]{RF-17} \\
	...
	\begin{atributos}{RF-18}{RF-18 --- Gestionar el flujo de solicitud de adopción por etapas.}
		Identificador & \hyperref[tab:RF-18]{RF-18} \\
		...
		Precondiciones & La solicitud de adopción debe haber sido aceptada en su revisión inicial por el propietario (\hyperref[tab:RF-05]{RF-05}), y el usuario debe estar verificado (\hyperref[tab:RF-12]{RF-12}). \\
	...
	\begin{atributos}{RF-19}{RF-19 --- Validar certificados veterinarios antes de habilitar la cruza.}
		Identificador & \hyperref[tab:RF-19]{RF-19} \\
		...
		Precondiciones & Ambas mascotas deben estar registradas y contar con antecedentes genéticos (\hyperref[tab:RF-16]{RF-16}). \\
	...
	\begin{atributos}{RF-20}{RF-20 --- Coordinar encuentros de socialización supervisados.}
		Identificador & \hyperref[tab:RF-20]{RF-20} \\
	...
	\begin{atributos}{RF-21}{RF-21 --- Registrar el identificador de microchip de la mascota.}
		Identificador & \hyperref[tab:RF-21]{RF-21} \\
	...
	\begin{atributos}{RF-22}{RF-22 --- Dar seguimiento post-adopción.}
		Identificador & \hyperref[tab:RF-22]{RF-22} \\
		...
		Precondiciones & La adopción debe haberse completado (\hyperref[tab:RF-18]{RF-18}). \\
	...
	\begin{atributos}{RF-23}{RF-23 --- Emitir alertas de riesgo sanitario o físico en interacciones y cruces.}
		Identificador & \hyperref[tab:RF-23]{RF-23} \\
	...
	\begin{atributos}{RF-24}{RF-24 --- Registrar trazabilidad de cepa, lote y aplicador de cada vacuna.}
		Identificador & \hyperref[tab:RF-24]{RF-24} \\
		...
		Precondiciones & La mascota debe estar registrada y contar con carnet de vacunación (\hyperref[tab:RF-14]{RF-14}). \\
	...
	\begin{atributos}{RF-25}{RF-25 --- Publicar aviso de mascota extraviada.}
		Identificador & \hyperref[tab:RF-25]{RF-25} \\
	...
	\begin{atributos}{RF-26}{RF-26 --- Habilitar validación médica por segunda opinión veterinaria.}
		Identificador & \hyperref[tab:RF-26]{RF-26} \\
		...
		Precondiciones & La información médica de la mascota debe contar con una validación previa realizada por una veterinaria (\hyperref[tab:RF-09]{RF-09}). \\
	...
	\begin{atributos}{RF-27}{RF-27 --- Controlar y alertar sobre solicitudes repetitivas de cruza de una misma mascota.}
		Identificador & \hyperref[tab:RF-27]{RF-27} \\
		...
		Precondiciones & La mascota debe contar con al menos un proceso de cruza previo registrado (\hyperref[tab:RF-08]{RF-08}, \hyperref[tab:RF-15]{RF-15}). \\
	...
	\begin{table}[H]
		...
		\begin{tabularx}{\textwidth}{@{}p{5.2cm} p{3.2cm} X@{}}
			...
			\midrule
			Adecuación funcional & \hyperref[tab:RNF-06]{RNF-06}, \hyperref[tab:RNF-07]{RNF-07} & Cubierta desde la 1B \\
			Eficiencia del desempeño & \hyperref[tab:RNF-03]{RNF-03} & Cubierta desde la 1B \\
			Compatibilidad & \hyperref[tab:RNF-09]{RNF-09} & Cubierta desde la 2A \\
			Interacción de usuario (usabilidad) & \hyperref[tab:RNF-04]{RNF-04}, \hyperref[tab:RNF-13]{RNF-13}, \hyperref[tab:RNF-14]{RNF-14}, \hyperref[tab:RNF-15]{RNF-15} & Cubierta y ampliada con explicabilidad \\
			Fiabilidad & \hyperref[tab:RNF-02]{RNF-02}, \hyperref[tab:RNF-08]{RNF-08} & Cubierta desde la 1B \\
			Seguridad & \hyperref[tab:RNF-01]{RNF-01}, \hyperref[tab:RNF-05]{RNF-05}, \hyperref[tab:RNF-16]{RNF-16} & Cubierta y ampliada en esta entrega \\
			Mantenibilidad & \hyperref[tab:RNF-10]{RNF-10} & Cubierta desde la 2A \\
			Portabilidad & \hyperref[tab:RNF-11]{RNF-11} & Cubierta desde la 2A \\
			Flexibilidad & \hyperref[tab:RNF-12]{RNF-12} & Cubierta desde la 2A \\
			\bottomrule
		\end{tabularx}
	\end{table}
	...
	\begin{atributos}{RNF-01}{RNF-01 --- Protección de la información de propietarios y mascotas.}
		Identificador & \hyperref[tab:RNF-01]{RNF-01} \\
		Nombre & Protección de la información de propietarios y mascotas \\
		Característica ISO 25010 & Seguridad \\
		Descripción & El sistema deberá impedir el acceso no autorizado a la información personal de los propietarios y al historial médico de las mascotas, garantizando que durante las pruebas de seguridad el porcentaje de accesos no autorizados sea del 0\,\%. \\
	...
	\begin{atributos}{RNF-02}{RNF-02 --- Disponibilidad del sistema.}
		Identificador & \hyperref[tab:RNF-02]{RNF-02} \\
		Nombre & Disponibilidad del sistema \\
		Característica ISO 25010 & Fiabilidad \\
		Descripción & El sistema deberá mantener una disponibilidad mínima del 99\,\% mensual, permitiendo el acceso continuo a sus funcionalidades durante la operación normal de la plataforma. \\
	...
	\begin{atributos}{RNF-03}{RNF-03 --- Tiempo de respuesta del sistema.}
		Identificador & \hyperref[tab:RNF-03]{RNF-03} \\
	...
	\begin{atributos}{RNF-04}{RNF-04 --- Facilidad de uso de la plataforma.}
		Identificador & \hyperref[tab:RNF-04]{RNF-04} \\
	...
	\begin{atributos}{RNF-05}{RNF-05 --- Integridad de la información médica.}
		Identificador & \hyperref[tab:RNF-05]{RNF-05} \\
		Nombre & Integridad de la información médica \\
		Característica ISO 25010 & Seguridad \\
		Descripción & El sistema deberá garantizar que únicamente los propietarios puedan modificar el historial médico de su mascota, evitando que durante las pruebas se produzcan modificaciones no autorizadas. \\
	...
	\begin{atributos}{RNF-06}{RNF-06 --- Exactitud del componente de inteligencia artificial.}
		Identificador & \hyperref[tab:RNF-06]{RNF-06} \\
		Nombre & Exactitud del componente de inteligencia artificial \\
		Característica ISO 25010 & Adecuación funcional \\
		Descripción & El sistema deberá generar, mediante su componente de inteligencia artificial, recomendaciones de compatibilidad para procesos de cruza con una coincidencia mínima del 85\,\% respecto a las evaluaciones realizadas por médicos veterinarios durante las pruebas de validación. \\
	...
	\begin{atributos}{RNF-07}{RNF-07 --- Precisión en la validación de imágenes.}
		Identificador & \hyperref[tab:RNF-07]{RNF-07} \\
	...
	\begin{atributos}{RNF-08}{RNF-08 --- Actualización de la información registrada.}
		Identificador & \hyperref[tab:RNF-08]{RNF-08} \\
	...
	\begin{atributos}{RNF-09}{RNF-09 --- Interoperabilidad de notificaciones con canales externos.}
		Identificador & \hyperref[tab:RNF-09]{RNF-09} \\
		Nombre & Interoperabilidad de notificaciones con canales externos \\
		Característica ISO 25010 & Compatibilidad \\
		Descripción & El sistema deberá integrarse con servicios externos de mensajería (WhatsApp Business API) para el envío de recordatorios de controles preventivos (\hyperref[tab:RF-10]{RF-10}), garantizando la entrega correcta del mensaje sin duplicarlo ni perderlo. \\
	...
	\begin{atributos}{RNF-10}{RNF-10 --- Facilidad de mantenimiento del código.}
		Identificador & \hyperref[tab:RNF-10]{RNF-10} \\
		Nombre & Facilidad de mantenimiento del código \\
		Característica ISO 25010 & Mantenibilidad \\
		Descripción & El sistema deberá organizarse en módulos independientes (gestión de mascotas, adopción/cruza, mensajería, componente de IA), de forma que una modificación en un módulo no requiera cambios en más del 10\,\% de los archivos de otro módulo, en línea con \hyperref[tab:RD-05]{RD-05}. \\
	...
	\begin{atributos}{RNF-11}{RNF-11 --- Portabilidad entre navegadores y dispositivos.}
		Identificador & \hyperref[tab:RNF-11]{RNF-11} \\
	...
	\begin{atributos}{RNF-12}{RNF-12 --- Escalabilidad ante crecimiento de usuarios.}
		Identificador & \hyperref[tab:RNF-12]{RNF-12} \\
		Nombre & Escalabilidad ante crecimiento de usuarios \\
		Característica ISO 25010 & Flexibilidad \\
		Descripción & El sistema deberá mantener un tiempo de respuesta igual o menor a 3 segundos en el percentil 95 (P95) al triplicar la carga nominal de usuarios concurrentes (hasta 300 usuarios concurrentes), permitiendo escalar sin necesidad de cambios en el código. \\
	...
	\begin{atributos}{RNF-13}{RNF-13 --- Explicabilidad de la recomendación de compatibilidad para cruza (RF-06).}
		Identificador & \hyperref[tab:RNF-13]{RNF-13} \\
		...
		Descripción & El sistema deberá presentar la explicación causal del resultado de compatibilidad con una extensión máxima de 60 palabras, un tiempo de generación no mayor a 2 segundos, y una calificación media de comprensión igual o superior a 4/5 en la escala Likert (1–5) al evaluarse con usuarios técnicos y no técnicos. \\
		...
		Responsable & Morales Sánchez Gary Alejandro. \\
		Frecuencia de medición & Verificación funcional en cada versión desplegada; encuesta de comprensión trimestral sobre una muestra de usuarios activos. \\
	\end{atributos}
	...
	\begin{atributos}{RNF-14}{RNF-14 --- Explicabilidad del rechazo o aceptación de imágenes (RF-17).}
		Identificador & \hyperref[tab:RNF-14]{RNF-14} \\
		Nombre & Explicabilidad del rechazo o aceptación de imágenes \\
		Característica ISO 25010 & Interacción de usuario (Explicabilidad) \\
		Descripción & El sistema deberá presentar la explicación por contraste del rechazo de una imagen con una extensión máxima de 30 palabras, garantizando que el 100 \% de los rechazos incluyan dicha explicación, y alcanzando una calificación media de comprensión igual o superior a 4/5 en la escala Likert (1–5) al evaluarse con usuarios representativos. \\
		...
		Responsable & Mendoza Párraga Andy Johel. \\
		Frecuencia de medición & Verificación funcional en cada versión desplegada; encuesta de comprensión trimestral sobre una muestra de rechazos registrados. \\
	\end{atributos}
	...
	\begin{atributos}{RNF-15}{RNF-15 --- Explicabilidad de la moderación de mensajes.}
		Identificador & \hyperref[tab:RNF-15]{RNF-15} \\
		Nombre & Explicabilidad de la moderación de mensajes \\
		Característica ISO 25010 & Interacción de usuario (Explicabilidad) \\
		Descripción & El sistema deberá presentar la categoría del motivo de un mensaje reportado con un nivel de comprensión suficiente para el remitente, alcanzando una calificación media igual o superior a 4/5 en la escala Likert (1–5) al evaluarse con usuarios representativos, sin exceder las 20 palabras por notificación. \\
		...
		Prioridad (MoSCoW) & Should \\
		Criterio de verificación & Reportar un mensaje de prueba mediante el mecanismo de \hyperref[tab:RF-07]{RF-07} y comprobar que el remitente reportado recibe la categoría del motivo señalado. \\
		Responsable & Fuertes Arraes Edson Daniel. \\
		Frecuencia de medición & Verificación funcional en cada versión desplegada; encuesta de comprensión trimestral sobre una muestra de reportes procesados. \\
	\end{atributos}
	...
	\begin{atributos}{RNF-16}{RNF-16 --- Confidencialidad de datos de ubicación y contacto.}
		Identificador & \hyperref[tab:RNF-16]{RNF-16} \\
		Nombre & Confidencialidad de datos de ubicación y contacto \\
		Característica ISO 25010 & Seguridad \\
		Descripción & El sistema deberá mostrar únicamente la zona aproximada (radio $\geq$ 1 km), y no la ubicación exacta ni el contacto directo, al consultar el perfil de una mascota desde una cuenta distinta a la del propietario y sin permiso explícito otorgado sobre esa mascota. \\
	...
	(\hyperref[rl:06]{RL-06}, Tabla~\ref{tab:legales}) reconoce al titular el derecho a no ser objeto
	...
	componente y el tipo de explicación exigido; los requisitos \hyperref[tab:RNF-13]{RNF-13}, \hyperref[tab:RNF-14]{RNF-14} y
	\hyperref[tab:RNF-15]{RNF-15} especifican estas necesidades de forma medible y verificable.
	...
	\begin{table}[H]
		...
		\begin{tabularx}{\textwidth}{@{}p{2.0cm} p{3.0cm} p{2.6cm} X@{}}
			...
			\midrule
			\hyperref[tab:RF-06]{RF-06} & Recomendación de compatibilidad para cruza & Propietario de mascota, interesado en cruza & Explicación causal de los factores considerados (raza, edad, estado de salud, antecedentes genéticos) --- especificada en \hyperref[tab:RNF-13]{RNF-13} \\
			\addlinespace
			\hyperref[tab:RF-17]{RF-17} & Aceptación o rechazo de una imagen cargada & Propietario de mascota, veterinaria & Explicación por contraste con el motivo específico del rechazo --- especificada en \hyperref[tab:RNF-14]{RNF-14} \\
			\addlinespace
			Moderación & Marcado de un mensaje como inapropiado & Usuarios del módulo de mensajería (\hyperref[tab:RF-07]{RF-07}) & Explicación por ejemplos con la categoría de infracción detectada --- especificada en \hyperref[tab:RNF-15]{RNF-15} \\
			\bottomrule
		\end{tabularx}
	\end{table}
	Los mismos tres componentes cuentan además con un requisito de equidad,
	un mecanismo de supervisión humana, una clasificación de riesgo y un
	plan de monitoreo en producción, detallados en la
	Sección~\ref{sec:equidad-riesgo-ia}.
	
\subsubsection{Equidad, clasificación de riesgo y plan de monitoreo de los componentes de inteligencia artificial}
\label{sec:equidad-riesgo-ia}

Complementando la exactitud (\hyperref[tab:RNF-06]{RNF-06}, \hyperref[tab:RNF-07]{RNF-07}) y la explicabilidad (\hyperref[tab:RNF-13]{RNF-13},
\hyperref[tab:RNF-14]{RNF-14}, \hyperref[tab:RNF-15]{RNF-15}) ya especificadas, esta sección define, para cada uno de los
tres componentes de inteligencia artificial de MundiPets, su requisito de
equidad, su mecanismo de supervisión humana, su clasificación de riesgo y
su plan de monitoreo en producción, de modo que ningún componente de IA
quede solo declarado sin una especificación medible de estos cuatro
aspectos.

\paragraph{Requisitos de equidad.}
Cada componente mide la diferencia de desempeño entre subgrupos con
distinta representación en los datos de referencia, con una brecha máxima
tolerada expresada en puntos porcentuales. La Tabla~\ref{tab:equidad-ia}
detalla el requisito de equidad de cada componente.

\begin{table}[H]
	\centering
	\caption{Requisitos de equidad de los componentes de inteligencia artificial.}
	\label{tab:equidad-ia}
	\small
	\begin{tabularx}{\textwidth}{@{}p{1.9cm} X p{1.7cm} p{1.6cm} p{1.7cm}@{}}
		\toprule
		\textbf{Componente} & \textbf{Métrica y subgrupos comparados} & \textbf{Brecha máxima tolerada} & \textbf{Responsable} & \textbf{Frecuencia} \\
		\midrule
		\emph{AI compatibility} (\hyperref[tab:RF-06]{RF-06}) & Diferencia de tasa de coincidencia con la evaluación veterinaria de referencia entre razas con más de 30 casos en el conjunto de evaluación y razas con menos de 30 casos. & 10 puntos porcentuales & Morales Sánchez Gary Alejandro & Trimestral, sobre el acumulado de recomendaciones emitidas \\
		\addlinespace
		\emph{AI image validation} (\hyperref[tab:RF-17]{RF-17}) & Diferencia de tasa de clasificación correcta entre fotografías tomadas en interiores y en exteriores, condición que afecta la iluminación y la nitidez de la imagen. & 10 puntos porcentuales & Mendoza Párraga Andy Johel & Trimestral, sobre el acumulado de imágenes procesadas \\
		\addlinespace
		Moderación de mensajes (\hyperref[tab:RF-07]{RF-07}) & Diferencia de tasa de falsos bloqueos entre mensajes de longitud breve (menos de 15 palabras) y mensajes de longitud extensa, dado que los mensajes breves ofrecen menos contexto para la clasificación. & 10 puntos porcentuales & Fuertes Arraes Edson Daniel & Trimestral, sobre el acumulado de mensajes bloqueados \\
		\bottomrule
	\end{tabularx}
\end{table}

\paragraph{Requisitos de supervisión humana.}
Cada componente define quién puede revisar, modificar o descartar su
salida antes de que produzca un efecto sobre el usuario, y cómo queda
registrada esa intervención. La Tabla~\ref{tab:supervision-ia} detalla el
mecanismo de supervisión de cada componente.

\begin{table}[H]
	\centering
	\caption{Requisitos de supervisión humana de los componentes de inteligencia artificial.}
	\label{tab:supervision-ia}
	\small
	\begin{tabularx}{\textwidth}{@{}p{1.9cm} X p{1.7cm} p{1.7cm}@{}}
		\toprule
		\textbf{Componente} & \textbf{Mecanismo de supervisión y registro} & \textbf{Responsable} & \textbf{Frecuencia} \\
		\midrule
		\emph{AI compatibility} (\hyperref[tab:RF-06]{RF-06}) & El interesado en cruza recibe la recomendación como apoyo y decide si solicita el contacto con el propietario de la otra mascota; ninguna recomendación autoriza un proceso de cruza por sí sola. Cada decisión del usuario sobre la recomendación queda registrada en el historial de la solicitud. & Morales Sánchez Gary Alejandro & Continua, sobre cada recomendación emitida \\
		\addlinespace
		\emph{AI image validation} (\hyperref[tab:RF-17]{RF-17}) & El propietario puede volver a cargar otra fotografía tras un rechazo, y puede solicitar revisión manual si considera que el rechazo fue incorrecto; esa solicitud queda registrada junto con la imagen original. & Mendoza Párraga Andy Johel & Continua, sobre cada solicitud de revisión manual recibida \\
		\addlinespace
		Moderación de mensajes (\hyperref[tab:RF-07]{RF-07}) & El remitente de un mensaje bloqueado puede apelar el bloqueo por el canal de soporte; el mensaje original permanece disponible para que una persona revise la decisión y la confirme o la revierta, quedando registrado el resultado de esa revisión. & Fuertes Arraes Edson Daniel & Continua, sobre cada apelación recibida \\
		\bottomrule
	\end{tabularx}
\end{table}

\paragraph{Clasificación de riesgo.}
Conforme al enfoque basado en niveles de riesgo del Reglamento (UE)
2024/1689 \cite{euaiact2024}, ninguno de los tres componentes de MundiPets
pertenece a la categoría de riesgo inaceptable ni de alto riesgo: ninguno
realiza identificación biométrica, puntuación social ni evaluación de
acceso a servicios esenciales. La Tabla~\ref{tab:riesgo-ia} detalla la
clasificación de cada componente.

\begin{table}[H]
	\centering
	\caption{Clasificación de riesgo de los componentes de inteligencia artificial.}
	\label{tab:riesgo-ia}
	\small
	\begin{tabularx}{\textwidth}{@{}p{2.3cm} p{2.0cm} X@{}}
		\toprule
		\textbf{Componente} & \textbf{Nivel} & \textbf{Justificación} \\
		\midrule
		\emph{AI compatibility} (\hyperref[tab:RF-06]{RF-06}) & Riesgo limitado & Influye en una decisión con efecto sobre el bienestar animal (proceso de cruza) y está sujeto al derecho a explicación del Art.~20 LOPDP (\hyperref[rl:06]{RL-06}), lo que exige las obligaciones de transparencia ya cubiertas por \hyperref[tab:RNF-13]{RNF-13}. \\
		\addlinespace
		\emph{AI image validation} (\hyperref[tab:RF-17]{RF-17}) & Riesgo mínimo/limitado & Su salida (aceptar o rechazar una fotografía) no afecta la salud ni la seguridad de personas o animales; al tratarse igualmente de una valoración automatizada, está sujeto al mismo derecho a explicación del Art.~20 LOPDP (\hyperref[rl:06]{RL-06}), cuya obligación de transparencia ya está cubierta por \hyperref[tab:RNF-14]{RNF-14}. \\
		\addlinespace
		Moderación de mensajes (\hyperref[tab:RF-07]{RF-07}) & Riesgo mínimo/limitado & Su salida (bloquear o permitir un mensaje) es revisable por los propios usuarios mediante el canal de comunicación directa; al tratarse igualmente de una valoración automatizada, está sujeto al mismo derecho a explicación del Art.~20 LOPDP (\hyperref[rl:06]{RL-06}), cuya obligación de transparencia ya está cubierta por \hyperref[tab:RNF-15]{RNF-15}. \\
		\bottomrule
	\end{tabularx}
\end{table}

En los tres casos, la salida del componente es de apoyo y no de decisión
definitiva: \emph{AI compatibility} no autoriza ni rechaza un proceso de
cruza, \emph{AI image validation} no impide de forma permanente la
publicación de una mascota (el propietario puede volver a intentarlo con
otra fotografía), y la moderación de mensajes no elimina el mensaje
original, que permanece disponible para su revisión. Ninguno de los tres
componentes reemplaza la evaluación veterinaria profesional exigida en
otras partes del sistema (\hyperref[tab:RF-09]{RF-09}, \hyperref[tab:CU-09]{CU-09}).

\paragraph{Plan de monitoreo en producción.}
La Tabla~\ref{tab:monitoreo-ia} define, para cada componente, el
indicador de desempeño en producción, su frecuencia de medición, el
umbral de alerta por deriva y el criterio de reentrenamiento o retirada
temporal.
...
\begin{table}[H]
	...
	\small
	\begin{tabularx}{\textwidth}{@{}p{2.0cm} X p{4.3cm}@{}}
		\toprule
		\textbf{Componente} & \textbf{Indicador y frecuencia} & \textbf{Umbral de alerta y acción} \\
		\midrule
		\emph{AI compatibility} & Coincidencia con la evaluación veterinaria (\hyperref[tab:RNF-06]{RNF-06}), medida mensualmente sobre una muestra auditada de recomendaciones emitidas. & Caída sostenida por debajo del 85\,\% durante dos meses consecutivos: reentrenar con los casos auditados del período; si no se recupera el umbral en el siguiente ciclo, retirar temporalmente el componente y operar en modo manual (evaluación veterinaria directa). \\
		\addlinespace
		\emph{AI image validation} & Tasa de clasificación correcta (\hyperref[tab:RNF-07]{RNF-07}), medida mensualmente sobre una muestra de imágenes revisadas manualmente como control de calidad. & Caída sostenida por debajo del 95\,\% durante dos meses consecutivos: reentrenar incorporando los casos mal clasificados detectados; si no se recupera, derivar temporalmente las imágenes a revisión manual antes de la publicación. \\
		\addlinespace
...
\end{table}
	...
	\begin{table}[H]
		...
		\begin{tabularx}{\textwidth}{@{}p{1.1cm} p{1.9cm} p{5.4cm} X@{}}
			...
			\midrule
			\hyperref[rl:01]{RL-01}\label{rl:01} & Art. 7 y 8 & El tratamiento de datos personales del propietario debe basarse en un consentimiento libre, específico, informado e inequívoco, revocable en cualquier momento. & \hyperref[tab:RF-01]{RF-01} (registro con aceptación de términos), \hyperref[tab:RF-12]{RF-12} (verificación de identidad), Apéndice B.6 (consentimientos de campo) cubren el otorgamiento del consentimiento; la revocación en cualquier momento no cuenta aún con un RF propio (véase nota posterior a esta tabla) \\
			\addlinespace
			\hyperref[rl:02]{RL-02}\label{rl:02} & Art. 12 y 13 & El titular debe ser informado sobre la finalidad, el responsable y las transferencias de sus datos, y tiene derecho a acceder gratuitamente a ellos dentro de 15 días. & \hyperref[tab:RF-03]{RF-03} (consultar perfil completo), \hyperref[tab:RF-11]{RF-11} (privacidad) \\
			\addlinespace
			\hyperref[rl:03]{RL-03}\label{rl:03} & Art. 14 & El titular tiene derecho a la rectificación y actualización de sus datos inexactos o incompletos, en un plazo de 15 días. & \hyperref[tab:RF-02]{RF-02} (gestionar historial médico), \hyperref[tab:RF-14]{RF-14} (carnet de vacunación) \\
			\addlinespace
			\hyperref[rl:04]{RL-04}\label{rl:04} & Art. 15 & El titular tiene derecho a solicitar la eliminación de sus datos personales cuando revoque el consentimiento o estos ya no sean necesarios. & \hyperref[tab:RD-06]{RD-06} (Mecanismo de eliminación de cuenta y datos personales) \\
			\addlinespace
			\hyperref[rl:05]{RL-05}\label{rl:05} & Art. 10 lit. e) y j) & Los datos deben limitarse a lo estrictamente necesario para la finalidad (minimización) y protegerse frente a accesos no autorizados (seguridad). & \hyperref[tab:RF-11]{RF-11} (privacidad), \hyperref[tab:RNF-01]{RNF-01} y \hyperref[tab:RNF-16]{RNF-16} (protección/confidencialidad de ubicación y contacto) \\
			\addlinespace
			\hyperref[rl:06]{RL-06}\label{rl:06} & Art. 20 & El titular tiene derecho a no ser objeto de una decisión basada únicamente en valoraciones automatizadas sin recibir una explicación motivada sobre ella. & \hyperref[tab:RF-06]{RF-06} (evaluación de compatibilidad por IA), \hyperref[tab:RNF-13]{RNF-13}, \hyperref[tab:RNF-14]{RNF-14}, \hyperref[tab:RNF-15]{RNF-15} (explicabilidad) --- este artículo es la base legal directa de todo el bloque de explicabilidad \\
			\addlinespace
			\hyperref[rl:07]{RL-07}\label{rl:07} & Art. 25, 26 y 30 & Los datos sensibles y los datos de salud del propietario requieren consentimiento explícito y confidencialidad reforzada; el historial clínico de la mascota se protege como extensión indirecta de esos datos. & \hyperref[tab:RD-04]{RD-04} (protección de datos personales), \hyperref[tab:RNF-01]{RNF-01}, \hyperref[tab:RNF-05]{RNF-05} \\
			\addlinespace
			\hyperref[rl:08]{RL-08}\label{rl:08} & Art. 39 & El sistema debe implementar protección de datos desde el diseño y por defecto, tratando solo los datos estrictamente necesarios para cada finalidad. & \hyperref[tab:RD-02]{RD-02} (acceso basado en roles), \hyperref[tab:RNF-16]{RNF-16} (confidencialidad de ubicación/contacto) \\
			\addlinespace
			\hyperref[rl:09]{RL-09}\label{rl:09} & Art. 43 y 46 & Ante una vulneración de seguridad que afecte derechos del titular, debe notificarse a la Autoridad de Protección de Datos en máximo 5 días y al titular afectado en máximo 3 días. & \hyperref[tab:RD-07]{RD-07} (Protocolo de notificación de vulneraciones de seguridad) \\
			\addlinespace
			\bottomrule
		\end{tabularx}
	\end{table}
	...
	Las restricciones de diseño acotan el espacio de soluciones admisibles.
	Las restricciones \hyperref[tab:RD-06]{RD-06} a \hyperref[tab:RD-09]{RD-09} derivan del análisis legal de la
	Sección~\ref{sec:legales}, que evidencia obligaciones de la LOPDP sin
	cobertura en el resto de restricciones.
	...
	\begin{atributos}{RD-01}{RD-01 --- Integración de componentes de inteligencia artificial.}
		Identificador & \hyperref[tab:RD-01]{RD-01} \\
	...
	\begin{atributos}{RD-02}{RD-02 --- Acceso basado en roles.}
		Identificador & \hyperref[tab:RD-02]{RD-02} \\
		...
		Justificación & Las entrevistas muestran que cada tipo de usuario realiza funciones distintas y accede a diferente información; refuerza además \hyperref[rl:05]{RL-05} y \hyperref[rl:08]{RL-08} (minimización y protección desde el diseño, Art. 10 y 39 LOPDP). \\
	...
	\begin{atributos}{RD-03}{RD-03 --- Validación profesional de la información médica.}
		Identificador & \hyperref[tab:RD-03]{RD-03} \\
	...
	\begin{atributos}{RD-04}{RD-04 --- Protección de datos personales.}
		Identificador & \hyperref[tab:RD-04]{RD-04} \\
		...
		Justificación & Los entrevistados consideran prioritaria la privacidad de la información registrada (EV-09, EV-10, EV-18); implementa directamente \hyperref[rl:05]{RL-05}, \hyperref[rl:07]{RL-07} y \hyperref[rl:08]{RL-08}. \\
	...
	\begin{atributos}{RD-05}{RD-05 --- Arquitectura modular del sistema.}
		Identificador & \hyperref[tab:RD-05]{RD-05} \\
		...
		Justificación & La plataforma integra funcionalidades de gestión, adopción, cruza e IA, por lo que una arquitectura modular facilitará su evolución y mantenimiento; sustenta directamente \hyperref[tab:RNF-10]{RNF-10} (mantenibilidad). \\
	...
	\begin{atributos}{RD-06}{RD-06 --- Mecanismo de eliminación de cuenta y datos personales.}
		Identificador & \hyperref[tab:RD-06]{RD-06} \\
		...
		Justificación & El Art. 15 de la LOPDP reconoce el derecho de eliminación del titular; ningún RF de los 27 vigentes cubre esta función (vacío detectado en 3.4, \hyperref[rl:04]{RL-04}). \\
	...
	\begin{atributos}{RD-07}{RD-07 --- Protocolo de notificación de vulneraciones de seguridad.}
		Identificador & \hyperref[tab:RD-07]{RD-07} \\
		...
		Justificación & Los Art. 43 y 46 de la LOPDP exigen esta notificación; no existía ningún RD que la contemplara (vacío detectado en 3.4, \hyperref[rl:09]{RL-09}). \\
	...
	\begin{atributos}{RD-08}{RD-08 --- Dependencia de servicios externos de mensajería.}
		Identificador & \hyperref[tab:RD-08]{RD-08} \\
		Nombre & Dependencia de servicios externos de mensajería \\
		Descripción & El sistema deberá integrarse con un proveedor externo de mensajería (WhatsApp Business API) para el envío de recordatorios (\hyperref[tab:RF-10]{RF-10}), quedando su disponibilidad condicionada a las políticas y disponibilidad de dicho proveedor. \\
		Justificación & La evidencia de campo (EV-11) muestra una fuerte preferencia de los usuarios por recibir recordatorios por ese canal; sustenta \hyperref[tab:RNF-09]{RNF-09} (compatibilidad). \\
		Impacto & Arquitectura de integración externa; requiere canal de respaldo (notificación in-app) ante caídas del proveedor. \\
		Prioridad (MoSCoW) & Should \\
	\end{atributos}
	
	\subsubsection{RD-09 --- Anonimización de evidencias de campo para publicación abierta}
	
	\begin{atributos}{RD-09}{RD-09 --- Anonimización de evidencias de campo para publicación abierta.}
		Identificador & \hyperref[tab:RD-09]{RD-09} \\
		...
		Impacto & Proceso de preparación de datos (paquete FAIR), scripts de anonimización, README\_dataset.md. \\
		Prioridad (MoSCoW) & Must \\
	\end{atributos}
	...
	historias \hyperref[tab:HU-01]{HU-01} a \hyperref[tab:HU-15]{HU-15} corresponden a los quince RF de prioridad
	\emph{Must}; las historias \hyperref[tab:HU-16]{HU-16} a \hyperref[tab:HU-27]{HU-27}, incorporadas en esta entrega,
	corresponden a los doce RF de prioridad \emph{Should} y \emph{Could}
	(\hyperref[tab:RF-04]{RF-04}, \hyperref[tab:RF-07]{RF-07}, \hyperref[tab:RF-10]{RF-10}, \hyperref[tab:RF-15]{RF-15}, \hyperref[tab:RF-17]{RF-17}, \hyperref[tab:RF-19]{RF-19} a \hyperref[tab:RF-21]{RF-21} y \hyperref[tab:RF-24]{RF-24} a \hyperref[tab:RF-27]{RF-27}). La
	...
	\begin{atributos}{HU-01}{HU-01 --- Historia de usuario asociada al RF-01.}
		Identificador & \hyperref[tab:HU-01]{HU-01} \\
		RF asociado & \hyperref[tab:RF-01]{RF-01} \\
		...
		Criterio de aceptación & \hyperref[lst:ca01]{CA-01} (véase el escenario Gherkin siguiente). \\
	\end{atributos}
	...
	\begin{atributos}{HU-02}{HU-02 --- Historia de usuario asociada al RF-02.}
		Identificador & \hyperref[tab:HU-02]{HU-02} \\
		RF asociado & \hyperref[tab:RF-02]{RF-02} \\
		...
		Independiente (I) & Depende únicamente de que la mascota ya esté registrada (\hyperref[tab:RF-01]{RF-01}), no de ninguna otra historia de usuario. \\
		...
		Estimable (E) & Estimable porque reutiliza el mismo patrón de formulario que \hyperref[tab:HU-01]{HU-01}, con una lista de eventos clínicos. \\
		Pequeña (S) & Se limita a registrar y actualizar el historial, sin incluir la validación por veterinaria (esa es \hyperref[tab:HU-07]{HU-07}). \\
		Verificable (T) & Verificable registrando un nuevo antecedente médico y comprobando que aparece en el historial. \\
		Criterio de aceptación & \hyperref[lst:ca02]{CA-02} (véase el escenario Gherkin siguiente). \\
	\end{atributos}
	...
	\begin{atributos}{HU-03}{HU-03 --- Historia de usuario asociada al RF-03.}
		Identificador & \hyperref[tab:HU-03]{HU-03} \\
		RF asociado & \hyperref[tab:RF-03]{RF-03} \\
		...
		Independiente (I) & Puede desarrollarse en paralelo a otras historias porque solo depende de que existan mascotas registradas (\hyperref[tab:RF-01]{RF-01}). \\
		...
		Pequeña (S) & Se limita a la consulta; no incluye la lógica de permisos avanzados de privacidad (esa es \hyperref[tab:HU-08]{HU-08}). \\
		Verificable (T) & Verificable consultando una mascota registrada y comprobando que se muestra toda la información autorizada. \\
		Criterio de aceptación & \hyperref[lst:ca03]{CA-03} (véase el escenario Gherkin siguiente). \\
	\end{atributos}
	...
	\begin{atributos}{HU-04}{HU-04 --- Historia de usuario asociada al RF-05.}
		Identificador & \hyperref[tab:HU-04]{HU-04} \\
		RF asociado & \hyperref[tab:RF-05]{RF-05} \\
		...
		Independiente (I) & Depende de que la mascota esté publicada (\hyperref[tab:RF-13]{RF-13}) y el usuario esté verificado (\hyperref[tab:RF-12]{RF-12}), pero puede probarse de forma independiente con datos de prueba. \\
		...
		Pequeña (S) & Cubre solo el envío y la decisión inicial, sin las etapas posteriores del proceso (esas están en \hyperref[tab:HU-13]{HU-13}). \\
		Verificable (T) & Verificable registrando una solicitud y comprobando que el propietario puede aceptarla o rechazarla. \\
		Criterio de aceptación & \hyperref[lst:ca04]{CA-04} (véase el escenario Gherkin siguiente). \\
	\end{atributos}
	...
	\begin{atributos}{HU-05}{HU-05 --- Historia de usuario asociada al RF-06.}
		Identificador & \hyperref[tab:HU-05]{HU-05} \\
		RF asociado & \hyperref[tab:RF-06]{RF-06} \\
		...
		Independiente (I) & Depende de que ambas mascotas tengan antecedentes genéticos (\hyperref[tab:RF-16]{RF-16}) e historial médico (\hyperref[tab:RF-02]{RF-02}) registrados, pero la lógica de evaluación puede construirse y probarse de forma aislada. \\
		...
		Pequeña (S) & Pequeña dentro de su alcance porque solo produce el resultado comparativo, sin incluir la alerta de riesgo (esa es \hyperref[tab:HU-15]{HU-15}). \\
		...
		Criterio de aceptación & \hyperref[lst:ca05]{CA-05} (véase el escenario Gherkin siguiente). \\
	\end{atributos}
	...
	\begin{atributos}{HU-06}{HU-06 --- Historia de usuario asociada al RF-08.}
		Identificador & \hyperref[tab:HU-06]{HU-06} \\
		RF asociado & \hyperref[tab:RF-08]{RF-08} \\
		...
		Independiente (I) & Depende de que ambas mascotas estén registradas y publicadas para cruza (\hyperref[tab:RF-13]{RF-13}), pero su lógica de aceptación o rechazo es independiente de otras historias. \\
		...
		Estimable (E) & Estimable porque reutiliza el mismo patrón de máquina de estados que \hyperref[tab:HU-04]{HU-04}. \\
		Pequeña (S) & Limitada al envío y respuesta de la solicitud, sin incluir la validación de certificados veterinarios (esa es \hyperref[tab:RF-19]{RF-19}, Should). \\
		...
		Criterio de aceptación & \hyperref[lst:ca06]{CA-06} (véase el escenario Gherkin siguiente). \\
	\end{atributos}
	...
	\begin{atributos}{HU-07}{HU-07 --- Historia de usuario asociada al RF-09.}
		Identificador & \hyperref[tab:HU-07]{HU-07} \\
		RF asociado & \hyperref[tab:RF-09]{RF-09} \\
		...
		Independiente (I) & Depende de que exista un historial médico cargado (\hyperref[tab:RF-02]{RF-02}), pero la acción de validación es una funcionalidad independiente y acotada. \\
		...
		Pequeña (S) & Se limita a la validación; no incluye la carga original del historial (esa es \hyperref[tab:HU-02]{HU-02}). \\
		...
		Criterio de aceptación & \hyperref[lst:ca07]{CA-07} (véase el escenario Gherkin siguiente). \\
	\end{atributos}
	...
	\begin{atributos}{HU-08}{HU-08 --- Historia de usuario asociada al RF-11.}
		Identificador & \hyperref[tab:HU-08]{HU-08} \\
		RF asociado & \hyperref[tab:RF-11]{RF-11} \\
		...
		Criterio de aceptación & \hyperref[lst:ca08]{CA-08} (véase el escenario Gherkin siguiente). \\
	\end{atributos}
	...
	\begin{atributos}{HU-09}{HU-09 --- Historia de usuario asociada al RF-12.}
		Identificador & \hyperref[tab:HU-09]{HU-09} \\
		RF asociado & \hyperref[tab:RF-12]{RF-12} \\
		...
		Criterio de aceptación & \hyperref[lst:ca09]{CA-09} (véase el escenario Gherkin siguiente). \\
	\end{atributos}
	...
	\begin{atributos}{HU-10}{HU-10 --- Historia de usuario asociada al RF-13.}
		Identificador & \hyperref[tab:HU-10]{HU-10} \\
		RF asociado & \hyperref[tab:RF-13]{RF-13} \\
		...
		Independiente (I) & Depende de que la mascota esté registrada (\hyperref[tab:RF-01]{RF-01}), pero es independiente de las solicitudes que después se generen sobre ella. \\
		...
		Criterio de aceptación & \hyperref[lst:ca10]{CA-10} (véase el escenario Gherkin siguiente). \\
	\end{atributos}
	...
	\begin{atributos}{HU-11}{HU-11 --- Historia de usuario asociada al RF-14.}
		Identificador & \hyperref[tab:HU-11]{HU-11} \\
		RF asociado & \hyperref[tab:RF-14]{RF-14} \\
		...
		Independiente (I) & Depende de que la mascota esté registrada (\hyperref[tab:RF-01]{RF-01}), pero no de ninguna otra historia Must. \\
		Negociable (N) & Negociable si se incluye un recordatorio automático dentro de esta historia, o si eso se deja a \hyperref[tab:RF-10]{RF-10} (Should). \\
		...
		Estimable (E) & Estimable porque reutiliza el mismo patrón de historial de \hyperref[tab:HU-02]{HU-02}. \\
		Pequeña (S) & Cubre el registro y consulta del carnet, sin la trazabilidad de cepa y lote (\hyperref[tab:RF-24]{RF-24}, Should, fuera del alcance Must). \\
		Verificable (T) & Verificable registrando una vacuna y comprobando que aparece en el carnet. \\
		Criterio de aceptación & \hyperref[lst:ca11]{CA-11} (véase el escenario Gherkin siguiente). \\
	\end{atributos}
	...
	\begin{atributos}{HU-12}{HU-12 --- Historia de usuario asociada al RF-16.}
		Identificador & \hyperref[tab:HU-12]{HU-12} \\
		RF asociado & \hyperref[tab:RF-16]{RF-16} \\
		...
		Independiente (I) & Depende de que la mascota esté registrada (\hyperref[tab:RF-01]{RF-01}), pero es independiente de la lógica de evaluación de compatibilidad (\hyperref[tab:RF-06]{RF-06}), que solo consume esta información. \\
		Negociable (N) & El nivel de detalle del árbol genealógico (cuántas generaciones se registran) es negociable con el equipo. \\
		Valiosa (V) & Es la base de datos que permite evaluar compatibilidad (\hyperref[tab:HU-05]{HU-05}) y emitir alertas de riesgo (\hyperref[tab:HU-15]{HU-15}), aportando valor directo a la cruza responsable. \\
		...
		Criterio de aceptación & \hyperref[lst:ca12]{CA-12} (véase el escenario Gherkin siguiente). \\
	\end{atributos}
	...
	\begin{atributos}{HU-13}{HU-13 --- Historia de usuario asociada al RF-18.}
		Identificador & \hyperref[tab:HU-13]{HU-13} \\
		RF asociado & \hyperref[tab:RF-18]{RF-18} \\
		...
		Independiente (I) & Depende de que exista una solicitud de adopción aceptada (\hyperref[tab:RF-05]{RF-05}), pero la lógica de etapas es un componente independiente que orquesta ese proceso. \\
		...
		Pequeña (S) & Se mantiene pequeña al limitarse a la orquestación de estados, sin implementar el seguimiento post-adopción (ese es \hyperref[tab:HU-14]{HU-14}). \\
		...
		Criterio de aceptación & \hyperref[lst:ca13]{CA-13} (véase el escenario Gherkin siguiente). \\
	\end{atributos}
	...
	\begin{atributos}{HU-14}{HU-14 --- Historia de usuario asociada al RF-22.}
		Identificador & \hyperref[tab:HU-14]{HU-14} \\
		RF asociado & \hyperref[tab:RF-22]{RF-22} \\
		...
		Independiente (I) & Depende de que exista una adopción completada (\hyperref[tab:RF-18]{RF-18}), pero es una funcionalidad independiente de ella una vez alcanzado ese estado. \\
		...
		Estimable (E) & Estimable porque reutiliza el sistema de mensajería (\hyperref[tab:RF-07]{RF-07}) y un registro simple de reportes. \\
		...
		Criterio de aceptación & \hyperref[lst:ca14]{CA-14} (véase el escenario Gherkin siguiente). \\
	\end{atributos}
	...
	\begin{atributos}{HU-15}{HU-15 --- Historia de usuario asociada al RF-23.}
		Identificador & \hyperref[tab:HU-15]{HU-15} \\
		RF asociado & \hyperref[tab:RF-23]{RF-23} \\
		...
		Independiente (I) & Depende de que existan historial médico (\hyperref[tab:RF-02]{RF-02}) y antecedentes genéticos (\hyperref[tab:RF-16]{RF-16}) registrados, pero la lógica de alerta es un componente independiente que evalúa esos datos. \\
		...
		Criterio de aceptación & \hyperref[lst:ca15]{CA-15} (véase el escenario Gherkin siguiente). \\
	\end{atributos}
	...
	\begin{atributos}{HU-16}{HU-16 --- Historia de usuario asociada al RF-04.}
		Identificador & \hyperref[tab:HU-16]{HU-16} \\
		RF asociado & \hyperref[tab:RF-04]{RF-04} \\
		...
		Independiente (I) & Depende de que existan mascotas registradas y publicadas (\hyperref[tab:RF-01]{RF-01}, \hyperref[tab:RF-13]{RF-13}), pero la lógica de filtrado es independiente de cualquier otra historia. \\
		...
		Criterio de aceptación & \hyperref[lst:ca16]{CA-16} (véase el escenario Gherkin siguiente). \\
	\end{atributos}
	...
	\begin{atributos}{HU-17}{HU-17 --- Historia de usuario asociada al RF-07.}
		Identificador & \hyperref[tab:HU-17]{HU-17} \\
		RF asociado & \hyperref[tab:RF-07]{RF-07} \\
		...
		Independiente (I) & Depende de que exista una solicitud de adopción o cruza iniciada (\hyperref[tab:RF-05]{RF-05}, \hyperref[tab:RF-08]{RF-08}), pero el envío y el reporte de mensajes son funcionalidades independientes entre sí. \\
		...
		Pequeña (S) & Se limita al envío, recepción y reporte de mensajes, sin incluir la moderación automática por IA (esa es \hyperref[tab:RNF-15]{RNF-15}). \\
		...
		Criterio de aceptación & \hyperref[lst:ca17]{CA-17} (véase el escenario Gherkin siguiente). \\
	\end{atributos}
	...
	\begin{atributos}{HU-18}{HU-18 --- Historia de usuario asociada al RF-10.}
		Identificador & \hyperref[tab:HU-18]{HU-18} \\
		RF asociado & \hyperref[tab:RF-10]{RF-10} \\
		...
		Independiente (I) & Depende de que existan fechas de control registradas en el carnet de vacunación (\hyperref[tab:RF-14]{RF-14}), pero la lógica de envío de recordatorios es independiente del resto de historias. \\
		...
		Estimable (E) & Estimable porque combina una tarea programada simple con la integración ya definida de la WhatsApp Business API (\hyperref[tab:RD-08]{RD-08}). \\
		...
		Criterio de aceptación & \hyperref[lst:ca18]{CA-18} (véase el escenario Gherkin siguiente). \\
	\end{atributos}
	...
	\begin{atributos}{HU-19}{HU-19 --- Historia de usuario asociada al RF-15.}
		Identificador & \hyperref[tab:HU-19]{HU-19} \\
		RF asociado & \hyperref[tab:RF-15]{RF-15} \\
		...
		Independiente (I) & Depende de que existan solicitudes de adopción (\hyperref[tab:RF-05]{RF-05}) o cruza (\hyperref[tab:RF-08]{RF-08}) registradas, pero la consulta es una vista de solo lectura independiente de esas historias. \\
		...
		Pequeña (S) & Se limita a mostrar el historial; no incluye la lógica de avance de etapas (esa es \hyperref[tab:HU-13]{HU-13}). \\
		...
		Criterio de aceptación & \hyperref[lst:ca19]{CA-19} (véase el escenario Gherkin siguiente). \\
	\end{atributos}
	...
	\begin{atributos}{HU-20}{HU-20 --- Historia de usuario asociada al RF-17.}
		Identificador & \hyperref[tab:HU-20]{HU-20} \\
		RF asociado & \hyperref[tab:RF-17]{RF-17} \\
		...
		Independiente (I) & Depende de que exista una carga de fotografía dentro de otra historia (\hyperref[tab:RF-01]{RF-01}, \hyperref[tab:RF-13]{RF-13}), pero la validación en sí es un componente independiente que se invoca sobre cualquier imagen entrante. \\
		Negociable (N) & El umbral exacto de confianza de la clasificación (\hyperref[tab:RNF-07]{RNF-07}) es negociable con el equipo dentro del rango técnico disponible. \\
		...
		Estimable (E) & Estimable porque reutiliza el componente de validación de imágenes por IA ya definido en la arquitectura (\hyperref[tab:RD-01]{RD-01}). \\
		Pequeña (S) & Se limita a aceptar o rechazar la imagen con su motivo; no incluye la explicación detallada por contraste (esa es \hyperref[tab:RNF-14]{RNF-14}). \\
		...
		Criterio de aceptación & \hyperref[lst:ca20]{CA-20} (véase el escenario Gherkin siguiente). \\
	\end{atributos}
	...
	\begin{atributos}{HU-21}{HU-21 --- Historia de usuario asociada al RF-19.}
		Identificador & \hyperref[tab:HU-21]{HU-21} \\
		RF asociado & \hyperref[tab:RF-19]{RF-19} \\
		...
		Independiente (I) & Depende de que ambas mascotas cuenten con antecedentes genéticos registrados (\hyperref[tab:RF-16]{RF-16}), pero la validación del certificado es una acción independiente sobre un documento ya cargado. \\
		...
		Estimable (E) & Estimable porque reutiliza la misma máquina de estados de verificación de documentos que \hyperref[tab:HU-07]{HU-07}. \\
		Pequeña (S) & Se limita a la validación del certificado; no incluye la evaluación de compatibilidad en sí (esa es \hyperref[tab:HU-05]{HU-05}). \\
		...
		Criterio de aceptación & \hyperref[lst:ca21]{CA-21} (véase el escenario Gherkin siguiente). \\
	\end{atributos}
	...
	\begin{atributos}{HU-22}{HU-22 --- Historia de usuario asociada al RF-21.}
		Identificador & \hyperref[tab:HU-22]{HU-22} \\
		RF asociado & \hyperref[tab:RF-21]{RF-21} \\
		...
		Independiente (I) & Depende de que la mascota esté registrada (\hyperref[tab:RF-01]{RF-01}), pero no de ninguna otra historia. \\
		Negociable (N) & El formato exacto del número de microchip (ISO 11784/11785 u otro) es negociable con el equipo. \\
		Valiosa (V) & Complementa el carnet de vacunación con trazabilidad adicional, útil para el aviso de mascota extraviada (\hyperref[tab:RF-25]{RF-25}). \\
		Estimable (E) & Estimable porque reutiliza el mismo patrón de formulario de \hyperref[tab:HU-11]{HU-11}. \\
		...
		Criterio de aceptación & \hyperref[lst:ca22]{CA-22} (véase el escenario Gherkin siguiente). \\
	\end{atributos}
	...
	\begin{atributos}{HU-23}{HU-23 --- Historia de usuario asociada al RF-24.}
		Identificador & \hyperref[tab:HU-23]{HU-23} \\
		RF asociado & \hyperref[tab:RF-24]{RF-24} \\
		...
		Independiente (I) & Depende de que exista un carnet de vacunación con al menos una vacuna registrada (\hyperref[tab:RF-14]{RF-14}), pero agrega datos a un registro ya existente sin depender de otras historias. \\
		...
		Estimable (E) & Estimable porque agrega tres campos a un formulario ya definido en \hyperref[tab:HU-11]{HU-11}. \\
		...
		Criterio de aceptación & \hyperref[lst:ca23]{CA-23} (véase el escenario Gherkin siguiente). \\
	\end{atributos}
	...
	\begin{atributos}{HU-24}{HU-24 --- Historia de usuario asociada al RF-25.}
		Identificador & \hyperref[tab:HU-24]{HU-24} \\
		RF asociado & \hyperref[tab:RF-25]{RF-25} \\
		...
		Independiente (I) & Depende de que la mascota esté registrada (\hyperref[tab:RF-01]{RF-01}), y se apoya opcionalmente en el microchip (\hyperref[tab:RF-21]{RF-21}) si existe, pero puede publicarse sin él. \\
		...
		Criterio de aceptación & \hyperref[lst:ca24]{CA-24} (véase el escenario Gherkin siguiente). \\
	\end{atributos}
	...
	\begin{atributos}{HU-25}{HU-25 --- Historia de usuario asociada al RF-26.}
		Identificador & \hyperref[tab:HU-25]{HU-25} \\
		RF asociado & \hyperref[tab:RF-26]{RF-26} \\
		...
		Independiente (I) & Depende de que exista una primera validación médica registrada (\hyperref[tab:RF-09]{RF-09}), pero la segunda opinión es una acción independiente ejercida por una veterinaria distinta. \\
		...
		Estimable (E) & Estimable porque reutiliza la misma acción de validación ya definida en \hyperref[tab:HU-07]{HU-07}, sobre un registro que ya existe. \\
		...
		Criterio de aceptación & \hyperref[lst:ca25]{CA-25} (véase el escenario Gherkin siguiente). \\
	\end{atributos}
	...
	\begin{atributos}{HU-26}{HU-26 --- Historia de usuario asociada al RF-27.}
		Identificador & \hyperref[tab:HU-26]{HU-26} \\
		RF asociado & \hyperref[tab:RF-27]{RF-27} \\
		...
		Independiente (I) & Depende de que existan procesos de cruza completados registrados (\hyperref[tab:RF-08]{RF-08}) y consultables (\hyperref[tab:RF-15]{RF-15}), pero la lógica de conteo y alerta es un componente independiente que solo los consulta. \\
		...
		Criterio de aceptación & \hyperref[lst:ca26]{CA-26} (véase el escenario Gherkin siguiente). \\
	\end{atributos}
	...
	\begin{atributos}{HU-27}{HU-27 --- Historia de usuario asociada al RF-20.}
		Identificador & \hyperref[tab:HU-27]{HU-27} \\
		RF asociado & \hyperref[tab:RF-20]{RF-20} \\
		...
		Independiente (I) & Depende de que ambos propietarios estén registrados y verificados (\hyperref[tab:RF-12]{RF-12}), pero la coordinación del encuentro es independiente de cualquier solicitud de adopción o cruza en curso. \\
		...
		Estimable (E) & Estimable porque reutiliza el mismo patrón de coordinación y notificación de \hyperref[tab:HU-06]{HU-06}. \\
		Pequeña (S) & Se limita a programar y notificar el encuentro; no incluye lógica de evaluación de compatibilidad (esa es \hyperref[tab:HU-05]{HU-05}). \\
		...
		Criterio de aceptación & \hyperref[lst:ca27]{CA-27} (véase el escenario Gherkin siguiente). \\
	\end{atributos}
	...
	consolidada con los 61 casos de prueba (\hyperref[tab:CP-01]{CP-01} a \hyperref[cp:61]{CP-61}), uno por cada
	elemento vigente del documento.

	\begin{atributos}{CP-01}{CP-01 --- Caso de prueba del RF-01.}
		Identificador & \hyperref[tab:CP-01]{CP-01} \\
		Requisito verificado & \hyperref[tab:RF-01]{RF-01} --- Registrar mascota \\
		...
		Resultado esperado & El sistema almacena el perfil de la mascota y este queda disponible para consulta posterior (equivalente al escenario \emph{Registro exitoso con datos completos} de \hyperref[lst:ca01]{CA-01}). \\
		Caso alternativo & Al omitir la especie (campo obligatorio) y confirmar, el sistema rechaza la operación y muestra el motivo junto al campo incompleto (escenario \emph{Registro rechazado por campo obligatorio ausente} de \hyperref[lst:ca01]{CA-01}). \\
		Tipo & Funcional \\
		Prioridad & Alta (RF Must) \\
	\end{atributos}

	\begin{atributos}{CP-16}{CP-16 --- Caso de prueba del RF-16.}
		Identificador & \hyperref[tab:CP-16]{CP-16} \\
		Requisito verificado & \hyperref[tab:RF-16]{RF-16} --- Gestionar antecedentes genéticos y parentesco de la mascota \\
		...
		Precondiciones & La mascota está registrada (\hyperref[tab:RF-01]{RF-01}). \\
		...
		Resultado esperado & El sistema almacena los antecedentes genéticos de la mascota (equivalente al escenario \emph{Registro exitoso de antecedentes genéticos} de \hyperref[lst:ca12]{CA-12}). \\
		Caso alternativo & Un interesado en cruza que consulta el perfil de una mascota con antecedentes registrados visualiza la información genética autorizada (escenario \emph{Consulta de antecedentes genéticos por un interesado en cruza} de \hyperref[lst:ca12]{CA-12}). \\
		Tipo & Funcional \\
		Prioridad & Alta (RF Must) \\
	\end{atributos}

	\begin{atributos}{CP-33}{CP-33 --- Caso de prueba del RNF-06.}
		Identificador & \hyperref[tab:CP-33]{CP-33} \\
		Requisito verificado & \hyperref[tab:RNF-06]{RNF-06} --- Exactitud del componente de inteligencia artificial \\
		...
		Resultado esperado & El porcentaje de coincidencia calculado es igual o superior al 85\,\% exigido por \hyperref[tab:RNF-06]{RNF-06}. \\
	...
	\begin{atributos}{CP-58}{CP-58 --- Caso de prueba del RL-06.}
		Identificador & \hyperref[tab:CP-58]{CP-58} \\
		Requisito verificado & \hyperref[rl:06]{RL-06} --- Derecho a explicación de decisiones automatizadas (Art.\ 20 LOPDP) \\
		...
		Precondiciones & Se ejecuta al menos una de las tres funcionalidades con componente de IA: evaluación de compatibilidad (\hyperref[tab:RF-06]{RF-06}), validación de imágenes (\hyperref[tab:RF-17]{RF-17}) o moderación de mensajes (\hyperref[tab:RF-07]{RF-07}). \\
		Pasos y datos de entrada & 1. Ejecutar cada una de las tres funcionalidades con componente de IA sobre un caso de prueba representativo. 2. Verificar que el resultado entregado al usuario incluye la explicación exigida por \hyperref[tab:RNF-13]{RNF-13}, \hyperref[tab:RNF-14]{RNF-14} o \hyperref[tab:RNF-15]{RNF-15} según corresponda. \\
	...
	\begin{landscape}
		\begin{longtable}{@{}p{1.3cm} p{1.3cm} p{4.6cm} p{3.6cm} p{5.2cm} p{1.6cm}@{}}
			...
			\hyperref[tab:CP-01]{CP-01} & \hyperref[tab:RF-01]{RF-01} & Comprobar que un propietario puede registrar una mascota con datos validos y que el perfil queda disponible para consulta. & Propietario autenticado en la plataforma. & El sistema almacena el perfil con los datos ingresados; el perfil aparece en consultas posteriores (ejecuta \hyperref[lst:ca01]{CA-01}). & Funcional \\ \midrule
			\hyperref[cp:02]{CP-02}\label{cp:02} & \hyperref[tab:RF-02]{RF-02} & Comprobar que un antecedente medico nuevo se registra y aparece en el historial de la mascota. & Mascota registrada (\hyperref[tab:RF-01]{RF-01}). & El antecedente queda almacenado y visible en el historial medico (ejecuta \hyperref[lst:ca02]{CA-02}). & Funcional \\ \midrule
			\hyperref[cp:03]{CP-03}\label{cp:03} & \hyperref[tab:RF-03]{RF-03} & Comprobar que un usuario autorizado consulta el perfil completo de una mascota y que los datos restringidos no se exponen a quien no tiene permiso. & Mascota con perfil publico y con al menos un dato marcado como privado. & Se muestra la informacion autorizada; el dato restringido no aparece para el usuario sin permiso (ejecuta \hyperref[lst:ca03]{CA-03}). & Funcional \\ \midrule
			\hyperref[cp:04]{CP-04}\label{cp:04} & \hyperref[tab:RF-04]{RF-04} & Comprobar que el listado de mascotas se filtra correctamente al combinar dos o mas criterios de busqueda. & Existen mascotas publicadas con atributos distintos (especie, edad, ubicacion). & Solo se muestran las mascotas que cumplen simultaneamente todos los filtros aplicados (ejecuta \hyperref[lst:ca16]{CA-16}). & Funcional \\ \midrule
			\hyperref[cp:05]{CP-05}\label{cp:05} & \hyperref[tab:RF-05]{RF-05} & Comprobar el ciclo completo de una solicitud de adopcion: envio, aceptacion y rechazo. & Adoptante verificado; mascota disponible para adopcion. & La solicitud se registra en estado pendiente y transita correctamente a aceptada o rechazada segun la decision del propietario (ejecuta \hyperref[lst:ca04]{CA-04}). & Funcional \\ \midrule
			\hyperref[cp:06]{CP-06}\label{cp:06} & \hyperref[tab:RF-06]{RF-06} & Comprobar que la evaluacion de compatibilidad para cruza se genera cuando ambas mascotas cuentan con historial medico y antecedentes geneticos, y que se bloquea cuando falta informacion. & Dos mascotas registradas para evaluar. & Con informacion completa se muestra el resultado comparativo y la advertencia orientativa; con informacion incompleta el sistema indica que falta informacion y no genera resultado (ejecuta \hyperref[lst:ca05]{CA-05}). & Funcional \\ \midrule
			\hyperref[cp:07]{CP-07}\label{cp:07} & \hyperref[tab:RF-07]{RF-07} & Comprobar el envio de un mensaje entre dos usuarios y el reporte de una conversacion sospechosa. & Ambos usuarios registrados; existe una solicitud en curso. & El mensaje se almacena y es visible para el destinatario; el reporte manual queda asociado a la conversacion con su motivo (ejecuta \hyperref[lst:ca17]{CA-17}). & Funcional \\ \midrule
			\hyperref[cp:08]{CP-08}\label{cp:08} & \hyperref[tab:RF-08]{RF-08} & Comprobar el ciclo completo de una solicitud de cruza: envio y aceptacion por el propietario receptor. & Ambas mascotas registradas y publicadas para cruza. & La solicitud se registra en estado pendiente y transita a aceptada, notificando al solicitante (ejecuta \hyperref[lst:ca06]{CA-06}). & Funcional \\ \midrule
			\hyperref[cp:09]{CP-09}\label{cp:09} & \hyperref[tab:RF-09]{RF-09} & Comprobar que una veterinaria autorizada puede validar un historial medico y que un usuario sin ese rol no puede hacerlo. & Historial medico cargado para una mascota; existen usuarios con y sin rol de veterinaria. & El historial queda marcado como validado con la veterinaria verificadora; el usuario sin permisos recibe un rechazo explicito (ejecuta \hyperref[lst:ca07]{CA-07}). & Funcional \\ \midrule
			\hyperref[cp:10]{CP-10}\label{cp:10} & \hyperref[tab:RF-10]{RF-10} & Comprobar que el recordatorio de un control preventivo se envia por WhatsApp y, en su ausencia, por el canal interno de respaldo. & Mascota con fecha de control registrada dentro de los proximos siete dias. & El recordatorio se entrega por WhatsApp; si el proveedor no responde, se entrega por notificacion in-app (ejecuta \hyperref[lst:ca18]{CA-18}). & Funcional \\ \midrule
			\hyperref[cp:11]{CP-11}\label{cp:11} & \hyperref[tab:RF-11]{RF-11} & Comprobar que un propietario puede restringir un dato sensible y que dicho dato deja de ser visible para usuarios no autorizados. & Propietario autenticado con mascota registrada. & El dato marcado como privado deja de mostrarse a usuarios sin permiso sobre el (ejecuta \hyperref[lst:ca08]{CA-08}). & Funcional \\ \midrule
			\hyperref[cp:12]{CP-12}\label{cp:12} & \hyperref[tab:RF-12]{RF-12} & Comprobar la verificacion de identidad de un usuario nuevo con documentacion valida e invalida. & Usuario que completo el formulario de registro. & Con documentacion valida la cuenta queda marcada como verificada; con documentacion invalida o incompleta la verificacion se rechaza indicando el motivo (ejecuta \hyperref[lst:ca09]{CA-09}). & Funcional \\ \midrule
			\hyperref[cp:13]{CP-13}\label{cp:13} & \hyperref[tab:RF-13]{RF-13} & Comprobar que una mascota registrada puede publicarse como disponible para adopcion o cruza y aparecer en el listado correspondiente. & Mascota registrada (\hyperref[tab:RF-01]{RF-01}). & La mascota queda publicada y visible en el listado; una mascota no registrada no puede publicarse (ejecuta \hyperref[lst:ca10]{CA-10}). & Funcional \\ \midrule
			\hyperref[cp:14]{CP-14}\label{cp:14} & \hyperref[tab:RF-14]{RF-14} & Comprobar el registro de una vacuna en el carnet y la consulta del estado de vigencia. & Mascota registrada. & La vacuna se almacena en el carnet; una vacuna con proxima dosis vencida se muestra con estado vencido (ejecuta \hyperref[lst:ca11]{CA-11}). & Funcional \\ \midrule
			\hyperref[cp:15]{CP-15}\label{cp:15} & \hyperref[tab:RF-15]{RF-15} & Comprobar que el historial combinado de procesos de adopcion y cruza de una mascota se muestra ordenado por fecha. & Mascota con al menos una solicitud de adopcion y una de cruza registradas. & Ambos tipos de proceso aparecen en una sola linea de tiempo con su estado vigente (ejecuta \hyperref[lst:ca19]{CA-19}). & Funcional \\ \midrule
			\hyperref[tab:CP-16]{CP-16} & \hyperref[tab:RF-16]{RF-16} & Comprobar el registro y la consulta de antecedentes geneticos y parentesco de una mascota. & Mascota registrada. & Los antecedentes quedan almacenados y disponibles para consulta por un interesado en cruza (ejecuta \hyperref[lst:ca12]{CA-12}). & Funcional \\ \midrule
			\hyperref[cp:17]{CP-17}\label{cp:17} & \hyperref[tab:RF-17]{RF-17} & Comprobar que una imagen valida es aceptada y una imagen invalida o inapropiada es rechazada con motivo especifico. & Usuario autenticado cargando una fotografia. & La imagen valida queda almacenada; la imagen invalida se rechaza mostrando el motivo (ejecuta \hyperref[lst:ca20]{CA-20}). & Funcional \\ \midrule
			\hyperref[cp:18]{CP-18}\label{cp:18} & \hyperref[tab:RF-18]{RF-18} & Comprobar el avance de una solicitud de adopcion por las etapas definidas y su bloqueo ante un resultado desfavorable. & Solicitud de adopcion aceptada, en la etapa de entrevista. & La solicitud avanza de etapa en etapa hasta completada; un resultado desfavorable la marca como rechazada sin permitir avanzar (ejecuta \hyperref[lst:ca13]{CA-13}). & Funcional \\ \midrule
			\hyperref[cp:19]{CP-19}\label{cp:19} & \hyperref[tab:RF-19]{RF-19} & Comprobar que la opcion de cruza permanece bloqueada hasta que ambos certificados veterinarios son validados. & Ambas mascotas con antecedentes geneticos registrados (\hyperref[tab:RF-16]{RF-16}) y certificados adjuntados. & La opcion de cruza se habilita solo cuando ambos certificados estan validados; permanece bloqueada mientras alguno este pendiente (ejecuta \hyperref[lst:ca21]{CA-21}). & Funcional \\ \midrule
			\hyperref[cp:20]{CP-20}\label{cp:20} & \hyperref[tab:RF-20]{RF-20} & Comprobar la coordinacion de un encuentro de socializacion entre dos mascotas y sus propietarios verificados. & Ambos propietarios registrados y verificados (\hyperref[tab:RF-12]{RF-12}). & El encuentro queda registrado y notificado a ambos participantes, y aparece en el historial de interacciones de ambas mascotas (ejecuta \hyperref[lst:ca27]{CA-27}). & Funcional \\ \midrule
			\hyperref[cp:21]{CP-21}\label{cp:21} & \hyperref[tab:RF-21]{RF-21} & Comprobar el registro y la consulta del numero de microchip de una mascota. & Mascota registrada. & El microchip queda asociado al perfil y es consultable por usuarios autorizados (ejecuta \hyperref[lst:ca22]{CA-22}). & Funcional \\ \midrule
			\hyperref[cp:22]{CP-22}\label{cp:22} & \hyperref[tab:RF-22]{RF-22} & Comprobar que el seguimiento post-adopcion solo puede registrarse sobre una adopcion completada. & Existe una adopcion completada y otra aun no completada. & Se permite registrar el reporte de seguimiento sobre la adopcion completada; se rechaza sobre la no completada (ejecuta \hyperref[lst:ca14]{CA-14}). & Funcional \\ \midrule
			\hyperref[cp:23]{CP-23}\label{cp:23} & \hyperref[tab:RF-23]{RF-23} & Comprobar que se muestra una alerta de riesgo ante una interaccion con enfermedad contagiosa activa, y que no se muestra alerta cuando no hay riesgo. & Dos mascotas, una con enfermedad contagiosa activa registrada y otra sin condiciones de riesgo. & El sistema muestra la alerta en el primer caso y permite continuar sin alerta en el segundo (ejecuta \hyperref[lst:ca15]{CA-15}). & Funcional \\ \midrule
			\hyperref[cp:24]{CP-24}\label{cp:24} & \hyperref[tab:RF-24]{RF-24} & Comprobar el registro y la consulta de la cepa, el lote y el aplicador de una vacuna. & Carnet de vacunacion con al menos una vacuna registrada (\hyperref[tab:RF-14]{RF-14}). & Los tres datos de trazabilidad quedan almacenados y visibles junto a la fecha de aplicacion (ejecuta \hyperref[lst:ca23]{CA-23}). & Funcional \\ \midrule
			\hyperref[cp:25]{CP-25}\label{cp:25} & \hyperref[tab:RF-25]{RF-25} & Comprobar la publicacion y el cierre de un aviso de mascota extraviada. & Mascota registrada. & El aviso queda visible publicamente hasta que el propietario lo marca como resuelto, momento en el que deja de listarse (ejecuta \hyperref[lst:ca24]{CA-24}). & Funcional \\ \midrule
			\hyperref[cp:26]{CP-26}\label{cp:26} & \hyperref[tab:RF-26]{RF-26} & Comprobar que una segunda veterinaria puede confirmar o cuestionar una validacion medica previa, dejando registro de ambas. & Historial medico con una validacion previa registrada (\hyperref[tab:RF-09]{RF-09}). & Ambas validaciones (concordante o en desacuerdo) quedan registradas y visibles en el historial (ejecuta \hyperref[lst:ca25]{CA-25}). & Funcional \\ \midrule
			\hyperref[cp:27]{CP-27}\label{cp:27} & \hyperref[tab:RF-27]{RF-27} & Comprobar que se muestra una alerta al intentar una tercera solicitud de cruza de la misma mascota dentro de seis meses. & Mascota con dos procesos de cruza completados en los ultimos seis meses. & El sistema muestra la alerta de uso repetitivo antes de confirmar la tercera solicitud; con un unico proceso previo no se muestra alerta (ejecuta \hyperref[lst:ca26]{CA-26}). & Funcional \\ \midrule
			\hyperref[cp:28]{CP-28}\label{cp:28} & \hyperref[tab:RNF-01]{RNF-01} & Comprobar que la informacion de propietarios y mascotas esta protegida frente a accesos no autorizados (cifrado en transito y en reposo). & Cuenta de usuario con datos personales registrados. & Un intento de acceso sin autenticacion valida o sin autorizacion sobre el recurso es rechazado y los datos permanecen cifrados en la base de datos. & No funcional \\ \midrule
			\hyperref[cp:29]{CP-29}\label{cp:29} & \hyperref[tab:RNF-02]{RNF-02} & Medir la disponibilidad del sistema durante una ventana de monitoreo continua y verificar que se mantiene sobre el umbral definido. & Sistema desplegado en el entorno de pruebas de disponibilidad. & La disponibilidad medida iguala o supera el porcentaje objetivo definido para el sistema en produccion. & No funcional \\ \midrule
			\hyperref[cp:30]{CP-30}\label{cp:30} & \hyperref[tab:RNF-03]{RNF-03} & Medir el tiempo de respuesta de las operaciones criticas bajo carga normal y comparar contra el umbral establecido. & Entorno de pruebas con datos representativos cargados. & El tiempo de respuesta medido no supera el umbral maximo definido para las operaciones evaluadas. & No funcional \\ \midrule
			\hyperref[cp:31]{CP-31}\label{cp:31} & \hyperref[tab:RNF-04]{RNF-04} & Evaluar la facilidad de uso de la plataforma mediante una prueba de usabilidad con tareas representativas. & Participantes representativos de los perfiles de usuario del sistema. & Los participantes completan las tareas representativas dentro del tiempo y con la tasa de error definidos como aceptables. & No funcional \\ \midrule
			\hyperref[cp:32]{CP-32}\label{cp:32} & \hyperref[tab:RNF-05]{RNF-05} & Comprobar que el historial medico no puede ser modificado por un usuario sin autorizacion y que las modificaciones autorizadas quedan auditadas. & Historial medico existente para una mascota. & Un intento de modificacion no autorizada es rechazado; una modificacion autorizada queda registrada con su autor y fecha. & No funcional \\ \midrule
			\hyperref[tab:CP-33]{CP-33} & \hyperref[tab:RNF-06]{RNF-06} & Medir la exactitud del componente de evaluacion de compatibilidad contra el criterio experto de un conjunto de casos de referencia. & Conjunto de casos de referencia evaluados previamente por veterinarios. & La coincidencia entre el resultado del componente y el criterio experto alcanza o supera el 85\,\% definido en \hyperref[tab:RNF-06]{RNF-06}. & No funcional \\ \midrule
			\hyperref[cp:34]{CP-34}\label{cp:34} & \hyperref[tab:RNF-07]{RNF-07} & Medir la tasa de clasificacion correcta del componente de validacion de imagenes sobre un conjunto de imagenes de prueba etiquetadas. & Conjunto de imagenes de prueba etiquetadas como validas e invalidas. & La tasa de clasificacion correcta alcanza o supera el 95\,\% definido en \hyperref[tab:RNF-07]{RNF-07}. & No funcional \\ \midrule
			\hyperref[cp:35]{CP-35}\label{cp:35} & \hyperref[tab:RNF-08]{RNF-08} & Medir el tiempo transcurrido entre una actualizacion de datos y su reflejo en las consultas del sistema. & Mascota, historial medico o publicacion con datos actualizables. & La actualizacion se refleja en las consultas dentro del tiempo maximo de cinco segundos definido en \hyperref[tab:RNF-08]{RNF-08}. & No funcional \\ \midrule
			\hyperref[cp:36]{CP-36}\label{cp:36} & \hyperref[tab:RNF-09]{RNF-09} & Comprobar el envio de una notificacion por WhatsApp Business API y su entrega sin duplicacion ni perdida. & Recordatorio programado para su envio (\hyperref[tab:RF-10]{RF-10}). & La notificacion se entrega exactamente una vez por el canal correspondiente, sin duplicarse ni perderse. & No funcional \\ \midrule
			\hyperref[cp:37]{CP-37}\label{cp:37} & \hyperref[tab:RNF-10]{RNF-10} & Modificar una funcionalidad dentro de un modulo y verificar que el cambio no requiere modificar mas del 10\,\% de los archivos de otro modulo. & Codigo fuente organizado en los modulos definidos por \hyperref[tab:RD-05]{RD-05}. & El cambio realizado se contiene dentro del modulo afectado, sin exceder el 10\,\% de archivos modificados en otro modulo. & No funcional \\ \midrule
			\hyperref[cp:38]{CP-38}\label{cp:38} & \hyperref[tab:RNF-11]{RNF-11} & Ejecutar el mismo flujo de usuario en Chrome, Firefox y Safari (dos ultimas versiones estables) y en un dispositivo movil. & Entorno de pruebas con los navegadores y dispositivos definidos disponibles. & El flujo se comporta de forma equivalente en todos los navegadores y se adapta de forma responsiva al dispositivo movil. & No funcional \\ \midrule
			\hyperref[cp:39]{CP-39}\label{cp:39} & \hyperref[tab:RNF-12]{RNF-12} & Simular un incremento de usuarios concurrentes y verificar que el tiempo de respuesta no se degrada mas alla del umbral tolerado. & Entorno de pruebas de carga configurado. & El sistema escala horizontalmente y el tiempo de respuesta se mantiene dentro del rango tolerado bajo el incremento simulado. & No funcional \\ \midrule
			\hyperref[cp:40]{CP-40}\label{cp:40} & \hyperref[tab:RNF-13]{RNF-13} & Comprobar que la explicacion causal de una evaluacion de compatibilidad se genera dentro de 60 palabras y en no mas de dos segundos. & Evaluacion de compatibilidad ejecutada (\hyperref[tab:RF-06]{RF-06}). & La explicacion generada no supera las 60 palabras y se produce en un tiempo igual o menor a dos segundos. & No funcional \\ \midrule
			\hyperref[cp:41]{CP-41}\label{cp:41} & \hyperref[tab:RNF-14]{RNF-14} & Comprobar que el motivo especifico de rechazo de una imagen se muestra en no mas de 30 palabras. & Imagen rechazada por el componente de validacion (\hyperref[tab:RF-17]{RF-17}). & El motivo de rechazo mostrado no supera las 30 palabras y corresponde a una de las categorias definidas. & No funcional \\ \midrule
			\hyperref[cp:42]{CP-42}\label{cp:42} & \hyperref[tab:RNF-15]{RNF-15} & Comprobar que el remitente de un mensaje reportado recibe la categoria del motivo de moderacion mediante una explicacion por ejemplos. & Mensaje reportado conforme al mecanismo de \hyperref[tab:RF-07]{RF-07}. & El remitente recibe la categoria del motivo (lenguaje ofensivo, spam o acuerdo externo indebido) con un ejemplo ilustrativo. & No funcional \\ \midrule
			\hyperref[cp:43]{CP-43}\label{cp:43} & \hyperref[tab:RNF-16]{RNF-16} & Comprobar que la ubicacion exacta y los datos de contacto directo permanecen ocultos por defecto para usuarios no autorizados. & Perfil de propietario con ubicacion y contacto registrados. & Un usuario no autorizado solo observa una zona aproximada (radio igual o mayor a 1 km) y no el dato exacto. & No funcional \\ \midrule
			\hyperref[cp:44]{CP-44}\label{cp:44} & \hyperref[tab:RD-01]{RD-01} & Comprobar que los tres componentes de inteligencia artificial se integran mediante API interna REST/JSON expuesta por un modulo separado. & Arquitectura desplegada en el entorno de pruebas. & Cada componente de IA responde a traves de su API interna documentada, sin acoplarse directamente a la logica de negocio del monolito. & No funcional \\ \midrule
			\hyperref[cp:45]{CP-45}\label{cp:45} & \hyperref[tab:RD-02]{RD-02} & Comprobar que cada rol (propietario, adoptante, interesado en cruza, refugio, veterinaria, administrador) solo accede a las funciones autorizadas para su rol. & Usuarios de prueba creados para cada rol definido. & Un usuario que intenta ejecutar una accion fuera de su rol recibe un rechazo de autorizacion; las acciones permitidas se ejecutan con normalidad. & No funcional \\ \midrule
			\hyperref[cp:46]{CP-46}\label{cp:46} & \hyperref[tab:RD-03]{RD-03} & Comprobar que la informacion medica cargada por un propietario queda en estado PendienteDeValidacion hasta la revision de una veterinaria. & Historial medico o documento cargado por un propietario. & El registro permanece en PendienteDeValidacion y no se presenta como validado hasta que una veterinaria autorizada lo confirme. & No funcional \\ \midrule
			\hyperref[cp:47]{CP-47}\label{cp:47} & \hyperref[tab:RD-04]{RD-04} & Comprobar que los datos personales del propietario se almacenan cifrados y con acceso restringido segun el rol. & Datos personales registrados en el sistema. & Los datos personales no son legibles directamente en la base de datos sin el mecanismo de descifrado autorizado, y solo son accesibles segun el rol. & No funcional \\ \midrule
			\hyperref[cp:48]{CP-48}\label{cp:48} & \hyperref[tab:RD-05]{RD-05} & Comprobar que los modulos definidos (gestion de mascotas, adopcion/cruza, mensajeria, componente de IA) pueden desplegarse y probarse de forma independiente. & Arquitectura modular desplegada en el entorno de pruebas. & Cada modulo opera y se prueba de forma aislada sin requerir que los demas modulos esten activos para sus pruebas unitarias. & No funcional \\ \midrule
			\hyperref[cp:49]{CP-49}\label{cp:49} & \hyperref[tab:RD-06]{RD-06} & Comprobar que un propietario puede solicitar la eliminacion de su cuenta y de sus datos personales, y que la solicitud se ejecuta conforme al derecho de supresion. & Cuenta de propietario activa con datos personales registrados. & La cuenta y los datos personales asociados quedan eliminados o anonimizados conforme al plazo y alcance definidos por \hyperref[rl:04]{RL-04}. & No funcional \\ \midrule
			\hyperref[cp:50]{CP-50}\label{cp:50} & \hyperref[tab:RD-07]{RD-07} & Simular una vulneracion de seguridad y comprobar que la notificacion a la Autoridad de Proteccion de Datos y al titular se genera dentro de los plazos legales. & Escenario de prueba de vulneracion de seguridad simulada. & La notificacion a la autoridad se genera dentro de 5 dias y al titular afectado dentro de 3 dias, conforme a \hyperref[rl:09]{RL-09}. & No funcional \\ \midrule
			\hyperref[cp:51]{CP-51}\label{cp:51} & \hyperref[tab:RD-08]{RD-08} & Comprobar que, ante la caida simulada del proveedor externo de mensajeria, el sistema conmuta al canal de respaldo sin perder el recordatorio. & Recordatorio programado con el proveedor externo simulado como no disponible. & El recordatorio se entrega por el canal de respaldo in-app sin perdida, conforme a \hyperref[tab:RNF-09]{RNF-09}. & No funcional \\ \midrule
			\hyperref[cp:52]{CP-52}\label{cp:52} & \hyperref[tab:RD-09]{RD-09} & Comprobar que las transcripciones, videos y respuestas de cuestionario incluidas en el paquete de datos abierto no contienen identificadores directos del propietario. & Conjunto de datos preparado para el deposito en Zenodo/OSF. & Ningun archivo del paquete de datos abierto contiene nombre, cedula, rostro o dato de contacto directo del propietario sin seudonimizar. & No funcional \\ \midrule
			\hyperref[cp:53]{CP-53}\label{cp:53} & \hyperref[rl:01]{RL-01} & Comprobar que el registro de un propietario exige un consentimiento libre, especifico, informado e inequivoco, revocable en cualquier momento. & Usuario en el formulario de registro (\hyperref[tab:RF-01]{RF-01}, \hyperref[tab:RF-12]{RF-12}). & El registro no se completa sin la aceptacion explicita del consentimiento; el usuario puede revocarlo posteriormente desde su configuracion. & No funcional \\ \midrule
			\hyperref[cp:54]{CP-54}\label{cp:54} & \hyperref[rl:02]{RL-02} & Comprobar que el titular puede acceder gratuitamente a sus datos personales dentro de un plazo de 15 dias desde la solicitud. & Titular con una solicitud de acceso a sus datos registrada. & El sistema entrega la informacion solicitada dentro del plazo de 15 dias, sin costo para el titular. & No funcional \\ \midrule
			\hyperref[cp:55]{CP-55}\label{cp:55} & \hyperref[rl:03]{RL-03} & Comprobar que el titular puede rectificar datos medicos inexactos o incompletos dentro de un plazo de 15 dias. & Historial medico con un dato inexacto identificado por el titular. & La rectificacion solicitada se aplica y queda reflejada en el historial dentro del plazo de 15 dias. & No funcional \\ \midrule
			\hyperref[cp:56]{CP-56}\label{cp:56} & \hyperref[rl:04]{RL-04} & Comprobar que el titular puede solicitar la eliminacion de sus datos personales cuando revoca el consentimiento. & Titular con consentimiento previamente otorgado. & La solicitud de eliminacion se ejecuta conforme al mecanismo de \hyperref[tab:RD-06]{RD-06}, sin dejar datos personales identificables remanentes. & No funcional \\ \midrule
			\hyperref[cp:57]{CP-57}\label{cp:57} & \hyperref[rl:05]{RL-05} & Comprobar que solo se recolectan y muestran los datos estrictamente necesarios para cada finalidad, y que estan protegidos frente a accesos no autorizados. & Perfil de usuario y de mascota con datos sensibles registrados. & Ningun campo no necesario para la finalidad declarada se solicita o se expone; los datos sensibles quedan protegidos conforme a \hyperref[tab:RNF-01]{RNF-01} y \hyperref[tab:RNF-16]{RNF-16}. & No funcional \\ \midrule
			\hyperref[tab:CP-58]{CP-58} & \hyperref[rl:06]{RL-06} & Comprobar que el titular recibe una explicacion motivada ante una decision basada unicamente en una valoracion automatizada. & Evaluacion de compatibilidad, validacion de imagenes o moderacion de mensajes ejecutada por un componente de IA. & El sistema entrega la explicacion causal, por contraste o por ejemplos correspondiente (\hyperref[tab:RNF-13]{RNF-13}, \hyperref[tab:RNF-14]{RNF-14}, \hyperref[tab:RNF-15]{RNF-15}), y no presenta el resultado como definitivo sin ella. & No funcional \\ \midrule
			\hyperref[cp:59]{CP-59}\label{cp:59} & \hyperref[rl:07]{RL-07} & Comprobar que los datos sensibles y de salud del propietario requieren consentimiento explicito y quedan con confidencialidad reforzada. & Datos de salud del propietario y de la mascota registrados. & El acceso a estos datos exige el consentimiento explicito previamente otorgado y queda restringido conforme a \hyperref[tab:RD-04]{RD-04} y \hyperref[tab:RNF-05]{RNF-05}. & No funcional \\ \midrule
			\hyperref[cp:60]{CP-60}\label{cp:60} & \hyperref[rl:08]{RL-08} & Comprobar que el sistema aplica proteccion de datos desde el diseno y por defecto, mostrando unicamente lo estrictamente necesario segun el rol. & Usuario autenticado con un rol definido (\hyperref[tab:RD-02]{RD-02}). & La configuracion por defecto del sistema no expone datos mas alla de lo necesario para el rol, sin requerir configuracion manual adicional. & No funcional \\ \midrule
			\hyperref[cp:61]{CP-61}\label{cp:61} & \hyperref[rl:09]{RL-09} & Comprobar que, ante una vulneracion de seguridad, la notificacion a la autoridad y al titular se ejecuta dentro de los plazos de 5 y 3 dias respectivamente. & Escenario de vulneracion de seguridad simulada (mismo escenario de \hyperref[cp:50]{CP-50}). & La notificacion se genera y se registra dentro de los plazos maximos de 5 y 3 dias definidos por el Art. 43 y 46 de la LOPDP. & No funcional \\ \midrule
	...
	\hyperref[tab:RF-26]{RF-26} y \hyperref[tab:RF-27]{RF-27} se apoyan en los actores, clases, pasos de control y
	estados ya representados en las figuras de esta sección: ambos
	...
	(\hyperref[tab:RF-01]{RF-01}), \emph{Post pet for breeding} (\hyperref[tab:RF-08]{RF-08}, \hyperref[tab:RF-13]{RF-13}), \emph{Register
	medical history} (\hyperref[tab:RF-02]{RF-02}) y \emph{Configure medical privacy} (\hyperref[tab:RF-11]{RF-11}), lo
	...
	certificados (\hyperref[tab:RF-14]{RF-14}) es un comportamiento opcional que amplía el registro
	...
	(\hyperref[tab:RF-04]{RF-04}) o \emph{Evaluate breeding compatibility} (\hyperref[tab:RF-06]{RF-06}) antes de decidir,
	y ambos participan en \emph{Manage communication between users} (\hyperref[tab:RF-07]{RF-07})
	...
	(\hyperref[tab:RF-10]{RF-10}). El punto central del diagrama son las dos relaciones de
	...
	coherencia con \hyperref[tab:RF-03]{RF-03} y con el requisito legal de decisión informada
	...
	\emph{Validate medical information} (\hyperref[tab:RF-09]{RF-09}), caso de uso que no se origina
	...
	restricción de diseño \hyperref[tab:RD-03]{RD-03} y el requisito no funcional de integridad de
	la información médica (\hyperref[tab:RNF-05]{RNF-05}).

	Los dos requisitos funcionales incorporados en la revisión 4.0 del ERS,
	\hyperref[tab:RF-26]{RF-26} (segunda opinión veterinaria) y \hyperref[tab:RF-27]{RF-27} (control de solicitudes
	...
	\begin{atributos}{CU-01}{CU-01 --- Registrar mascota.}
		Identificador & \hyperref[tab:CU-01]{CU-01} \\
		Nombre & Registrar mascota \\
		Actor principal & Propietario de mascota \\
		Actores secundarios & Refugio de animales; componente de validación de imágenes \\
		RF asociados & \hyperref[tab:RF-01]{RF-01}, \hyperref[tab:RF-17]{RF-17}, \hyperref[tab:RF-21]{RF-21} \\
		Precondiciones & El propietario está autenticado y su identidad ha sido verificada (\hyperref[tab:RF-12]{RF-12}). \\
		Flujo principal &
		\begin{enumerate}[leftmargin=*, nosep]
			...
			\item El sistema envía la fotografía al componente de validación de imágenes (\hyperref[tab:RF-17]{RF-17}).
		...
		Flujos alternativos &
		\begin{enumerate}[leftmargin=*, nosep]
			\item[3.1] El propietario registra el identificador de microchip (\hyperref[tab:RF-21]{RF-21}); el sistema lo asocia al perfil.
		...
		\begin{enumerate}[leftmargin=*, nosep]
			...
			\item[7.1] La imagen es rechazada por el componente de IA: el sistema notifica el motivo del rechazo (\hyperref[tab:RNF-14]{RNF-14}) y solicita una nueva fotografía.
		\end{enumerate} \\
	...
	\begin{atributos}{CU-02}{CU-02 --- Registrar historial médico y antecedentes genéticos.}
		Identificador & \hyperref[tab:CU-02]{CU-02} \\
		Nombre & Registrar historial médico y antecedentes genéticos \\
		Actor principal & Propietario de mascota \\
		Actores secundarios & Veterinaria \\
		RF asociados & \hyperref[tab:RF-02]{RF-02}, \hyperref[tab:RF-16]{RF-16}, \hyperref[tab:RF-24]{RF-24} \\
		Precondiciones & La mascota está registrada en el sistema (\hyperref[tab:CU-01]{CU-01}) y el usuario tiene permisos de escritura sobre su historial. \\
		Flujo principal &
		\begin{enumerate}[leftmargin=*, nosep]
			...
			\item El usuario ingresa la trazabilidad de cepa, lote y profesional aplicador de cada vacuna (\hyperref[tab:RF-24]{RF-24}).
			\item El usuario registra los antecedentes genéticos y de parentesco de la mascota (\hyperref[tab:RF-16]{RF-16}).
		...
		Flujos alternativos &
		\begin{enumerate}[leftmargin=*, nosep]
			\item[7.1] Si quien registra es una \textbf{veterinaria} autorizada, el sistema marca el registro como validado profesionalmente (\hyperref[tab:RD-03]{RD-03}) e identifica al profesional responsable.
			\item[7.2] Si quien registra es el \textbf{propietario}, el registro queda en estado \emph{PendienteDeValidación} hasta que una veterinaria ejecute \hyperref[tab:CU-09]{CU-09}.
		\end{enumerate} \\
		Flujos de excepción &
		\begin{enumerate}[leftmargin=*, nosep]
		...
		\end{enumerate} \\
		Postcondiciones & El historial médico y los antecedentes genéticos quedan actualizados y disponibles para los usuarios autorizados según la configuración de privacidad (\hyperref[tab:RF-11]{RF-11}). \\
	\end{atributos}
	
	\begin{atributos}{CU-03}{CU-03 --- Cargar documento veterinario y carnet de vacunación.}
		Identificador & \hyperref[tab:CU-03]{CU-03} \\
		Nombre & Cargar documento veterinario y carnet de vacunación \\
		Actor principal & Propietario de mascota \\
		Actores secundarios & Veterinaria (validación posterior) \\
		RF asociados & \hyperref[tab:RF-14]{RF-14}, \hyperref[tab:RF-24]{RF-24} \\
		...
		\end{enumerate} \\
		Postcondiciones & El documento queda almacenado y visible para el propietario, en estado \emph{PendienteDeValidación} hasta la ejecución de \hyperref[tab:CU-09]{CU-09} por parte de una veterinaria autorizada. \\
	\end{atributos}
	
	\begin{atributos}{CU-04}{CU-04 --- Publicar mascota en adopción.}
		Identificador & \hyperref[tab:CU-04]{CU-04} \\
		Nombre & Publicar mascota en adopción \\
		Actor principal & Propietario de mascota \\
		Actores secundarios & Refugio de animales \\
		RF asociados & \hyperref[tab:RF-13]{RF-13}, \hyperref[tab:RF-05]{RF-05} \\
		Precondiciones & La mascota está registrada (\hyperref[tab:CU-01]{CU-01}) y el propietario es su titular en el sistema. \\
		Flujo principal &
		\begin{enumerate}[leftmargin=*, nosep]
		...
		\end{enumerate} \\
		Postcondiciones & La mascota transita al estado \emph{Publicada} y queda visible en las búsquedas de adopción (\hyperref[tab:CU-11]{CU-11}). \\
	\end{atributos}
	
	\begin{atributos}{CU-05}{CU-05 --- Publicar mascota para cruza y gestionar solicitud.}
		Identificador & \hyperref[tab:CU-05]{CU-05} \\
		Nombre & Publicar mascota para cruza y gestionar solicitud \\
		Actor principal & Propietario de mascota \\
		Actores secundarios & Interesado en cruza \\
		RF asociados & \hyperref[tab:RF-08]{RF-08}, \hyperref[tab:RF-13]{RF-13}, \hyperref[tab:RF-16]{RF-16}, \hyperref[tab:RF-19]{RF-19}, \hyperref[tab:RF-27]{RF-27} \\
		Precondiciones & La mascota está registrada y cuenta con antecedentes genéticos registrados (\hyperref[tab:RF-16]{RF-16}). \\
		Flujo principal &
		\begin{enumerate}[leftmargin=*, nosep]
			...
			\item El sistema verifica la vigencia del certificado veterinario asociado (\hyperref[tab:RF-19]{RF-19}).
			...
			\item El sistema verifica la cantidad de procesos de cruza completados por la mascota en los últimos seis meses (\hyperref[tab:RF-27]{RF-27}).
		...
		Flujos alternativos &
		\begin{enumerate}[leftmargin=*, nosep]
			\item[2.1] No existen antecedentes genéticos: el sistema redirige al propietario al formulario de \hyperref[tab:RF-16]{RF-16} antes de permitir continuar.
			\item[6.1] La mascota supera los dos procesos de cruza completados en seis meses: el sistema muestra la alerta de uso repetitivo (\hyperref[tab:RF-27]{RF-27}) al propietario y al interesado antes de continuar, sin bloquear de forma definitiva la solicitud.
			\item[8.1] El propietario acepta: el sistema habilita el canal de comunicación (\hyperref[tab:CU-08]{CU-08}) y la mascota transita a \emph{EnProcesoDeCruza}.
		\end{enumerate} \\
		Flujos de excepción &
		\begin{enumerate}[leftmargin=*, nosep]
		...
		\end{enumerate} \\
		Postcondiciones & La publicación de cruza queda activa y las solicitudes recibidas quedan registradas con su estado y fecha para su consulta posterior (\hyperref[tab:CU-07]{CU-07}), incluyendo el conteo actualizado de procesos de cruza de la mascota para la alerta de uso repetitivo (\hyperref[tab:RF-27]{RF-27}). \\
	\end{atributos}
	
	\begin{atributos}{CU-06}{CU-06 --- Evaluar compatibilidad de cruza.}
		Identificador & \hyperref[tab:CU-06]{CU-06} \\
		...
		RF asociados & \hyperref[tab:RF-06]{RF-06}, \hyperref[tab:RF-16]{RF-16}, \hyperref[tab:RF-23]{RF-23} \\
		...
		\begin{enumerate}[leftmargin=*, nosep]
			...
			\item El sistema ejecuta una comprobación determinística del grado de parentesco a partir del linaje declarado.
			\item El sistema envía los datos al componente de evaluación de compatibilidad.
			\item El componente calcula el resultado orientativo y genera la explicación causal asociada (\hyperref[tab:RNF-13]{RNF-13}).
		...
		Flujos alternativos &
		\begin{enumerate}[leftmargin=*, nosep]
			\item[3.1] Se detecta un grado de parentesco incompatible: el sistema muestra una alerta explícita (\hyperref[tab:RF-23]{RF-23}) y finaliza sin generar recomendación.
		...
		\begin{enumerate}[leftmargin=*, nosep]
			...
			\item[5.1] El componente de IA no responde dentro del umbral definido en \hyperref[tab:RNF-03]{RNF-03}: el sistema informa la indisponibilidad temporal del servicio.
		\end{enumerate} \\
	...
	\begin{atributos}{CU-07}{CU-07 --- Gestionar solicitud de adopción.}
		Identificador & \hyperref[tab:CU-07]{CU-07} \\
		...
		RF asociados & \hyperref[tab:RF-05]{RF-05}, \hyperref[tab:RF-15]{RF-15}, \hyperref[tab:RF-08]{RF-08} \\
		Precondiciones & El adoptante está autenticado y la mascota está publicada en adopción. \\
		Flujo principal &
		\begin{enumerate}[leftmargin=*, nosep]
			\item El adoptante selecciona la mascota de interés y envía la solicitud de adopción.
			\item El sistema registra la solicitud con estado \emph{pendiente} y notifica al propietario o refugio.
			\item El propietario o refugio revisa los detalles de la solicitud.
			\item El propietario o refugio acepta o rechaza la solicitud.
			\item El sistema actualiza el estado de la solicitud y notifica el resultado al adoptante.
		\end{enumerate} \\
		Flujos alternativos &
		\begin{enumerate}[leftmargin=*, nosep]
			\item[5.1] El propietario o refugio consulta el historial consolidado de solicitudes de adopción y cruza de sus mascotas, filtrando por estado, tipo de proceso o rango de fechas.
		\end{enumerate} \\
		Flujos de excepción &
		\begin{enumerate}[leftmargin=*, nosep]
			\item[2.1] No existen solicitudes registradas para una mascota: el sistema muestra un mensaje informativo en lugar de una lista vacía.
		\end{enumerate} \\
		Postcondiciones & La solicitud queda marcada como aceptada o rechazada, y el adoptante y el propietario visualizan la trazabilidad completa del proceso sin que se expongan datos de contacto de solicitantes no aceptados, en cumplimiento del principio de minimización de datos (\hyperref[rl:05]{RL-05}). \\
	\end{atributos}
	
	\begin{atributos}{CU-08}{CU-08 --- Gestionar comunicación entre usuarios.}
		Identificador & \hyperref[tab:CU-08]{CU-08} \\
		...
		RF asociados & \hyperref[tab:RF-07]{RF-07} \\
		...
		Flujos alternativos &
		\begin{enumerate}[leftmargin=*, nosep]
			\item[2.1] El usuario adjunta una imagen; el sistema la somete a la validación de imágenes (\hyperref[tab:RF-17]{RF-17}) antes de la moderación de texto.
		\end{enumerate} \\
		Flujos de excepción &
		\begin{enumerate}[leftmargin=*, nosep]
			\item[4.1] El mensaje es marcado como inapropiado: el sistema bloquea el envío y notifica al remitente la categoría de infracción detectada conforme a \hyperref[tab:RNF-15]{RNF-15}, sin entregarlo al destinatario.
	...
	\begin{atributos}{CU-09}{CU-09 --- Validar información médica.}
		Identificador & \hyperref[tab:CU-09]{CU-09} \\
		Nombre & Validar información médica \\
		Actor principal & Veterinaria \\
		Actores secundarios & Propietario de mascota \\
		RF asociados & \hyperref[tab:RF-09]{RF-09}, \hyperref[tab:RF-14]{RF-14}, \hyperref[tab:RF-24]{RF-24}, \hyperref[tab:RF-26]{RF-26} \\
		Precondiciones & La veterinaria está autenticada, su identidad profesional ha sido verificada (\hyperref[tab:RF-12]{RF-12}) y existe información médica en estado \emph{PendienteDeValidación}. \\
		Flujo principal &
		\begin{enumerate}[leftmargin=*, nosep]
		...
		\begin{enumerate}[leftmargin=*, nosep]
			...
			\item[5.1] Una segunda veterinaria autorizada solicita revisar un registro ya validado (\hyperref[tab:RF-26]{RF-26}): el sistema le muestra el registro y la validación previa, y permite confirmar o cuestionar el resultado sin sustituir la primera validación.
		\end{enumerate} \\
		Flujos de excepción &
		\begin{enumerate}[leftmargin=*, nosep]
		...
		\end{enumerate} \\
		Postcondiciones & El registro médico queda en estado \emph{Validado} o \emph{Rechazado} con trazabilidad del profesional responsable y de la fecha de la resolución, en cumplimiento de \hyperref[tab:RD-03]{RD-03} y \hyperref[tab:RNF-05]{RNF-05}, y puede quedar con una segunda validación registrada cuando aplica \hyperref[tab:RF-26]{RF-26}. \\
	\end{atributos}
	
	\begin{atributos}{CU-10}{CU-10 --- Consultar perfil médico.}
		Identificador & \hyperref[tab:CU-10]{CU-10} \\
		...
		RF asociados & \hyperref[tab:RF-03]{RF-03}, \hyperref[tab:RF-11]{RF-11} \\
		...
		\begin{enumerate}[leftmargin=*, nosep]
			...
			\item El sistema aplica las reglas de privacidad configuradas por el propietario (\hyperref[tab:RF-11]{RF-11}).
			\item El sistema filtra la ubicación exacta y los datos de contacto directo, mostrando únicamente la zona aproximada (\hyperref[tab:RNF-16]{RNF-16}).
		...
		\begin{enumerate}[leftmargin=*, nosep]
			...
			\item[2.2] El consultante es una veterinaria autorizada: el sistema muestra los campos médicos obligatorios aun cuando el propietario los haya restringido para otros roles (\hyperref[tab:RD-03]{RD-03}).
		\end{enumerate} \\
		Flujos de excepción &
		\begin{enumerate}[leftmargin=*, nosep]
	...
	\begin{atributos}{CU-11}{CU-11 --- Buscar mascota disponible.}
		Identificador & \hyperref[tab:CU-11]{CU-11} \\
		Nombre & Buscar mascota disponible \\
		Actor principal & Adoptante, Interesado en cruza \\
		Actores secundarios & --- \\
		RF asociados & \hyperref[tab:RF-04]{RF-04}, \hyperref[tab:RF-13]{RF-13} \\
		...
		Flujos alternativos &
		\begin{enumerate}[leftmargin=*, nosep]
			\item[6.1] El usuario selecciona una mascota del listado e invoca \hyperref[tab:CU-10]{CU-10} para consultar su perfil.
		\end{enumerate} \\
		Flujos de excepción &
		\begin{enumerate}[leftmargin=*, nosep]
	...
	\begin{atributos}{CU-12}{CU-12 --- Configurar privacidad médica.}
		...
		\begin{enumerate}[leftmargin=*, nosep]
			...
			\item El sistema aplica las nuevas reglas a todas las consultas posteriores (\hyperref[tab:CU-10]{CU-10}).
		\end{enumerate} \\
		Flujos alternativos &
		\begin{enumerate}[leftmargin=*, nosep]
		...
		\end{enumerate} \\
		Postcondiciones & La configuración de privacidad queda vigente y se aplica de forma inmediata sobre todas las consultas del perfil, garantizando la visibilidad mínima exigida por \hyperref[tab:RD-03]{RD-03}. \\
	\end{atributos}
	
	\begin{atributos}{CU-13}{CU-13 --- Consultar recomendaciones de cuidado.}
		Identificador & \hyperref[tab:CU-13]{CU-13} \\
		Nombre & Consultar recomendaciones de cuidado \\
		Actor principal & Adoptante \\
		Actores secundarios & --- \\
		RF asociados & \hyperref[tab:RF-10]{RF-10} \\
		Precondiciones & El adoptante está autenticado y ha seleccionado una mascota registrada. \\
		Flujo principal &
		\begin{enumerate}[leftmargin=*, nosep]
			\item El adoptante selecciona la mascota sobre la que desea recomendaciones de cuidado.
			\item El sistema recupera la especie, la raza y la fecha de nacimiento de la mascota.
			\item El sistema consulta las pautas de cuidado correspondientes a esa especie, raza y edad.
			\item El sistema consulta las vacunas pendientes de la mascota.
			\item El sistema presenta las recomendaciones de cuidado junto con el estado de vacunación.
		\end{enumerate} \\
		Flujos alternativos &
		\begin{enumerate}[leftmargin=*, nosep]
			\item[3.1] La especie o la raza no están especificadas: el sistema entrega recomendaciones básicas de cuidado general en lugar de recomendaciones personalizadas.
		\end{enumerate} \\
		Flujos de excepción &
		\begin{enumerate}[leftmargin=*, nosep]
			\item[2.1] La mascota no tiene fecha de nacimiento registrada: el sistema omite las pautas dependientes de edad y presenta el resto de las recomendaciones disponibles.
		\end{enumerate} \\
	...
	El diagrama de clases refinado incorpora los atributos, tipos de dato,
	...
	los datos generales exigidos por \hyperref[tab:RF-01]{RF-01} (\texttt{breed}, \texttt{name},
	\texttt{species}, \texttt{petPhotos}, \texttt{birthDate}) y se asocia
	mediante composición (rombo relleno) con \texttt{MedicalHistory}, que
	incorpora el antecedente genético exigido por \hyperref[tab:RF-16]{RF-16} como su atributo
	\texttt{geneticBackground} en lugar de una clase separada. \texttt{Pet} se
	relaciona además con \texttt{PetVaccine} (\hyperref[tab:RF-14]{RF-14}), una clase asociativa que
	enlaza cada aplicación de vacuna con su \texttt{Pet} y su
	\texttt{Vaccine}; con \texttt{VetDocument} (\hyperref[tab:RF-19]{RF-19}), que registra el estado
	de revisión de cada certificado adjunto; con \texttt{PrivacySettings}
	(\hyperref[tab:RF-11]{RF-11}), que administra la visibilidad de la ubicación y el contacto; y
	con \texttt{Interaction} (\hyperref[tab:RF-20]{RF-20}, \hyperref[tab:RF-23]{RF-23}), que registra los encuentros entre
	...
	verificación de identidad (\hyperref[tab:RF-12]{RF-12}) y evita duplicar reglas de acceso entre
	los distintos roles descritos en \hyperref[tab:RD-02]{RD-02}; \texttt{User} envía y recibe
	instancias de \texttt{Message} (\hyperref[tab:RF-07]{RF-07}) y presenta la operación
	\texttt{verifyIdentity()} exigida por \hyperref[tab:RF-12]{RF-12}. La clase
	\texttt{AdoptionRequest} incorpora los atributos de justificación y
	condiciones del hogar exigidos por \hyperref[tab:RF-05]{RF-05}, mientras que
	...
	inteligencia artificial (\hyperref[tab:RF-06]{RF-06}); ninguna de las dos clases de solicitud
	incorpora de forma explícita las etapas de entrevista, visita y periodo de
	prueba exigidas por \hyperref[tab:RF-18]{RF-18}, que se representan en cambio en la máquina de
	...
	exigida por \hyperref[tab:RF-13]{RF-13}, y \texttt{VeterinaryEvaluation} registra la validación
	profesional exigida por \hyperref[tab:RF-09]{RF-09}, con las operaciones
	...
	íntegramente en las clases ya representadas en el diagrama: \hyperref[tab:RF-26]{RF-26}
	reutiliza \texttt{MedicalHistory} y la generalización de
	\texttt{Veterinarian}, registrando la segunda validación como una nueva
	instancia de \texttt{VeterinaryEvaluation}; y \hyperref[tab:RF-27]{RF-27} reutiliza
	...
	\paragraph{Cobertura de \hyperref[tab:RF-21]{RF-21}, \hyperref[tab:RF-24]{RF-24} y \hyperref[tab:RF-25]{RF-25} en el diagrama de clases.}
	La clase \texttt{Pet} incluye los atributos
	\texttt{-microchipId~:~string} y \texttt{-microchipDate~:~datetime}
	(\hyperref[tab:RF-21]{RF-21}); la clase \texttt{PetVaccine} incluye los atributos
	\texttt{-batchNumber~:~string} y \texttt{-administeredBy~:~string}
	(\hyperref[tab:RF-24]{RF-24}); y la clase \texttt{LostPetAlert}
	(\hyperref[tab:RF-25]{RF-25}) incluye los atributos \texttt{-alertId~:~long},
	...
	% NOTA INTERNA (no visible al compilar): la vista general del diagrama
	% de clases (Figura fig:clases-refinado, DiagramaClasesRefinado_MundiPets.png)
	% aun no muestra microchipId, microchipDate ni LostPetAlert; solo la
	% vista parcial "Pets and Health" (fig:clases-pets-health) los incluye.
	% Falta reexportar la vista general desde la herramienta de modelado
	% para que ambas coincidan.

	\begin{figure}[H]
		\centering
		\includegraphics[width=0.72\textwidth]{figuras/DiagramaClasesRefinado_MundiPets.png}
		\caption{Diagrama de clases refinado del sistema MundiPets (vista general).}
	...
	Figura~\ref{fig:clases-refinado}.
	...
	\begin{figure}[H]
		\centering
		\includegraphics[width=\textwidth]{figuras/DiagramaClasesRefinado_MundiPets_UsersRoles.png}
	...
	sostienen \hyperref[tab:RF-07]{RF-07} y \hyperref[tab:RF-13]{RF-13}: \texttt{Message} (\texttt{messageId},
	...
	\hyperref[tab:RF-09]{RF-09}/\hyperref[tab:RF-26]{RF-26} y \hyperref[tab:RF-20]{RF-20}/\hyperref[tab:RF-23]{RF-23}, esta última mediante su método
	\texttt{checkRiskAlert()}.

	\begin{figure}[H]
		\centering
		\includegraphics[width=\textwidth]{figuras/DiagramaClasesRefinado_MundiPets_PetsHealth.png}
		\caption{Diagrama de clases refinado --- vista parcial \emph{Pets and Health}.}
	...
	presentan los trece diagramas completos (\hyperref[tab:CU-01]{CU-01} a \hyperref[tab:CU-13]{CU-13}).

	\subsubsection{Registrar mascota (CU-01)}

	El diagrama de secuencia de \hyperref[tab:CU-01]{CU-01} ilustra el flujo mediante el cual el
	...
	definidos en \hyperref[tab:RF-01]{RF-01} antes de invocar al repositorio; si la validación falla,
	...
	\begin{figure}[H]
		\centering
		\includegraphics[width=\textwidth]{figuras/DiagramaSecuencia_CU-01_MundiPets.png}
	...
	registrar la información médica de la mascota (\hyperref[tab:RF-02]{RF-02}). El propietario
	...
	con la trazabilidad de cepa, lote y aplicador exigida por \hyperref[tab:RF-24]{RF-24}, se
	...
	\begin{figure}[H]
		\centering
		\includegraphics[width=\textwidth]{figuras/DiagramaSecuencia_CU-02_MundiPets.png}
	...
	El diagrama de secuencia de \hyperref[tab:CU-03]{CU-03} detalla la carga de documentos
	veterinarios digitalizados (\hyperref[tab:RF-14]{RF-14}). El propietario adjunta el archivo; el
	...
	\begin{figure}[H]
		\centering
		\includegraphics[width=\textwidth]{figuras/DiagramaSecuencia_CU-03_MundiPets.png}
	...
	El diagrama de secuencia de \hyperref[tab:CU-04]{CU-04} representa el proceso de publicación
	descrito en \hyperref[tab:RF-13]{RF-13}. Antes de crear una publicación nueva, el controlador
	...
	\begin{figure}[H]
		\centering
		\includegraphics[width=\textwidth]{figuras/DiagramaSecuencia_CU-04_MundiPets.png}
	...
	disponible para cruza responsable (\hyperref[tab:RF-08]{RF-08}, \hyperref[tab:RF-13]{RF-13}). Antes de crear la
	...
	solicita al propietario completarlo primero, en cumplimiento de \hyperref[tab:RF-16]{RF-16}, y
	...
	receptor --- y la regla de negocio de \hyperref[tab:RF-27]{RF-27} sobre solicitudes repetitivas
	no quedan representadas en este diagrama de publicación, sino en el flujo
	de gestión de solicitudes descrito en \hyperref[tab:CU-07]{CU-07} y en la máquina de estados de
	la Sección~\ref{sec:estados}.

	\begin{figure}[H]
		\centering
		\includegraphics[width=\textwidth]{figuras/DiagramaSecuencia_CU-05_MundiPets.png}
	...
	Esta secuencia contiene la interacción más compleja del sistema desde el
	punto de vista de la inteligencia artificial, integrando en un solo
	diagrama tanto la verificación de parentesco como la evaluación de
	compatibilidad. Tras la selección de las dos mascotas, el controlador
	(\texttt{CompatibilityController}) verifica primero el grado de
	parentesco declarado y, si no existe incompatibilidad, extrae en un ciclo
	el historial clínico de cada mascota y envía los datos al servicio
	externo de inteligencia artificial (\texttt{ExternalAIService}); si el
	resultado obtenido indica un puntaje de compatibilidad igual o superior a
	0.85 y ausencia de consanguinidad, el sistema emite el informe de
	compatibilidad, y si detecta un campo faltante o consanguinidad, retorna
	en su lugar una alerta de riesgo biológico sin llegar a emitir una
	recomendación.

	\begin{figure}[H]
		\centering
		\includegraphics[width=\textwidth]{figuras/DiagramaSecuencia_CU-13_MundiPets.png}
		\caption{Diagrama de secuencia del caso de uso CU-06 --- Evaluar compatibilidad de cruza y verificación de parentesco.}
		\label{fig:sec-cu06}
	\end{figure}

	\subsubsection{Gestionar solicitud de adopción (CU-07)}

	Esta secuencia representa la gestión de una solicitud de adopción ya
	enviada (\hyperref[tab:RF-05]{RF-05}, \hyperref[tab:RF-18]{RF-18}), complementando la publicación cubierta por \hyperref[tab:CU-04]{CU-04}.
	El adoptante consulta el perfil (\hyperref[tab:CU-10]{CU-10}) y envía la solicitud, que el
	sistema registra en estado \emph{pending} y notifica al propietario o
	refugio receptor; este revisa la solicitud y decide aceptarla o
	rechazarla, y el sistema actualiza el estado correspondiente y notifica al
	adoptante en ambos casos. Esta secuencia es la contraparte, para adopción,
	de la gestión de solicitud de cruza descrita en el flujo alterno de \hyperref[tab:CU-05]{CU-05}.

	\begin{figure}[H]
		\centering
		\includegraphics[width=\textwidth]{figuras/DiagramaSecuencia_CU-07_MundiPets.png}
		\caption{Diagrama de secuencia del caso de uso CU-07 --- Gestionar solicitud de adopción.}
		\label{fig:sec-cu07}
	\end{figure}
	...
	(\hyperref[tab:RF-07]{RF-07}). El remitente selecciona al destinatario y envía el mensaje; el
	...
	explicación por ejemplos exigida por \hyperref[tab:RNF-15]{RNF-15} ante un mensaje reportado se
	documenta en la especificación textual de \hyperref[tab:RF-07]{RF-07} y en la ficha de \hyperref[tab:RNF-15]{RNF-15},
	no en esta secuencia de envío.

	\begin{figure}[H]
		\centering
		\includegraphics[width=\textwidth]{figuras/DiagramaSecuencia_CU-08_MundiPets.png}
	...
	El diagrama de secuencia de \hyperref[tab:CU-09]{CU-09} modela la revisión que realiza la
	veterinaria sobre el historial médico y los documentos cargados por el
	propietario (\hyperref[tab:RF-09]{RF-09}). El controlador (\texttt{ValidationController}) reúne
	...
	veterinaria de \hyperref[tab:RF-26]{RF-26} se satisface mediante una nueva ejecución de esta
	misma secuencia sobre un registro que ya se encuentra validado, iniciada
	por una veterinaria distinta.

	\begin{figure}[H]
		\centering
		\includegraphics[width=\textwidth]{figuras/DiagramaSecuencia_CU-09_MundiPets.png}
	...
	(\hyperref[tab:RF-03]{RF-03}), incluyendo la aplicación de las reglas de privacidad configuradas
	por el propietario (\hyperref[tab:RF-11]{RF-11}). El controlador (\texttt{PetProfileController})
	...
	\begin{figure}[H]
		\centering
		\includegraphics[width=\textwidth]{figuras/DiagramaSecuencia_CU-10_MundiPets.png}
	...
	El diagrama de secuencia de \hyperref[tab:CU-11]{CU-11} modela la búsqueda y el filtrado de
	mascotas disponibles (\hyperref[tab:RF-04]{RF-04}). El adoptante accede a la búsqueda y aplica
	...
	\begin{figure}[H]
		\centering
		\includegraphics[width=\textwidth]{figuras/DiagramaSecuencia_CU-11_MundiPets.png}
	...
	información descrita en \hyperref[tab:RF-11]{RF-11}. El propietario consulta las opciones de
	...
	exigida por \hyperref[tab:RD-03]{RD-03}, devolviendo una advertencia específica si se intenta
	ocultar ese campo en particular, mientras que el resto de la configuración
	se guarda con normalidad.

	\begin{figure}[H]
		\centering
		\includegraphics[width=\textwidth]{figuras/DiagramaSecuencia_CU-06_MundiPets.png}
		\caption{Diagrama de secuencia del caso de uso CU-12 --- Configurar privacidad médica.}
		\label{fig:sec-cu12}
	\end{figure}

	\subsubsection{Consultar recomendaciones de cuidado (CU-13)}

	Esta secuencia representa la consulta de recomendaciones de cuidado
	preventivo (\hyperref[tab:RF-10]{RF-10}). El controlador (\texttt{CareRecommendationController})
	obtiene la especie, la raza y la fecha de nacimiento de la mascota; si
	ambos datos están especificados, consulta las pautas de cuidado
	correspondientes y las vacunas pendientes para generar una recomendación
	específica, y si falta alguno de los dos datos, retorna en su lugar una
	recomendación básica genérica.

	\begin{figure}[H]
		\centering
		\includegraphics[width=\textwidth]{figuras/DiagramaSecuencia_CU-12_MundiPets.png}
		\caption{Diagrama de secuencia del caso de uso CU-13 --- Consultar recomendaciones de cuidado.}
		\label{fig:sec-cu13}
	\end{figure}
	...
	mensajes. A continuación se presentan los trece diagramas completos (\hyperref[tab:CU-01]{CU-01} a
	\hyperref[tab:CU-13]{CU-13}), organizados por carriles entre el actor y el sistema.

	\subsubsection{Registrar mascota (CU-01)}

	El diagrama de actividad de \hyperref[tab:CU-01]{CU-01} muestra el flujo completo desde que el
	...
	validación de imágenes exigida por \hyperref[tab:RF-17]{RF-17} --- y finalmente el almacenamiento
	del registro.
	...
	mascota (\hyperref[tab:RF-02]{RF-02}): el propietario completa y envía el formulario, y el
	...
	de cepa, lote y aplicador de \hyperref[tab:RF-24]{RF-24}; ambos aspectos se gestionan mediante el
	...
	\begin{figure}[H]
		\centering
		\includegraphics[width=0.8\textwidth]{figuras/DiagramaActividades_CU-02_MundiPets.png}
	...
	El diagrama de actividad de \hyperref[tab:CU-03]{CU-03} representa el flujo de carga de
	documentos veterinarios (\hyperref[tab:RF-14]{RF-14}). Tras adjuntar el archivo, el sistema
	...
	de \hyperref[tab:CU-09]{CU-09} --- queda pendiente hasta la revisión de una veterinaria
	autorizada.
	...
	Este diagrama expone el flujo de publicación para adopción (\hyperref[tab:RF-13]{RF-13}),
	...
	\begin{figure}[H]
		\centering
		\includegraphics[width=0.8\textwidth]{figuras/DiagramaActividades_CU-04_MundiPets.png}
	...
	cruza responsable (\hyperref[tab:RF-08]{RF-08}, \hyperref[tab:RF-13]{RF-13}). La bifurcación central del diagrama
	...
	exige \hyperref[tab:RF-16]{RF-16}. La gestión posterior de la solicitud recibida --- aceptación
	o rechazo por el propietario receptor --- y la regla de negocio de \hyperref[tab:RF-27]{RF-27}
	sobre solicitudes repetitivas se representan en el flujo de gestión de
	solicitud de adopción (\hyperref[tab:CU-07]{CU-07}), cuyo patrón es análogo para cruza, y en la
	máquina de estados de la Sección~\ref{sec:estados}.
	...
	Este diagrama expone la lógica de decisión que subyace a la
	recomendación de compatibilidad, integrando en un solo flujo la
	verificación de parentesco y la evaluación por inteligencia artificial.
	Tras la selección de las dos mascotas, el sistema verifica primero el
	grado de parentesco declarado y, si no existe incompatibilidad, extrae el
	historial clínico de ambas e invoca al servicio externo de evaluación; la
	...
	uso (flujo alterno 3.1 de la ficha de \hyperref[tab:CU-06]{CU-06}) considera inviable.

	\begin{figure}[H]
		\centering
		\includegraphics[width=1\textwidth]{figuras/DiagramaActividades_CU-13_MundiPets.png}
		\caption{Diagrama de actividad del caso de uso CU-06 --- Evaluar compatibilidad de cruza y verificación de parentesco.}
		\label{fig:act-cu06}
	\end{figure}
	...
	enviada (\hyperref[tab:RF-05]{RF-05}, \hyperref[tab:RF-18]{RF-18}): tras consultar el perfil (\hyperref[tab:CU-10]{CU-10}) y enviar la
	...
	\begin{figure}[H]
		\centering
		\includegraphics[width=\textwidth]{figuras/DiagramaActividades_CU-07_MundiPets.png}
		\caption{Diagrama de actividad del caso de uso CU-07 --- Gestionar solicitud de adopción.}
		\label{fig:act-cu07}
	\end{figure}

	\subsubsection{Gestionar comunicación entre usuarios (CU-08)}

	El diagrama de actividad de \hyperref[tab:CU-08]{CU-08} detalla el flujo de envío de un mensaje
	entre usuarios (\hyperref[tab:RF-07]{RF-07}): el sistema busca la conversación existente entre
	...
	manual y la explicación por ejemplos de \hyperref[tab:RNF-15]{RNF-15} se documentan en la
	especificación textual de \hyperref[tab:RF-07]{RF-07}, no como una actividad de este flujo de
	envío.

	\begin{figure}[H]
		\centering
		\includegraphics[width=\textwidth]{figuras/DiagramaActividades_CU-08_MundiPets.png}
	...
	sobre el historial médico y los documentos cargados (\hyperref[tab:RF-09]{RF-09}). La
	...
	Sección~\ref{sec:estados}. La segunda opinión veterinaria de \hyperref[tab:RF-26]{RF-26}
	corresponde a una nueva ejecución de este mismo diagrama de actividad
	sobre un registro ya validado.

	\begin{figure}[H]
		\centering
		\includegraphics[width=0.8\textwidth]{figuras/DiagramaActividades_CU-09_MundiPets.png}
	...
	El diagrama de actividad de \hyperref[tab:CU-10]{CU-10} expone el flujo de consulta de perfil
	completo de una mascota (\hyperref[tab:RF-03]{RF-03}), incluyendo la verificación de permisos de
	visibilidad según la configuración de privacidad definida por el
	propietario (\hyperref[tab:RF-11]{RF-11}). El flujo culmina con la presentación diferenciada del
	perfil: información completa para usuarios con permisos, y una versión
	restringida para el resto.

	\begin{figure}[H]
		\centering
		\includegraphics[width=\textwidth]{figuras/DiagramaActividades_CU-10_MundiPets.png}
	...
	(\hyperref[tab:RF-04]{RF-04}). Tras la selección de criterios por parte del usuario, el sistema
	...
	Este diagrama de actividad expone la lógica de decisión de la
	configuración de privacidad (\hyperref[tab:RF-11]{RF-11}). Tras seleccionar los datos a
	compartir y guardar la configuración, la bifurcación central (\emph{Field
	blocked?}) verifica si algún campo restringido es obligatorio para la
	validación veterinaria; en caso afirmativo, el sistema muestra una
	advertencia de visibilidad y no aplica esa restricción puntual, mientras
	que en caso negativo almacena la configuración completa y actualiza los
	permisos.
	...
	Este diagrama de actividad representa la consulta de recomendaciones de
	cuidado preventivo asociada a \hyperref[tab:RF-10]{RF-10}. El sistema obtiene los datos de la
	mascota y, si la especie y la raza están especificadas, consulta las
	pautas correspondientes y las vacunas pendientes para presentar una
	recomendación específica; si falta alguno de esos datos, presenta en su
	lugar una recomendación básica genérica.

	\begin{figure}[H]
		\centering
		\includegraphics[width=0.9\textwidth]{figuras/DiagramaActividades_CU-12_MundiPets.png}
		\caption{Diagrama de actividad del caso de uso CU-13 --- Consultar recomendaciones de cuidado.}
		\label{fig:act-cu13}
	\end{figure}
	...
	\emph{Drafted} al ejecutarse \texttt{registerPet()} (\hyperref[tab:RF-01]{RF-01}), y retorna a
	...
	imágenes exigida por \hyperref[tab:RF-17]{RF-17} --- y \emph{SpeciesCompatibilityChecked}. Al
	completarse esta verificación (\texttt{publishProfile()}), el perfil
	alcanza el estado \emph{Published} (\hyperref[tab:RF-13]{RF-13}), desde el cual una solicitud
	...
	post-adopción definido por \hyperref[tab:RF-22]{RF-22}.
	...
	Esta máquina de estados formaliza el flujo por etapas exigido por \hyperref[tab:RF-18]{RF-18}
	para las solicitudes de adopción, y el flujo de aceptación o rechazo
	definido por \hyperref[tab:RF-08]{RF-08} para las solicitudes de cruza. Una solicitud se crea
	...
	\emph{IdentityValidated} (\hyperref[tab:RF-12]{RF-12}), \emph{InformationReviewed} y
	...
	en \hyperref[tab:RF-15]{RF-15}. Cada transición de una solicitud de cruza hacia \emph{Approved}
	incrementa el contador de procesos completados de la mascota implicada,
	valor que \hyperref[tab:RF-27]{RF-27} consulta antes de admitir una nueva solicitud sobre la
	...
	\hyperref[tab:RD-03]{RD-03}. Del veredicto emitido, el documento transita a \emph{validated}
	...
	validación de \hyperref[tab:RF-26]{RF-26} se registra como un nuevo evento sobre el estado
	...
	(\hyperref[tab:RD-05]{RD-05}) en una organización concreta de módulos de software con interfaces
	...
	profiles} (\texttt{IProfile}, \hyperref[tab:RF-01]{RF-01}, \hyperref[tab:RF-02]{RF-02}, \hyperref[tab:RF-13]{RF-13}, \hyperref[tab:RF-16]{RF-16}, \hyperref[tab:RF-21]{RF-21}, \hyperref[tab:RF-25]{RF-25}),
	\emph{request handling} (\texttt{IRequest}, \hyperref[tab:RF-05]{RF-05}, \hyperref[tab:RF-08]{RF-08}, \hyperref[tab:RF-18]{RF-18}, \hyperref[tab:RF-19]{RF-19},
	\hyperref[tab:RF-20]{RF-20}, \hyperref[tab:RF-22]{RF-22}, \hyperref[tab:RF-27]{RF-27}) y \emph{medical validation} (\texttt{IMedical},
	\hyperref[tab:RF-09]{RF-09}, \hyperref[tab:RF-14]{RF-14}, \hyperref[tab:RF-24]{RF-24}, \hyperref[tab:RF-26]{RF-26}). Los dos componentes de inteligencia
	...
	capa de API: \emph{AI image validation} (\texttt{IImageCheck}, \hyperref[tab:RF-17]{RF-17}) es
	consumido por \emph{pet profiles}, y \emph{AI compatibility}
	(\texttt{IMatching}, \hyperref[tab:RF-06]{RF-06}) es consumido por \emph{request handling}. El
	...
	El diagrama no representa un componente de \emph{Mensajería} (\hyperref[tab:RF-07]{RF-07}) ni un
	...
	métrica de mantenibilidad definida en \hyperref[tab:RNF-10]{RNF-10}, según la cual una
	...
	WhatsApp Business API descrita en \hyperref[tab:RD-08]{RD-08} y \hyperref[tab:RNF-09]{RNF-09} para los recordatorios de
	controles preventivos de \hyperref[tab:RF-10]{RF-10}: el canal externo de notificación
	...
	íntegramente lo exigido por \hyperref[tab:RD-08]{RD-08}.

	\begin{figure}[H]
		\centering
		\includegraphics[width=\textwidth]{figuras/DiagramaDespliegue_MundiPets.png}
	...
	vigentes: 27 requisitos funcionales (\hyperref[tab:RF-01]{RF-01} a \hyperref[tab:RF-27]{RF-27}),
	16 requisitos no funcionales (\hyperref[tab:RNF-01]{RNF-01} a \hyperref[tab:RNF-16]{RNF-16}), 9 restricciones de diseño
	(\hyperref[tab:RD-01]{RD-01} a \hyperref[tab:RD-09]{RD-09}) y 9 requisitos legales (\hyperref[rl:01]{RL-01} a \hyperref[rl:09]{RL-09}), redactados en las
	Secciones~\ref{sec:rf}, \ref{sec:rnf}, \ref{sec:legales} y
	\ref{sec:restricciones}. \hyperref[tab:RF-26]{RF-26} y \hyperref[tab:RF-27]{RF-27} se incorporan a partir de la nueva
	evidencia recolectada de PROP-12 y VET-07. El alcance de \hyperref[tab:RF-06]{RF-06}, \hyperref[tab:RF-07]{RF-07},
	\hyperref[tab:RF-10]{RF-10}, \hyperref[tab:RNF-02]{RNF-02} y \hyperref[tab:RNF-03]{RNF-03} quedó acotado tras la revisión de calidad de
	requisitos (Sección~\ref{sec:trazabilidad}), y el de \hyperref[tab:RNF-15]{RNF-15} se redefinió
	para ser coherente con \hyperref[tab:RF-07]{RF-07}; dos propuestas de ampliación de alcance no se
	incorporaron por no contar con evidencia de entrevista que sustentara un
	requisito funcional nuevo y verificable, y quedan documentadas como
	limitaciones de alcance reconocidas en \hyperref[tab:RD-06]{RD-06} y \hyperref[rl:01]{RL-01}, respectivamente.
	...
	\begin{longtable}{@{}p{2.2cm} p{7.5cm} p{3.5cm}@{}}
		...
		\hline
		\endfoot
		\hyperref[tab:RF-01]{RF-01} & Registrar mascota & Requisito funcional \\
		\hline
		\hyperref[tab:RF-02]{RF-02} & Gestionar historial médico de la mascota & Requisito funcional \\
		\hline
		\hyperref[tab:RF-03]{RF-03} & Consultar perfil completo de una mascota & Requisito funcional \\
		\hline
		\hyperref[tab:RF-04]{RF-04} & Buscar y filtrar mascotas & Requisito funcional \\
		\hline
		\hyperref[tab:RF-05]{RF-05} & Gestionar solicitudes de adopción & Requisito funcional \\
		\hline
		\hyperref[tab:RF-06]{RF-06} & Evaluar compatibilidad entre mascotas para cruza & Requisito funcional \\
		\hline
		\hyperref[tab:RF-07]{RF-07} & Sistema de mensajería entre usuarios & Requisito funcional \\
		\hline
		\hyperref[tab:RF-08]{RF-08} & Gestionar solicitudes de cruza & Requisito funcional \\
		\hline
		\hyperref[tab:RF-09]{RF-09} & Validar la información médica de una mascota & Requisito funcional \\
		\hline
		\hyperref[tab:RF-10]{RF-10} & Gestionar recordatorios de controles preventivos & Requisito funcional \\
		\hline
		\hyperref[tab:RF-11]{RF-11} & Administrar la privacidad de la información & Requisito funcional \\
		\hline
		\hyperref[tab:RF-12]{RF-12} & Verificar la identidad de los usuarios & Requisito funcional \\
		\hline
		\hyperref[tab:RF-13]{RF-13} & Registrar publicaciones de mascotas & Requisito funcional \\
		\hline
		\hyperref[tab:RF-14]{RF-14} & Gestionar carnet de vacunación & Requisito funcional \\
		\hline
		\hyperref[tab:RF-15]{RF-15} & Consultar el historial de procesos de adopción y cruza & Requisito funcional \\
		\hline
		\hyperref[tab:RF-16]{RF-16} & Gestionar antecedentes genéticos y parentesco de la mascota & Requisito funcional \\
		\hline
		\hyperref[tab:RF-17]{RF-17} & Validar imágenes & Requisito funcional \\
		\hline
		\hyperref[tab:RF-18]{RF-18} & Gestionar el flujo de solicitud de adopción por etapas & Requisito funcional \\
		\hline
		\hyperref[tab:RF-19]{RF-19} & Validar certificados veterinarios antes de habilitar la cruza & Requisito funcional \\
		\hline
		\hyperref[tab:RF-20]{RF-20} & Coordinar encuentros de socialización supervisados & Requisito funcional \\
		\hline
		\hyperref[tab:RF-21]{RF-21} & Registrar el identificador de microchip de la mascota & Requisito funcional \\
		\hline
		\hyperref[tab:RF-22]{RF-22} & Dar seguimiento post-adopción & Requisito funcional \\
		\hline
		\hyperref[tab:RF-23]{RF-23} & Emitir alertas de riesgo sanitario o físico en interacciones y cruces & Requisito funcional \\
		\hline
		\hyperref[tab:RF-24]{RF-24} & Registrar trazabilidad de cepa, lote y aplicador de cada vacuna & Requisito funcional \\
		\hline
		\hyperref[tab:RF-25]{RF-25} & Publicar aviso de mascota extraviada & Requisito funcional \\
		\hline
		\hyperref[tab:RF-26]{RF-26} & Habilitar validación médica por segunda opinión veterinaria & Requisito funcional \\
		\hline
		\hyperref[tab:RF-27]{RF-27} & Controlar y alertar sobre solicitudes repetitivas de cruza de una misma mascota & Requisito funcional \\
		\hline
		\hyperref[tab:RNF-01]{RNF-01} & Protección de la información de propietarios y mascotas & Requisito no funcional \\
		\hline
		\hyperref[tab:RNF-02]{RNF-02} & Disponibilidad del sistema & Requisito no funcional \\
		\hline
		\hyperref[tab:RNF-03]{RNF-03} & Tiempo de respuesta del sistema & Requisito no funcional \\
		\hline
		\hyperref[tab:RNF-04]{RNF-04} & Facilidad de uso de la plataforma & Requisito no funcional \\
		\hline
		\hyperref[tab:RNF-05]{RNF-05} & Integridad de la información médica & Requisito no funcional \\
		\hline
		\hyperref[tab:RNF-06]{RNF-06} & Exactitud del componente de inteligencia artificial & Requisito no funcional \\
		\hline
		\hyperref[tab:RNF-07]{RNF-07} & Precisión en la validación de imágenes & Requisito no funcional \\
		\hline
		\hyperref[tab:RNF-08]{RNF-08} & Actualización de la información registrada & Requisito no funcional \\
		\hline
		\hyperref[tab:RNF-09]{RNF-09} & Interoperabilidad de notificaciones con canales externos & Requisito no funcional \\
		\hline
		\hyperref[tab:RNF-10]{RNF-10} & Facilidad de mantenimiento del código & Requisito no funcional \\
		\hline
		\hyperref[tab:RNF-11]{RNF-11} & Portabilidad entre navegadores y dispositivos & Requisito no funcional \\
		\hline
		\hyperref[tab:RNF-12]{RNF-12} & Escalabilidad ante crecimiento de usuarios & Requisito no funcional \\
		\hline
		\hyperref[tab:RNF-13]{RNF-13} & Explicabilidad de la recomendación de compatibilidad para cruza & Requisito no funcional \\
		\hline
		\hyperref[tab:RNF-14]{RNF-14} & Explicabilidad del rechazo o aceptación de imágenes & Requisito no funcional \\
		\hline
		\hyperref[tab:RNF-15]{RNF-15} & Explicabilidad de la moderación de mensajes & Requisito no funcional \\
		\hline
		\hyperref[tab:RNF-16]{RNF-16} & Confidencialidad de datos de ubicación y contacto & Requisito no funcional \\
		\hline
		\hyperref[tab:RD-01]{RD-01} & Integración de componentes de inteligencia artificial & Restricción de diseño \\
		\hline
		\hyperref[tab:RD-02]{RD-02} & Acceso basado en roles & Restricción de diseño \\
		\hline
		\hyperref[tab:RD-03]{RD-03} & Validación profesional de la información médica & Restricción de diseño \\
		\hline
		\hyperref[tab:RD-04]{RD-04} & Protección de datos personales & Restricción de diseño \\
		\hline
		\hyperref[tab:RD-05]{RD-05} & Arquitectura modular del sistema & Restricción de diseño \\
		\hline
		\hyperref[tab:RD-06]{RD-06} & Mecanismo de eliminación de cuenta y datos personales & Restricción de diseño \\
		\hline
		\hyperref[tab:RD-07]{RD-07} & Protocolo de notificación de vulneraciones de seguridad & Restricción de diseño \\
		\hline
		\hyperref[tab:RD-08]{RD-08} & Dependencia de servicios externos de mensajería & Restricción de diseño \\
		\hline
		\hyperref[tab:RD-09]{RD-09} & Anonimización de evidencias de campo para publicación abierta & Restricción de diseño \\
		\hline
		\hyperref[rl:01]{RL-01} & Consentimiento libre, específico, informado e inequívoco & Requisito legal \\
		\hline
		\hyperref[rl:02]{RL-02} & Información de finalidad y derecho de acceso & Requisito legal \\
		\hline
		\hyperref[rl:03]{RL-03} & Derecho de rectificación y actualización & Requisito legal \\
		\hline
		\hyperref[rl:04]{RL-04} & Derecho de supresión de datos personales & Requisito legal \\
		\hline
		\hyperref[rl:05]{RL-05} & Minimización y seguridad de los datos & Requisito legal \\
		\hline
		\hyperref[rl:06]{RL-06} & Derecho a explicación de decisiones automatizadas & Requisito legal \\
		\hline
		\hyperref[rl:07]{RL-07} & Confidencialidad reforzada de datos sensibles y de salud & Requisito legal \\
		\hline
		\hyperref[rl:08]{RL-08} & Protección de datos desde el diseño y por defecto & Requisito legal \\
		\hline
		\hyperref[rl:09]{RL-09} & Notificación de vulneraciones de seguridad & Requisito legal \\
	\end{longtable}
	...
	MoSCoW extendida de los 61 requisitos vigentes del proyecto. La misma
	priorización, junto con su clasificación Kano y su valor WSJF, se
	consolida en el
	...
	\begin{longtable}{@{}p{1.6cm} p{4.8cm} p{1.6cm} p{5.2cm}@{}}
		...
		\hline
		\endfoot
		\hyperref[tab:RF-01]{RF-01} & Registrar mascota & Must & Es el pilar del sistema; sin este RF no puede existir ningún perfil de mascota ni proceso posterior. \\
		\hline
		\hyperref[tab:RF-02]{RF-02} & Gestionar historial médico & Must & Crítico para la salud animal y para \hyperref[rl:03]{RL-03} (rectificación de datos médicos). \\
		\hline
		\hyperref[tab:RF-03]{RF-03} & Consultar perfil completo & Must & Fundamental para que adoptantes e interesados en cruza tomen decisiones informadas; sustenta \hyperref[rl:02]{RL-02}. \\
		\hline
		\hyperref[tab:RF-04]{RF-04} & Buscar y filtrar mascotas & Should & Mejora la usabilidad, pero el sistema puede operar inicialmente con listas generales. \\
		\hline
		\hyperref[tab:RF-05]{RF-05} & Gestionar solicitudes de adopción & Must & Núcleo del modelo operativo de la plataforma. \\
		\hline
		\hyperref[tab:RF-06]{RF-06} & Evaluar compatibilidad para cruza & Must & Requisito diferenciador que usa IA; motivó \hyperref[tab:RD-01]{RD-01} y exige la explicabilidad de \hyperref[rl:06]{RL-06}. \\
		\hline
		\hyperref[tab:RF-07]{RF-07} & Sistema de mensajería & Could & Deseable para coordinar procesos, pero el contacto inicial puede derivarse a canales externos si hay limitaciones de tiempo. \\
		\hline
		\hyperref[tab:RF-08]{RF-08} & Gestionar solicitudes de cruza & Must & Operación central del modelo de negocio junto a \hyperref[tab:RF-05]{RF-05}. \\
		\hline
		\hyperref[tab:RF-09]{RF-09} & Validar información médica & Must & Imprescindible para la confiabilidad clínica del sistema. \\
		\hline
		\hyperref[tab:RF-10]{RF-10} & Recordatorios de controles preventivos & Should & Aporta valor de bienestar animal, pero no bloquea el flujo principal de adopciones. \\
		\hline
		\hyperref[tab:RF-11]{RF-11} & Administrar privacidad & Must & Mandatorio por \hyperref[rl:02]{RL-02} y \hyperref[rl:05]{RL-05}; es el RF más citado en las entrevistas (9 EV). \\
		\hline
		\hyperref[tab:RF-12]{RF-12} & Verificar identidad de usuarios & Must & Indispensable para evitar fraude; sustenta el consentimiento de \hyperref[rl:01]{RL-01}. \\
		\hline
		\hyperref[tab:RF-13]{RF-13} & Registrar publicaciones & Must & Necesario para que un propietario active la disponibilidad de su mascota. \\
		\hline
		\hyperref[tab:RF-14]{RF-14} & Gestionar carnet de vacunación & Must & Crítico para la trazabilidad sanitaria; sustenta \hyperref[rl:03]{RL-03}. \\
		\hline
		\hyperref[tab:RF-15]{RF-15} & Consultar historial de procesos & Could & Aporta trazabilidad al usuario, pero no es indispensable para completar una adopción o cruza. \\
		\hline
		\hyperref[tab:RF-16]{RF-16} & Antecedentes genéticos y parentesco & Must & Necesario para que \hyperref[tab:RF-06]{RF-06} pueda evaluar compatibilidad de forma responsable. \\
		\hline
		\hyperref[tab:RF-17]{RF-17} & Validar imágenes con IA & Should & Refuerza la seguridad del contenido, pero el sistema puede operar con moderación manual inicial. \\
		\hline
		\hyperref[tab:RF-18]{RF-18} & Flujo de adopción por etapas & Must & Formaliza el proceso de adopción responsable identificado en la segunda ronda de campo (EV-11, EV-16). \\
		\hline
		\hyperref[tab:RF-19]{RF-19} & Validar certificados veterinarios antes de cruza & Should & Refuerza la confiabilidad de \hyperref[tab:RF-06]{RF-06}, pero no bloquea su funcionamiento básico. \\
		\hline
		\hyperref[tab:RF-20]{RF-20} & Encuentros de socialización supervisados & Could & Valor deseable de bienestar animal, mencionado por un solo participante (EV-13). \\
		\hline
		\hyperref[tab:RF-21]{RF-21} & Registrar identificador de microchip & Should & Aporta trazabilidad complementaria al carnet de vacunación. \\
		\hline
		\hyperref[tab:RF-22]{RF-22} & Seguimiento post-adopción & Must & Cierra el ciclo de responsabilidad de \hyperref[tab:RF-18]{RF-18}, exigido por participantes de la segunda ronda. \\
		\hline
		\hyperref[tab:RF-23]{RF-23} & Alertas de riesgo sanitario o físico & Must & Mitiga un riesgo real de seguridad y salud animal detectado en varias entrevistas (EV-09, EV-12, EV-15, EV-18). \\
		\hline
		\hyperref[tab:RF-24]{RF-24} & Trazabilidad de cepa, lote y aplicador & Should & Complementa \hyperref[tab:RF-14]{RF-14} con datos adicionales de trazabilidad, no bloqueante. \\
		\hline
		\hyperref[tab:RF-25]{RF-25} & Aviso de mascota extraviada & Could & Funcionalidad de valor social, pero fuera del núcleo de adopción/cruza del sistema. \\
		\hline
		\hyperref[tab:RF-26]{RF-26} & Segunda opinión veterinaria & Should & Refuerza la confiabilidad clínica mediante revisión entre pares; la veterinaria entrevistada (EV-24) la identificó como la mejora que más valor le aportaría, aunque no bloquea la validación individual ya cubierta por \hyperref[tab:RF-09]{RF-09}. \\
		\hline
		\hyperref[tab:RF-27]{RF-27} & Control de cruzas repetitivas & Should & Mitiga un riesgo de maltrato animal identificado explícitamente por la propietaria entrevistada (EV-23) al validar \hyperref[tab:RF-14]{RF-14} y en la síntesis final del walkthrough; no bloquea el flujo de solicitud de cruza ya cubierto por \hyperref[tab:RF-08]{RF-08}. \\
		\hline
		\hyperref[tab:RNF-01]{RNF-01} & Protección de información de propietarios y mascotas & Must & Requisito estricto de seguridad; sustenta \hyperref[rl:05]{RL-05} y \hyperref[rl:07]{RL-07}. \\
		\hline
		\hyperref[tab:RNF-02]{RNF-02} & Disponibilidad del sistema & Must & Crítico para la operatividad continua de la plataforma. \\
		\hline
		\hyperref[tab:RNF-03]{RNF-03} & Tiempo de respuesta del sistema & Should & Mejora la experiencia, pero el sistema es funcional con tiempos mayores en una primera versión. \\
		\hline
		\hyperref[tab:RNF-04]{RNF-04} & Facilidad de uso de la plataforma & Should & Relevante para adopción del producto, pero no bloquea sus funciones. \\
		\hline
		\hyperref[tab:RNF-05]{RNF-05} & Integridad de la información médica & Must & Evita modificaciones no autorizadas del historial clínico; sustenta \hyperref[rl:07]{RL-07}. \\
		\hline
		\hyperref[tab:RNF-06]{RNF-06} & Exactitud del componente de IA & Must & Sin una precisión mínima, \hyperref[tab:RF-06]{RF-06} pierde su valor diferenciador. \\
		\hline
		\hyperref[tab:RNF-07]{RNF-07} & Precisión en validación de imágenes & Should & Mejora la calidad del contenido, pero \hyperref[tab:RF-17]{RF-17} puede operar con una tasa de error inicial mayor. \\
		\hline
		\hyperref[tab:RNF-08]{RNF-08} & Actualización de la información registrada & Should & Mejora la confianza del usuario, no bloquea el flujo principal. \\
		\hline
		\hyperref[tab:RNF-09]{RNF-09} & Interoperabilidad de notificaciones & Should & Aporta valor (recordatorios por WhatsApp), pero el sistema opera sin ella mediante notificación interna. \\
		\hline
		\hyperref[tab:RNF-10]{RNF-10} & Facilidad de mantenimiento del código & Should & Importante para sostenibilidad técnica, pero no bloquea la operación funcional del sistema. \\
		\hline
		\hyperref[tab:RNF-11]{RNF-11} & Portabilidad entre navegadores & Should & Necesaria para alcance de usuarios, pero el sistema puede operar inicialmente en un navegador principal. \\
		\hline
		\hyperref[tab:RNF-12]{RNF-12} & Escalabilidad ante crecimiento de usuarios & Could & Deseable a mediano plazo; no crítica mientras la base de usuarios sea reducida. \\
		\hline
		\hyperref[tab:RNF-13]{RNF-13} & Explicabilidad de compatibilidad para cruza & Must & Exigido por el derecho legal a explicación (\hyperref[rl:06]{RL-06}) sobre decisiones automatizadas del \hyperref[tab:RF-06]{RF-06}. \\
		\hline
		\hyperref[tab:RNF-14]{RNF-14} & Explicabilidad de rechazo de imágenes & Should & Mejora la confianza del usuario, pero el sistema puede operar con un mensaje de rechazo genérico inicialmente. \\
		\hline
		\hyperref[tab:RNF-15]{RNF-15} & Explicabilidad de moderación de mensajes & Should & Igual que \hyperref[tab:RNF-14]{RNF-14}: deseable para confianza, no bloqueante. \\
		\hline
		\hyperref[tab:RNF-16]{RNF-16} & Confidencialidad de ubicación y contacto & Must & Obligación legal directa (\hyperref[rl:05]{RL-05}, principio de minimización). \\
		\hline
		\hyperref[tab:RD-01]{RD-01} & Integración de componentes de IA & Must & Restricción ineludible: arquitectura del principal valor agregado de MundiPets. \\
		\hline
		\hyperref[tab:RD-02]{RD-02} & Acceso basado en roles & Must & Mandatorio para diferenciar permisos de propietarios, veterinarias y administradores; sustenta \hyperref[rl:05]{RL-05} y \hyperref[rl:08]{RL-08}. \\
		\hline
		\hyperref[tab:RD-03]{RD-03} & Validación profesional de la información médica & Must & Exigido por los veterinarios entrevistados para garantizar confiabilidad clínica. \\
		\hline
		\hyperref[tab:RD-04]{RD-04} & Protección de datos personales & Must & Implementa directamente \hyperref[rl:05]{RL-05}, \hyperref[rl:07]{RL-07} y \hyperref[rl:08]{RL-08}. \\
		\hline
		\hyperref[tab:RD-05]{RD-05} & Arquitectura modular del sistema & Must & Facilita mantenimiento e incorporación de nuevas funcionalidades; sustenta \hyperref[tab:RNF-10]{RNF-10}. \\
		\hline
		\hyperref[tab:RD-06]{RD-06} & Eliminación de cuenta y datos personales & Must & Obligación legal ineludible (\hyperref[rl:04]{RL-04}, derecho de supresión); sin este mecanismo el sistema incumple la LOPDP. \\
		\hline
		\hyperref[tab:RD-07]{RD-07} & Protocolo de notificación de vulneraciones & Must & Exigido por \hyperref[rl:09]{RL-09} con plazos legales estrictos (5 y 3 días). \\
		\hline
		\hyperref[tab:RD-08]{RD-08} & Dependencia de servicios externos de mensajería & Should & Aporta valor (\hyperref[tab:RF-10]{RF-10}) pero requiere un canal de respaldo ante caídas del proveedor. \\
		\hline
		\hyperref[tab:RD-09]{RD-09} & Anonimización de evidencias para publicación abierta & Must & Exigido por los Art. 30-32 LOPDP para el uso de datos de investigación en el paquete FAIR. \\
		\hline
		\hyperref[rl:01]{RL-01} & Consentimiento libre, específico, informado e inequívoco & Must & Toda obligación legal es no negociable: su incumplimiento expone al proyecto a responsabilidad legal. \\
		\hline
		\hyperref[rl:02]{RL-02} & Información de finalidad y derecho de acceso & Must & Ídem: obligación legal directa (Art. 12-13 LOPDP). \\
		\hline
		\hyperref[rl:03]{RL-03} & Derecho de rectificación y actualización & Must & Ídem: obligación legal directa (Art. 14 LOPDP). \\
		\hline
		\hyperref[rl:04]{RL-04} & Derecho de supresión de datos personales & Must & Ídem: obligación legal directa (Art. 15 LOPDP). \\
		\hline
		\hyperref[rl:05]{RL-05} & Minimización y seguridad de los datos & Must & Ídem: obligación legal directa (Art. 10 LOPDP). \\
		\hline
		\hyperref[rl:06]{RL-06} & Derecho a explicación de decisiones automatizadas & Must & Ídem: obligación legal directa (Art. 20 LOPDP); base de todo el bloque de explicabilidad. \\
		\hline
		\hyperref[rl:07]{RL-07} & Confidencialidad reforzada de datos sensibles y de salud & Must & Ídem: obligación legal directa (Art. 25-30 LOPDP). \\
		\hline
		\hyperref[rl:08]{RL-08} & Protección de datos desde el diseño y por defecto & Must & Ídem: obligación legal directa (Art. 39 LOPDP). \\
		\hline
		\hyperref[rl:09]{RL-09} & Notificación de vulneraciones de seguridad & Must & Ídem: obligación legal directa (Art. 43 y 46 LOPDP), con plazos estrictos. \\
	\end{longtable}
	...
	\begin{table}[H]
		...
		\begin{tabularx}{\textwidth}{@{}l X c c c c@{}}
			...
			\midrule
			IM-01 & Registro del historial de vacunación & \hyperref[tab:RF-14]{RF-14}, \hyperref[tab:RF-24]{RF-24} & 4,67 & 82,0 & 0,83 \\
			IM-02 & Validación de la información médica por veterinaria & \hyperref[tab:RF-09]{RF-09}, \hyperref[tab:RD-03]{RD-03} & 4,69 & 77,0 & 0,65 \\
			IM-03 & Registro de enfermedades y condiciones previas & \hyperref[tab:RF-02]{RF-02} & 4,64 & 77,0 & 0,75 \\
			IM-04 & Registro de desparasitaciones & \hyperref[tab:RF-02]{RF-02} & 4,62 & 73,8 & 0,76 \\
			IM-05 & Evaluación de compatibilidad para cruza & \hyperref[tab:RF-06]{RF-06} & 4,51 & 62,3 & 0,70 \\
			IM-06 & Conocer la edad de la mascota & \hyperref[tab:RF-01]{RF-01}, \hyperref[tab:RF-03]{RF-03} & 4,43 & 59,0 & 0,83 \\
			IM-07 & Verificación de identidad de los usuarios & \hyperref[tab:RF-12]{RF-12} & 4,41 & 57,4 & 0,80 \\
			IM-08 & Filtros de búsqueda por edad, raza o ubicación & \hyperref[tab:RF-04]{RF-04} & 4,21 & 45,9 & 0,86 \\
			IM-09 & Fotografías actualizadas de la mascota & \hyperref[tab:RF-01]{RF-01}, \hyperref[tab:RF-03]{RF-03} & 4,05 & 36,1 & 0,92 \\
			IM-10 & Sistema de mensajería entre usuarios & \hyperref[tab:RF-07]{RF-07} & 3,98 & 32,8 & 0,88 \\
			IM-11 & Conocer la raza de la mascota & \hyperref[tab:RF-01]{RF-01}, \hyperref[tab:RF-03]{RF-03} & 3,92 & 42,6 & 1,20 \\
			\bottomrule
		\end{tabularx}
	\end{table}
	...
	diseñado para producir. \hyperref[tab:RF-26]{RF-26} y \hyperref[tab:RF-27]{RF-27}, incorporados en la presente
	...
	\emph{Must} y \emph{Should} —incluyendo \hyperref[tab:RF-26]{RF-26} y \hyperref[tab:RF-27]{RF-27}, incorporados en la
	...
	(\hyperref[tab:RF-07]{RF-07}, \hyperref[tab:RF-15]{RF-15}, \hyperref[tab:RF-20]{RF-20}, \hyperref[tab:RF-25]{RF-25}) se excluyeron del ejercicio por ser ya la
	...
	\begin{longtable}{@{}p{1.4cm} p{5.4cm} p{1.5cm} p{1.5cm} p{1.5cm} p{1.5cm} p{1.3cm}@{}}
		...
		\hline
		\endfoot
		\hyperref[tab:RF-01]{RF-01} & Registrar mascota & 20 & 13 & 13 & 5 & \textbf{9,20} \\
		\hline
		\hyperref[tab:RF-03]{RF-03} & Consultar perfil completo & 13 & 8 & 5 & 3 & \textbf{8,67} \\
		\hline
		\hyperref[tab:RF-13]{RF-13} & Registrar publicaciones de mascotas & 13 & 5 & 3 & 3 & \textbf{7,00} \\
		\hline
		\hyperref[tab:RF-12]{RF-12} & Verificar identidad de usuarios & 8 & 8 & 13 & 5 & \textbf{5,80} \\
		\hline
		\hyperref[tab:RF-11]{RF-11} & Administrar privacidad de la información & 20 & 13 & 13 & 8 & \textbf{5,75} \\
		\hline
		\hyperref[tab:RF-09]{RF-09} & Validar información médica & 8 & 5 & 8 & 5 & \textbf{4,20} \\
		\hline
		\hyperref[tab:RF-14]{RF-14} & Gestionar carnet de vacunación & 8 & 8 & 5 & 5 & \textbf{4,20} \\
		\hline
		\hyperref[tab:RF-02]{RF-02} & Gestionar historial médico & 13 & 8 & 8 & 8 & \textbf{3,63} \\
		\hline
		\hyperref[tab:RF-05]{RF-05} & Gestionar solicitudes de adopción & 13 & 8 & 8 & 8 & \textbf{3,63} \\
		\hline
		\hyperref[tab:RF-27]{RF-27} & Control de cruzas repetitivas & 3 & 2 & 5 & 3 & \textbf{3,33} \\
		\hline
		\hyperref[tab:RF-21]{RF-21} & Registrar identificador de microchip & 3 & 1 & 2 & 2 & \textbf{3,00} \\
		\hline
		\hyperref[tab:RF-08]{RF-08} & Gestionar solicitudes de cruza & 13 & 5 & 5 & 8 & \textbf{2,88} \\
		\hline
		\hyperref[tab:RF-10]{RF-10} & Recordatorios de controles preventivos & 8 & 3 & 3 & 5 & \textbf{2,80} \\
		\hline
		\hyperref[tab:RF-24]{RF-24} & Trazabilidad de cepa/lote/aplicador & 3 & 2 & 3 & 3 & \textbf{2,67} \\
		\hline
		\hyperref[tab:RF-23]{RF-23} & Alertas de riesgo sanitario/físico & 8 & 5 & 8 & 8 & \textbf{2,63} \\
		\hline
		\hyperref[tab:RF-19]{RF-19} & Validar certificados veterinarios & 5 & 3 & 5 & 5 & \textbf{2,60} \\
		\hline
		\hyperref[tab:RF-16]{RF-16} & Antecedentes genéticos y parentesco & 8 & 3 & 5 & 8 & \textbf{2,00} \\
		\hline
		\hyperref[tab:RF-04]{RF-04} & Buscar y filtrar mascotas & 5 & 2 & 2 & 5 & \textbf{1,80} \\
		\hline
		\hyperref[tab:RF-06]{RF-06} & Evaluar compatibilidad para cruza & 20 & 8 & 5 & 20 & \textbf{1,65} \\
		\hline
		\hyperref[tab:RF-26]{RF-26} & Segunda opinión veterinaria & 2 & 1 & 5 & 5 & \textbf{1,60} \\
		\hline
		\hyperref[tab:RF-22]{RF-22} & Seguimiento post-adopción & 5 & 2 & 3 & 8 & \textbf{1,25} \\
		\hline
		\hyperref[tab:RF-18]{RF-18} & Flujo de adopción por etapas & 8 & 3 & 5 & 13 & \textbf{1,23} \\
		\hline
		\hyperref[tab:RF-17]{RF-17} & Validar imágenes con IA & 8 & 3 & 5 & 13 & \textbf{1,23} \\
	\end{longtable}
	
	\textbf{Lectura del resultado.} El ranking confirma un patrón esperable en
	WSJF: no siempre lo más valioso encabeza la lista. \hyperref[tab:RF-06]{RF-06} (evaluar
	compatibilidad para cruza) y \hyperref[tab:RF-17]{RF-17} (validar imágenes con IA) tienen alto
	...
	entregar. En contraste, \hyperref[tab:RF-01]{RF-01}, \hyperref[tab:RF-03]{RF-03} y \hyperref[tab:RF-13]{RF-13} combinan alto valor con bajo
	...
	\ref{sec:mvp-alcance}. \hyperref[tab:RF-26]{RF-26} y \hyperref[tab:RF-27]{RF-27} obtienen un valor de negocio bajo
...
$\rightarrow$ Historia, en un formato de diez columnas que cierra en una
sola tabla los tres tipos de trazabilidad exigidos: vertical hacia atrás
(columna Fuente), vertical hacia adelante (Caso de Uso a Historia) y
horizontal entre requisitos del mismo nivel, esta última documentada
dentro de la propia especificación de cada requisito (Sección~\ref{sec:rf}
y siguientes) en lugar de una columna adicional. La matriz completa, con
sus 61 filas (\hyperref[tr:01]{TR-01} a \hyperref[tr:61]{TR-61}) y sus diez columnas, se presenta en el
\textbf{Apéndice D}.
...
las nueve filas de requisitos legales (\hyperref[tr:53]{TR-53} a \hyperref[tr:61]{TR-61}) derivan en cambio
directamente del texto de la LOPDP, en sentido inverso a las demás, tal
como se explica en el Apéndice~D. De las 61 filas del Apéndice~D (\hyperref[tr:01]{TR-01} a
\hyperref[tr:61]{TR-61}), 36 declaran una obligación legal explícita de la LOPDP con su
...
funcionales (\hyperref[tab:HU-01]{HU-01} a \hyperref[tab:HU-27]{HU-27}) más siete filas de requisitos no funcionales
...
real (\hyperref[tab:RNF-06]{RNF-06}, \hyperref[tab:RNF-07]{RNF-07}, \hyperref[tab:RNF-09]{RNF-09}, \hyperref[tab:RNF-13]{RNF-13}, \hyperref[tab:RNF-14]{RNF-14}, \hyperref[tab:RNF-15]{RNF-15} y \hyperref[tab:RNF-16]{RNF-16}, explicadas
...
CSV. \hyperref[tab:RF-26]{RF-26} y \hyperref[tab:RF-27]{RF-27}, incorporados en la presente revisión (4.0), quedan
trazados como \hyperref[tr:26]{TR-26} y \hyperref[tr:27]{TR-27}, con su evidencia correspondiente (EV-24 y
EV-23) e historia de usuario propia (\hyperref[tab:HU-25]{HU-25} y \hyperref[tab:HU-26]{HU-26}), aunque sin obligación
...
\begin{table}[H]
	...
	\begin{tabularx}{\textwidth}{@{}l X@{}}
		\toprule
		\textbf{Columna} & \textbf{Contenido y ejemplo (fila \hyperref[tr:01]{TR-01})} \\
		\midrule
		ID & Identificador de la fila --- \hyperref[tr:01]{TR-01} \\
		Fuente & Código de participante (Apéndice~B.1) u obligación legal de la que proviene el requisito, más los requisitos horizontales de los que también se deriva --- LOPDP, Art. 7 y 8; PROP-02, VET-01, VET-03, OBS-01; también \hyperref[tab:RF-12]{RF-12}, \hyperref[tab:RF-13]{RF-13} \\
		Requisito & Identificador RF, RNF, RD o RL de la fila --- \hyperref[tab:RF-01]{RF-01} \\
		Caso de Uso & Caso de uso que lo realiza --- \hyperref[tab:CU-01]{CU-01} \\
		...
		Criterio & Escenario Gherkin de la historia de usuario --- \hyperref[lst:ca01]{CA-01} \\
		Caso de Prueba & Ficha de caso de prueba de la Sección~\ref{sec:casos-prueba} --- \hyperref[tab:CP-01]{CP-01} \\
		Historia & Historia de usuario asociada --- \hyperref[tab:HU-01]{HU-01} \\
...
\end{table}
	...
	ejecutable, denominada \texttt{MVP\_MundiPets}, que materializa la
	totalidad de los requisitos funcionales definidos en la
	...
	implementó con un frontend en HTML5, CSS3 y JavaScript (sin frameworks ni
	proceso de compilación) servido por un backend en
	\href{https://expressjs.com/}{Express} sobre Node.js (\texttt{server.js}),
	que expone una API REST (\texttt{/api/...}) y persiste la información en
	una base de datos en memoria dentro del propio proceso de Node --sin motor
	de base de datos externo ni contenedores--, inicializada con un conjunto de
	datos ficticios en cada arranque del servidor. Las contraseñas de las
	cuentas de usuario se almacenan con hash \texttt{bcrypt} (biblioteca
	\texttt{bcryptjs}) y nunca se exponen en texto plano a través de la API. Se
	...
	lo mismo, el MVP todavía no satisface por completo las restricciones de
	arquitectura y de protección de datos declaradas en \hyperref[tab:RD-04]{RD-04}, \hyperref[tab:RD-05]{RD-05} y \hyperref[tab:RD-06]{RD-06}
	--en particular, la persistencia en memoria no sobrevive a un reinicio del
	servidor y el sistema no ofrece todavía un mecanismo de eliminación de
	cuenta--, restricciones que corresponden plenamente al sistema en
	producción y no a este prototipo.
	
	Según la priorización MoSCoW de
	...
	\hyperref[tab:RF-12]{RF-12}, se cubre en modo simulado: el registro y el inicio de sesión se
	realizan con credenciales reales validadas contra el backend (contraseña
	con hash \texttt{bcrypt} y política de seguridad exigida), pero el sistema
	no verifica la identidad del usuario contra un documento oficial, por lo
	que la verificación de identidad como tal permanece simulada. El MVP cubre
	además la totalidad de los requisitos \emph{Should} y \emph{Could}: de los
	12 requisitos de esas dos prioridades, cubre los 12 (100\,\%), incluyendo
	\hyperref[tab:RF-20]{RF-20} (\emph{Could}, coordinar encuentros de socialización supervisados,
	implementado en \texttt{encounters.html}). En conjunto, el MVP cubre
	\textbf{27 de los 27 RF vigentes (100\,\%)}.
	...
	\begin{longtable}{@{}p{1.5cm} p{5.2cm} p{1.7cm} p{3.0cm} p{3.4cm}@{}}
		...
		\hline
		\endfoot
		\hyperref[tab:RF-01]{RF-01} & Registrar mascota & Must & Cubierto & \texttt{pet-add.html} \\
		\hline
		\hyperref[tab:RF-02]{RF-02} & Gestionar historial médico de la mascota & Must & Cubierto & \texttt{pet-add.html}, \texttt{pet-profile.html} \\
		\hline
		\hyperref[tab:RF-03]{RF-03} & Consultar perfil completo de una mascota & Must & Cubierto & \texttt{pet-detail.html}, \texttt{pet-profile.html} \\
		\hline
		\hyperref[tab:RF-04]{RF-04} & Buscar y filtrar mascotas & Should & Cubierto & \texttt{explore.html} \\
		\hline
		\hyperref[tab:RF-05]{RF-05} & Gestionar solicitudes de adopción & Must & Cubierto & \texttt{request.html} \\
		\hline
		\hyperref[tab:RF-06]{RF-06} & Evaluar compatibilidad entre mascotas para cruza & Must & Cubierto (heurística explicable, no ML) & \texttt{compatibility.html} \\
		\hline
		\hyperref[tab:RF-07]{RF-07} & Sistema de mensajería entre usuarios & Could & Cubierto & \texttt{request.html} (panel de chat) \\
		\hline
		\hyperref[tab:RF-08]{RF-08} & Gestionar solicitudes de cruza & Must & Cubierto & \texttt{request.html} (tipo \emph{Cruza responsable}) \\
		\hline
		\hyperref[tab:RF-09]{RF-09} & Validar la información médica de una mascota & Must & Cubierto & \texttt{vet-panel.html} \\
		\hline
		\hyperref[tab:RF-10]{RF-10} & Gestionar recordatorios de controles preventivos & Should & Cubierto & \texttt{dashboard.html} (módulo de alertas y simulación de WhatsApp) \\
		\hline
		\hyperref[tab:RF-11]{RF-11} & Administrar la privacidad de la información & Must & Cubierto & \texttt{pet-profile.html} \\
		\hline
		\hyperref[tab:RF-12]{RF-12} & Verificar la identidad de los usuarios & Must & Cubierto (simulado) & \texttt{register.html}, \texttt{index.html} \\
		\hline
		\hyperref[tab:RF-13]{RF-13} & Registrar publicaciones de mascotas & Must & Cubierto & \texttt{pet-profile.html}, \texttt{explore.html} \\
		\hline
		\hyperref[tab:RF-14]{RF-14} & Gestionar carnet de vacunación & Must & Cubierto & \texttt{pet-add.html}, \texttt{pet-profile.html} \\
		\hline
		\hyperref[tab:RF-15]{RF-15} & Consultar el historial de procesos de adopción y cruza & Could & Cubierto & \texttt{dashboard.html} (sección expandible de solicitudes finalizadas) \\
		\hline
		\hyperref[tab:RF-16]{RF-16} & Gestionar antecedentes genéticos y parentesco de la mascota & Must & Cubierto & \texttt{pet-add.html}, \texttt{pet-profile.html}, \texttt{pet-detail.html} \\
		\hline
		\hyperref[tab:RF-17]{RF-17} & Validar imágenes & Should & Cubierto (simulado) & \texttt{pet-add.html} (analizador interactivo de IA en el paso 2 del asistente) \\
		\hline
		\hyperref[tab:RF-18]{RF-18} & Gestionar el flujo de solicitud de adopción por etapas & Must & Cubierto & \texttt{request.html} \\
		\hline
		\hyperref[tab:RF-19]{RF-19} & Validar certificados veterinarios antes de habilitar la cruza & Should & Cubierto & \texttt{pet-profile.html} (bloqueo de la opción de cruza sin certificado validado) \\
		\hline
		\hyperref[tab:RF-20]{RF-20} & Coordinar encuentros de socialización supervisados & Could & Cubierto & \texttt{encounters.html} \\
		\hline
		\hyperref[tab:RF-21]{RF-21} & Registrar el identificador de microchip de la mascota & Should & Cubierto & \texttt{pet-add.html}, \texttt{pet-profile.html}, \texttt{pet-detail.html}, \texttt{explore.html} \\
		\hline
		\hyperref[tab:RF-22]{RF-22} & Dar seguimiento post-adopción & Must & Cubierto & \texttt{post-adoption.html} \\
		\hline
		\hyperref[tab:RF-23]{RF-23} & Emitir alertas de riesgo sanitario o físico en interacciones y cruces & Must & Cubierto & \texttt{compatibility.html} \\
		\hline
		\hyperref[tab:RF-24]{RF-24} & Registrar trazabilidad de cepa, lote y aplicador de cada vacuna & Should & Cubierto & \texttt{pet-add.html}, \texttt{pet-profile.html} (campos del carnet referencial) \\
		\hline
		\hyperref[tab:RF-25]{RF-25} & Publicar aviso de mascota extraviada & Could & Cubierto & \texttt{pet-profile.html}, \texttt{explore.html} (alerta destacada y contacto directo) \\
		\hline
		\hyperref[tab:RF-26]{RF-26} & Habilitar validación médica por segunda opinión veterinaria & Should & Cubierto & \texttt{vet-panel.html}, \texttt{pet-profile.html} (módulo de doble validación y constancia) \\
		\hline
		\hyperref[tab:RF-27]{RF-27} & Controlar y alertar sobre solicitudes repetitivas de cruza de una misma mascota & Should & Cubierto & \texttt{compatibility.html} (alerta automática al superar el umbral semestral de cruzas) \\
	\end{longtable}
	
	\textbf{Resumen:} 27 de 27 RF cubiertos (100\,\% de cobertura total y
	100\,\% de los requisitos \emph{Debe tener}).
	...
	declarado, que el \hyperref[tab:RF-06]{RF-06} (evaluación de compatibilidad entre mascotas para
	...
	para la emisión de alertas de riesgo (\hyperref[tab:RF-23]{RF-23}). Por su naturaleza, esta
	heurística satisface de forma directa el requisito de explicabilidad
	\hyperref[tab:RNF-13]{RNF-13} y el derecho a explicación de \hyperref[rl:06]{RL-06}, ya que la pantalla de
	resultados muestra la regla concreta que originó cada advertencia; lo que
	no satisface todavía es el umbral de exactitud de \hyperref[tab:RNF-06]{RNF-06}, definido para el
	...
	El \hyperref[tab:RF-17]{RF-17} (validación de imágenes) cuenta con una representación funcional,
	...
	explicabilidad exigida por \hyperref[tab:RNF-14]{RNF-14} sin invocar un modelo de visión por
	computador real. De manera análoga, \hyperref[tab:RF-10]{RF-10} (recordatorios de controles
	...
	Sección~\ref{sec:interfaz-software}. Finalmente, \hyperref[tab:RF-27]{RF-27} (control de
	...
	La arquitectura del MVP se organiza en tres capas: presentación, backend de
	API y datos, distribuidas en catorce páginas HTML independientes --una por
	vista-- que comparten una hoja de estilos, un servidor Express y tres
	módulos de JavaScript en el cliente. La capa de presentación está
	compuesta por los archivos HTML y por \texttt{css/styles.css}, que define
	un diseño responsivo mediante CSS Grid y Flexbox con puntos de quiebre
	para dispositivos móviles, tabletas y escritorio, en coherencia con el
	requisito de portabilidad \hyperref[tab:RNF-11]{RNF-11}. La capa de backend, implementada en
	\texttt{server.js} sobre Express y Node.js, expone la API REST
	(\texttt{/api/...}) que atiende la autenticación y el resto de las
	operaciones del dominio; sobre ella, \texttt{js/auth.js} gestiona en el
	cliente el registro, el inicio de sesión, la recuperación de contraseña y
	el cambio de contraseña contra dicha API, cubriendo de forma real la
	verificación de credenciales exigida por \hyperref[tab:RF-12]{RF-12} --con contraseñas
	almacenadas mediante hash \texttt{bcrypt} y validadas en vivo contra una
	política de seguridad (mínimo 8 caracteres, mayúscula, minúscula, número y
	carácter especial)--, si bien no valida la identidad del usuario contra un
	documento oficial. La capa de datos, expuesta al cliente a través de
	\texttt{js/db.js} como un cliente de la API (basado en \texttt{fetch}) con
	la misma interfaz de operaciones \texttt{all}, \texttt{get},
	\texttt{insert}, \texttt{update} y \texttt{remove}, reside en el servidor
	como una base de datos en memoria del proceso Node, inicializada con el
	conjunto de datos ficticios en cada arranque y compartida entre todas las
	sesiones conectadas mientras el proceso permanece activo. Esta separación
	en capas responde a la restricción de arquitectura modular \hyperref[tab:RD-05]{RD-05} en su
	expresión mínima viable.
	...
	\begin{longtable}{@{}p{4.2cm} p{10.6cm}@{}}
		...
		\hline
		\endfoot
		\texttt{index.html} & Inicio de sesión mediante cuentas de demostración o credenciales propias; enlace para restablecer los datos de ejemplo. \\
		\hline
		\texttt{register.html} & Formulario de creación de cuenta con validación en vivo de la política de contraseñas y verificación de identidad simulada (\hyperref[tab:RF-12]{RF-12}). \\
		\hline
		\texttt{forgot-password.html} & Recuperación de contraseña en tres pasos (correo, código de verificación simulado y nueva contraseña), como apoyo a \hyperref[tab:RF-12]{RF-12}. \\
		\hline
		\texttt{profile.html} & Edición del perfil propio (nombre, correo, ciudad) y cambio de contraseña verificando la contraseña actual, como apoyo a \hyperref[tab:RF-12]{RF-12}. \\
		\hline
		\texttt{dashboard.html} & Panel de inicio diferenciado según el rol activo (Propietario, Adoptante, Interesado en cruza o Veterinaria); implementa el acceso basado en roles de \hyperref[tab:RD-02]{RD-02}, el módulo de alertas y simulación de recordatorios por WhatsApp (\hyperref[tab:RF-10]{RF-10}) y la sección expandible de historial de procesos finalizados (\hyperref[tab:RF-15]{RF-15}). \\
		\hline
		\texttt{pet-add.html} & Asistente de registro de mascota en cuatro pasos: datos generales, fotografía representativa con analizador interactivo de validación (\hyperref[tab:RF-17]{RF-17}), historial médico y carnet de vacunación con trazabilidad de cepa/lote/aplicador (\hyperref[tab:RF-14]{RF-14}, \hyperref[tab:RF-24]{RF-24}), y antecedentes genéticos (\hyperref[tab:RF-01]{RF-01}, \hyperref[tab:RF-02]{RF-02}, \hyperref[tab:RF-16]{RF-16}), incluyendo el identificador de microchip (\hyperref[tab:RF-21]{RF-21}). \\
		\hline
		\texttt{pet-profile.html} & Perfil propio de la mascota, con edición del historial médico, la publicación, la configuración de privacidad por campo sensible (\hyperref[tab:RF-03]{RF-03}, \hyperref[tab:RF-11]{RF-11}, \hyperref[tab:RF-13]{RF-13}), el bloqueo de la opción de cruza sin certificado veterinario validado (\hyperref[tab:RF-19]{RF-19}), la publicación de avisos de mascota extraviada (\hyperref[tab:RF-25]{RF-25}) y el módulo de doble validación por segunda opinión veterinaria (\hyperref[tab:RF-26]{RF-26}). \\
		\hline
		\texttt{explore.html} & Búsqueda y filtrado de mascotas publicadas por especie, estado de interés y ubicación aproximada (\hyperref[tab:RF-04]{RF-04}), incluyendo el identificador de microchip y los avisos de mascota extraviada en las tarjetas de resultado (\hyperref[tab:RF-21]{RF-21}, \hyperref[tab:RF-25]{RF-25}). \\
		\hline
		\texttt{pet-detail.html} & Perfil público de una mascota, con la información filtrada según el nivel de acceso del usuario consultante (\hyperref[tab:RF-03]{RF-03}, \hyperref[tab:RNF-16]{RNF-16}) y el identificador de microchip cuando corresponde (\hyperref[tab:RF-21]{RF-21}). \\
		\hline
		\texttt{request.html} & Formulario y seguimiento de solicitudes de adopción o cruza por etapas, con panel de mensajería asociado a cada solicitud (\hyperref[tab:RF-05]{RF-05}, \hyperref[tab:RF-07]{RF-07}, \hyperref[tab:RF-08]{RF-08}, \hyperref[tab:RF-18]{RF-18}). \\
		\hline
		\texttt{compatibility.html} & Evaluación de compatibilidad entre dos mascotas mediante una heurística de reglas explícitas, con emisión de alertas de riesgo (\hyperref[tab:RF-06]{RF-06}, \hyperref[tab:RF-23]{RF-23}, \hyperref[tab:RNF-13]{RNF-13}) y de la alerta automática por superación del umbral semestral de cruzas repetitivas (\hyperref[tab:RF-27]{RF-27}). \\
		\hline
		\texttt{vet-panel.html} & Panel de validación veterinaria de documentos e historiales médicos pendientes (\hyperref[tab:RF-09]{RF-09}, \hyperref[tab:RD-03]{RD-03}), incluyendo el flujo de segunda opinión sobre registros ya validados (\hyperref[tab:RF-26]{RF-26}). \\
		\hline
		\texttt{post-adoption.html} & Registro y consulta del seguimiento posterior a una adopción completada (\hyperref[tab:RF-22]{RF-22}). \\
		\hline
		\texttt{encounters.html} & Propuesta, confirmación o rechazo de encuentros de socialización supervisados entre mascotas, visibles en el historial de interacciones de ambas (\hyperref[tab:RF-20]{RF-20}). \\
		\hline
		\texttt{css/styles.css} & Hoja de estilos única con diseño responsivo mediante CSS Grid y Flexbox, y puntos de quiebre para dispositivos móviles, tabletas y escritorio (\hyperref[tab:RNF-11]{RNF-11}). \\
		\hline
		\texttt{server.js} & Servidor Express sobre Node.js: sirve el frontend estático, expone la API REST (\texttt{/api/...}) y mantiene la base de datos en memoria del proceso --usuarios, mascotas, historiales médicos, antecedentes genéticos, solicitudes, mensajes, evaluaciones de compatibilidad, seguimientos post-adopción y encuentros de socialización--, inicializada con el conjunto de datos ficticios de arranque y con las contraseñas de usuario almacenadas mediante hash \texttt{bcrypt}. \\
		\hline
		\texttt{package.json} & Declaración de dependencias (\texttt{express}, \texttt{bcryptjs}) y del script \texttt{start} para el despliegue local. \\
		\hline
		\texttt{js/db.js} & Cliente de la API REST (basado en \texttt{fetch}) que expone al resto del frontend la misma interfaz de operaciones \texttt{all}, \texttt{get}, \texttt{insert}, \texttt{update} y \texttt{remove} sobre las colecciones del dominio, delegando la persistencia real en \texttt{server.js}. \\
		\hline
		\texttt{js/auth.js} & Autenticación contra la API: inicio de sesión, registro, recuperación y cambio de contraseña, cubriendo de forma real la verificación de credenciales de \hyperref[tab:RF-12]{RF-12}. \\
		\hline
		\texttt{js/utils.js} & Funciones auxiliares compartidas entre pantallas: construcción de la barra superior según el rol, insignias de estado, notificaciones emergentes, validación en vivo de la política de contraseñas y alternancia de visibilidad de los campos de contraseña. \\
	\end{longtable}
	
	Al arrancar el servidor, la base de datos en memoria se inicializa
	automáticamente con un conjunto de datos ficticios de demostración: cuatro
	usuarios (un propietario, un adoptante, un interesado en cruza y una
	veterinaria) y cuatro mascotas (Firulais, Michi, Toby y Luna) con
	historiales médicos, antecedentes genéticos y una solicitud de adopción en
	curso, todos inspirados en los mockups de referencia presentados en la
	...
	reiniciar el estado de la demostración en cualquier momento sin necesidad
	de reiniciar el servidor. Al ser una base de datos en memoria, los datos
	también se reinician al conjunto semilla cada vez que el proceso de Node
	se reinicia, y se comparten entre todas las sesiones conectadas mientras
	el servidor permanece activo.
	
	El despliegue local del MVP requiere \href{https://nodejs.org/}{Node.js}
	(v18 o superior recomendado) y se levanta con un solo comando, sin
	necesidad de Docker ni de configurar una base de datos externa:
	\texttt{npm install} seguido de \texttt{npm start}. Luego se abre
	\texttt{http://localhost:3000} en el navegador; el servidor Express sirve
	tanto el frontend como la API, por lo que no hace falta levantar ningún
	servicio adicional por separado. El puerto puede modificarse de forma
	opcional mediante la variable de entorno \texttt{PORT} (por defecto
	\texttt{3000}).
	...
	\begin{enumerate}[leftmargin=*]
		...
		(\hyperref[tab:RF-01]{RF-01}, \hyperref[tab:RF-02]{RF-02}, \hyperref[tab:RF-11]{RF-11}, \hyperref[tab:RF-13]{RF-13}, \hyperref[tab:RF-14]{RF-14}, \hyperref[tab:RF-16]{RF-16}).
		...
		(\hyperref[tab:RF-03]{RF-03}, \hyperref[tab:RF-04]{RF-04}, \hyperref[tab:RF-05]{RF-05}, \hyperref[tab:RF-07]{RF-07}, \hyperref[tab:RF-12]{RF-12} en su forma simulada).
		...
		automáticamente (\hyperref[tab:RF-18]{RF-18}, \hyperref[tab:RF-22]{RF-22}).
		...
		pendiente, con opción de validarlo o rechazarlo (\hyperref[tab:RF-09]{RF-09}).
		...
		(\hyperref[tab:RF-06]{RF-06}, \hyperref[tab:RF-08]{RF-08}, \hyperref[tab:RF-23]{RF-23}).
	\end{enumerate}
	
	El recorrido anterior ejercita los requisitos \emph{Must}. Los requisitos
	de prioridad \emph{Should} se ejercitan mediante dos flujos adicionales,
	no numerados en la secuencia principal: en el panel de
	la Dra. Melissa Vera (\texttt{vet-panel.html}), la segunda opinión
	veterinaria de \hyperref[tab:RF-26]{RF-26} puede solicitarse sobre un registro ya validado; y al
	evaluar la compatibilidad de Toby (\texttt{compatibility.html}) como
	interesado en cruza, la alerta de uso repetitivo de \hyperref[tab:RF-27]{RF-27} se dispara de
	...
	reales, en coherencia con la restricción de anonimización \hyperref[tab:RD-09]{RD-09}.
	...
	\begin{figure}[H]
		...
		\caption{Pantalla de inicio de sesión del MVP (\texttt{index.html}). Se
			...
			Vera (veterinaria) y Jorge Intriago (interesado en cruza) ---, cuyo acceso
			se valida contra la API con contraseñas reales almacenadas mediante hash
			\texttt{bcrypt}, junto con el enlace de restablecimiento de los datos de
			ejemplo. La verificación de identidad de \hyperref[tab:RF-12]{RF-12} permanece simulada en
			cuanto no valida un documento oficial del usuario.}
		\label{fig:mvp-login}
	\end{figure}
	...
	\textbf{Justificación en función del sistema.} MundiPets cuenta con un
	corpus consolidado de especificación (27 RF, 16 RNF, 9 restricciones de
	diseño y 9 obligaciones legales), que constituye el insumo directo sobre
	el cual se ejecuta el detector de ambigüedad, sin depender de instrumentos
	adicionales de recolección con usuarios externos al proyecto.

	\textbf{Justificación en función de los datos disponibles.} Como parte de
	la revisión y el refinamiento del ERS/SRS, el equipo efectuó un ejercicio
	de identificación manual de defectos de redacción sobre el conjunto de
	requisitos, detectando patrones de ambigüedad, terminología imprecisa y
	requisitos no verificables consistentes con los \emph{malos olores}
	descritos por Ferrari \emph{et al.} \cite{ferrari2016}. Este antecedente
	ofrece una base real sobre la cual diseñar y calibrar el detector
	automático, en lugar de partir desde cero.
	...
	\begin{table}[H]
		...
		\begin{tabularx}{\textwidth}{@{}p{2.6cm} X@{}}
			...
			(RNF), restricciones de diseño (RD) y obligaciones legales (RL)
			especificados para el sistema MundiPets, redactados según la
			plantilla de atributos exigida por la asignatura y vigentes al
			momento de la ejecución del experimento. La población ejecutada
			es de \textbf{61 requisitos} (27 RF, 16 RNF, 9 RD y 9 RL),
			correspondientes al corpus consolidado en la versión final del
			ERS/SRS (Sección \ref{sec:rf}). \\
			...
			tres; significancia de la asociación detector--consenso. \\
	...
	clasificación de consenso de las personas expertas.
	...
	Esta hipótesis se contrasta mediante una prueba de chi-cuadrado de
	independencia de Pearson (con corrección de continuidad) sobre la tabla
	de contingencia 2$\times$2 detector--consenso, descrita en la Sección
	\ref{sec:plan-analisis}. El chi-cuadrado de independencia es la prueba
	estándar para contrastar asociación entre dos variables categóricas
	binarias medidas sobre las mismas unidades --el diseño exacto de este
	estudio-- y permite reportar directamente el tamaño del efecto como
	coeficiente $\phi$ (phi), interpretable en la misma escala que una
	correlación. La pregunta RQ2, al ser de carácter exploratorio sobre los
	patrones de desacuerdo, no se somete a prueba de hipótesis: se responde
	mediante análisis cualitativo de los casos en los que el detector y el
	consenso experto difieren, siguiendo el sexto paso del Enfoque 2.
	...
	\begin{itemize}[leftmargin=*]
		\item \textbf{Tipo de requisito} (RF, RNF, RD o RL), para verificar que el
	...
	mujeres y un hombre), identificados en el material publicado únicamente por
	código (\textbf{EXP-01}, \textbf{EXP-02}, \textbf{EXP-03}) para proteger su
	identidad; la correspondencia entre código e identidad real se conserva
	exclusivamente en un anexo de uso interno del equipo, fuera de la zona
	...
	MundiPets; (b) haber participado en la redacción de los RF, RNF, RD o RL que
	conforman la población del estudio (Sección \ref{sec:rq}).
	
	\textbf{Proceso de consentimiento informado.} Cada persona evaluadora firmó,
	el 28 de julio de 2026, un consentimiento informado específico para este rol,
	redactado en cumplimiento de la Ley Orgánica de Protección de Datos
	Personales del Ecuador \cite{lopdp2021}, que autoriza expresamente su
	participación como evaluadora experta y el uso de la clasificación
	producida --de forma anonimizada-- en publicaciones científicas revisadas
	por pares. La plantilla se detalla en la Sección \ref{sec:instrumentos}.
	...
	\begin{enumerate}[leftmargin=*]
		...
		ambigüedad, y una hoja de evaluación por persona (\texttt{Eval.
		EXP-01/02/03}) en la que cada evaluadora registra, para cada requisito,
		su identificador anónimo, el texto completo, la clasificación
		(\emph{ambiguo} / \emph{no ambiguo}), el tipo de mal olor detectado, una
		justificación breve y su nivel de confianza (escala 1--5). Los
		requisitos se presentan sin indicar su tipo (RF, RNF, RD o RL) y en
		orden aleatorizado, para reducir sesgos de expectativa. Una hoja
		adicional (\texttt{Clave Interna}), de uso exclusivo del equipo,
		conserva la clave que vincula cada identificador anónimo con el
		requisito real y la salida del detector.
		\item \textbf{Consentimientos firmados por las personas evaluadoras
		expertas} (\texttt{Consentimiento\_EXP-01\_Experto.png},
		\texttt{Consentimiento\_EXP-02\_Experto.png} y
		\texttt{Consentimiento\_EXP-03\_Experto.png}): adaptan el formato
		de consentimiento utilizado para las entrevistas de campo, ajustado al
		rol de evaluador experto e incorporando la autorización explícita
		para el uso de datos anonimizados en publicaciones revisadas por
		pares, conforme a la LOPDP \cite{lopdp2021}.
	\end{enumerate}
	
\subsection{Plan de análisis estadístico ejecutado}
\label{sec:plan-analisis}

\textbf{Análisis principal (RQ1, H\textsubscript{0}/H\textsubscript{1}).}
Se calcularon precisión, exhaustividad y medida F1 del detector automático
tomando como referencia el consenso experto (Sección \ref{sec:hipotesis}).
La hipótesis se contrastó mediante chi-cuadrado de independencia de Pearson
con corrección de continuidad (Sección \ref{sec:hipotesis}), y la magnitud
del acuerdo se interpretó con la escala de Landis y Koch: 0,00--0,20 leve;
...
\textbf{Análisis secundario (RQ2, exploratorio).} RQ2 se respondió de forma
cualitativa mediante el análisis de los tipos de mal olor predominantes en
los casos de desacuerdo entre el detector y el consenso experto
(Sección \ref{sec:resultados}), tal como permite el sexto paso del
Enfoque 2. El análisis cuantitativo complementario --comparación del nivel
de confianza reportado por las personas expertas (escala 1--5, registrado
en la rúbrica de evaluación, Sección \ref{sec:instrumentos}) entre casos de
acuerdo y desacuerdo, mediante prueba de Shapiro-Wilk seguida de $t$ de
Student o $U$ de Mann-Whitney, con corrección de Benjamini-Hochberg para
comparaciones por tipo de requisito-- queda declarado como línea de trabajo
futuro (Sección \ref{sec:futuro}) sobre el mismo conjunto de datos ya
depositado.

\textbf{Cálculo de potencia estadística.} Conforme a Molléri \emph{et al.}
\cite{molleri2020} y Wohlin \emph{et al.} \cite{wohlin2012}, se fijó
$\alpha = 0{,}05$ y $1 - \beta = 0{,}80$. El cálculo de potencia se reportó
en sentido inverso sobre el $N$ real disponible (61), mediante remuestreo
bootstrap (10\,000 réplicas, semilla fija = 42) para el intervalo de
confianza al 95\,\% de cada métrica (Sección \ref{sec:resultados}).

\textbf{Reproducibilidad.} Todos los cálculos se ejecutan mediante los
scripts versionados en \texttt{06\_Experimento/scripts\_analisis/} (Python:
\texttt{pandas}, \texttt{scipy}, \texttt{numpy}), orquestados por un único
punto de entrada (\texttt{run\_all.py}). Ninguna cifra o tabla de esta
sección se produjo de forma manual; se verificó la reproducibilidad
completa ejecutando el \emph{pipeline} sobre un clon limpio del
repositorio, partiendo únicamente de \texttt{datos\_crudos/}.

\subsection{Registro previo del protocolo en el OSF y desviaciones}
\label{sec:osf}
...
Toda diferencia entre lo registrado y lo efectivamente ejecutado se
documenta de forma explícita, en cumplimiento de la exigencia de la guía de
la asignatura de que ninguna desviación quede sin registrar:

\begin{enumerate}[leftmargin=*]
	\item \textbf{Tamaño y composición de la población}: el corpus ejecutado
	es de 61 requisitos (27 RF, 16 RNF, 9 RD y 9 RL), con la incorporación de
	la categoría RL, no contemplada en el registro original
	(Sección \ref{sec:rq}).
	\item \textbf{Prueba de significancia estadística}: el contraste de
	H\textsubscript{0}/H\textsubscript{1} se realiza mediante chi-cuadrado de
	independencia de Pearson con corrección de continuidad, en lugar de la
	prueba de significancia del $\kappa$ de Cohen prevista originalmente, por
	ser la prueba estándar para contrastar asociación entre dos variables
	categóricas binarias medidas sobre las mismas unidades
	(Sección \ref{sec:hipotesis}).
	\item \textbf{Análisis secundario de RQ2}: el componente cuantitativo
	(Shapiro-Wilk, prueba de comparación de grupos, corrección
	Benjamini-Hochberg) no se ejecutó dentro del plazo disponible; RQ2 se
	respondió mediante análisis cualitativo de los tipos de mal olor
	predominantes (Sección \ref{sec:resultados}) y el componente cuantitativo
	queda declarado como trabajo futuro (Sección \ref{sec:futuro}).
\end{enumerate}

Ninguna de las tres desviaciones alteró la variable dependiente, el diseño
ciego, ni el conjunto de reglas fijas del detector; todas se aplicaron de
forma completa sobre el corpus final, no de manera selectiva sobre
...
\begin{itemize}[leftmargin=*]
	\item \emph{Sesgo de diseño del detector.} El equipo que construyó el
	detector ya conocía, por la revisión manual previa realizada durante
	el refinamiento del ERS/SRS (Sección \ref{sec:enfoque}), algunos
	de los defectos presentes en \hyperref[tab:RF-01]{RF-01} a \hyperref[tab:RF-17]{RF-17}, lo que podría calibrar
	las reglas hacia esos casos conocidos e inflar artificialmente la
	precisión. \textbf{Mitigación:}
	las reglas del detector (Sección \ref{sec:enfoque}) se fijaron y
	documentaron antes de recibir la clasificación completa de los tres
	expertos, y se aplicaron sin modificación posterior
	...
	parcialmente al incluir en la población los \hyperref[tab:RF-18]{RF-18} a \hyperref[tab:RF-27]{RF-27} y las
	obligaciones legales (RL), que no formaron parte de esa revisión previa.
...
\begin{itemize}[leftmargin=*]
	\item \emph{Tamaño de muestra.} El corpus final ejecutado (N=61)
	supera el mínimo de 50 requisitos sugerido por la guía de la
	asignatura para este enfoque, lo que mejoró sustancialmente la
	potencia estadística respecto de la población inicialmente prevista
	(Sección \ref{sec:osf}). \textbf{Resultado observado:} con este $N$,
	la asociación detector--consenso resultó estadísticamente
	significativa ($p=0{,}0001$; Sección \ref{sec:resultados}). El
	cálculo de potencia en sentido inverso sobre el $N=61$ real
	disponible ($\alpha=0{,}05$, $1-\beta=0{,}80$) muestra que este
	diseño alcanzó una potencia de 0,9724 para el tamaño del efecto
	efectivamente observado ($\phi=0{,}4965$), y que el efecto mínimo
	detectable con este $N$ a una potencia de 0,80 es $\phi \geq 0{,}3587$;
	de haberse buscado detectar un efecto pequeño--moderado ($\phi=0{,}30$,
	Cohen) con la misma potencia, habría hecho falta $N=87$.
	\textbf{Limitación reconocida:} el diseño no tenía potencia
	suficiente para detectar de forma confiable efectos menores a
	$\phi=0{,}3587$; el $N=61$ no fue elegido libremente, sino impuesto
	por el tamaño real del corpus de especificación del sistema.
	\item \emph{Panel experto mínimo.} Solo tres personas conforman el
	consenso de referencia, el mínimo aceptado por la guía. \textbf{Mitigación:}
	se calculó explícitamente el acuerdo inter-evaluador mediante $\kappa$
	de Fleiss antes de aceptar el consenso como válido (Sección
	\ref{sec:resultados}). \textbf{Limitación reconocida:} un panel de
	tres es más vulnerable a la idiosincrasia individual que un panel de
	cinco.
\end{itemize}
...
\subsubsection{Clasificación por fuente}

\begin{table}[H]
	\centering
	\caption{Clasificación de ambigüedad por fuente (N=61).}
	\label{tab:clasificacion-fuente}
	\begin{tabular}{@{}lrrr@{}}
		\toprule
		\textbf{Fuente} & \textbf{N} & \textbf{Ambiguos (n)} & \textbf{Ambiguos (\%)} \\
		\midrule
		EXP-01 & 61 & 16 & 26,23 \\
		EXP-02 & 61 & 13 & 21,31 \\
		EXP-03 & 61 & 18 & 29,51 \\
...
\subsubsection{Acuerdo inter-evaluador}

\begin{table}[H]
	\centering
	\caption{Acuerdo inter-evaluador ($\kappa$ de Cohen por par, $\kappa$ de Fleiss para el panel).}
	\label{tab:acuerdo-interevaluador}
	\begin{tabular}{@{}lrrl@{}}
		\toprule
		\textbf{Comparación} & \textbf{$\kappa$} & \textbf{Acuerdo obs.} & \textbf{Interpretación} \\
		\midrule
		...
		\textbf{Panel completo ($\kappa$ de Fleiss)} & \textbf{0,8569} & \textbf{0,9454} & \textbf{Casi perfecto} \\
		\bottomrule
	\end{tabular}
\end{table}

El acuerdo casi perfecto obtenido en el panel completo respalda la validez
del consenso experto como referencia para evaluar el detector automático.

\subsubsection{Desempeño del detector y prueba de hipótesis}

\begin{table}[H]
	\centering
	\caption{Matriz de confusión: detector vs. consenso experto (N=61).}
	\label{tab:confusion-detector}
	\begin{tabular}{@{}lrr@{}}
		\toprule
		\textbf{Consenso experto} & \textbf{Detector: No ambiguo} & \textbf{Detector: Ambiguo} \\
		\midrule
		No ambiguo & 37 & 8 \\
		Ambiguo & 4 & 12 \\
		\bottomrule
...
\begin{table}[H]
	\centering
	\caption{Métricas del detector automático con intervalo de confianza al 95\,\% (bootstrap, 10\,000 réplicas).}
	\label{tab:metricas-detector}
	\begin{tabular}{@{}lrl@{}}
		\toprule
		...
		$\kappa$ detector--consenso & 0,5296 (moderado) & [0,2797; 0,7369] \\
		\bottomrule
	\end{tabular}
\end{table}

\textbf{Prueba de hipótesis:} $\chi^2(1) = 15{,}0374$; $p = 0{,}0001$;
$\phi = 0{,}4965$ (tamaño del efecto moderado--grande).

Con $p < 0{,}05$, se \textbf{rechaza H\textsubscript{0}}: la clasificación
del detector automático guarda asociación estadísticamente significativa
con el consenso experto, con un tamaño del efecto moderado--grande.

\begin{table}[H]
	\centering
	\caption{Métricas del detector desglosadas por tipo de requisito, con intervalo de confianza al 95\,\% (bootstrap, 10\,000 réplicas).}
	\label{tab:metricas-por-categoria}
	\small
	\begin{tabular}{@{}lrlrlrl@{}}
		\toprule
		\textbf{Tipo} & \textbf{Exactitud} & \textbf{IC 95\,\%} & \textbf{Precisión} & \textbf{IC 95\,\%} & \textbf{F1} & \textbf{IC 95\,\%} \\
		\midrule
		RF (N=27) & 0,7778 & [0,5926; 0,9259] & 0,5000 & [0,1429; 0,8571] & 0,5714 & [0,2222; 0,8421] \\
		RNF (N=16) & 0,8750 & [0,6875; 1,0000] & 0,3333 & [0,0000; 1,0000] & 0,5000 & [0,3333; 1,0000] \\
		RD (N=9) & 0,5556 & [0,2222; 0,8889] & 0,5000 & [0,0000; 1,0000] & 0,5000 & [0,2500; 0,8571] \\
		RL (N=9) & 1,0000 & [1,0000; 1,0000] & 1,0000 & [1,0000; 1,0000] & 1,0000 & [1,0000; 1,0000] \\
		\bottomrule
	\end{tabular}
\end{table}

Los intervalos por categoría son más anchos que el intervalo agregado
(Tabla~\ref{tab:metricas-detector}, N=61) por el menor tamaño de muestra
en cada grupo (RD=9, RL=9, RNF=16, RF=27). No se reporta la prueba de
chi-cuadrado por categoría porque el tamaño esperado mínimo de celda
($\geq$5) no se cumple en RD ni en RL con este N.

\subsubsection{Tipos de ambigüedad predominantes (RQ2)}

\begin{table}[H]
	\centering
	\caption{Distribución de tipos de mal olor sobre las marcaciones ambiguas.}
	\label{tab:tipos-ambiguedad}
	\begin{tabular}{@{}lrr@{}}
		\toprule
		\textbf{Tipo de ambigüedad} & \textbf{Frecuencia} & \textbf{\%} \\
		\midrule
...
El tipo de ambigüedad predominante en los casos marcados por el panel
experto es la terminología imprecisa, seguida de cerca por los
cuantificadores vagos --el tipo de mal olor que el detector basado en
reglas está diseñado para capturar de forma más directa--, lo que es
consistente con la sensibilidad de 0,75 obtenida.

\subsection{Síntesis de respuesta a las preguntas de investigación}
\label{sec:sintesis}

\textbf{RQ1.} El detector automático basado en reglas replica el juicio
consensuado de las personas expertas con una exactitud de 0,8033, una
medida F1 de 0,6667 y un acuerdo moderado ($\kappa=0{,}5296$) con el
consenso experto, una asociación estadísticamente significativa
($\chi^2(1)=15{,}0374$; $p=0{,}0001$; $\phi=0{,}4965$).

\textbf{RQ2.} Los desacuerdos entre el detector y el consenso experto se
concentran en requisitos donde predomina la terminología imprecisa y la
ausencia de criterio de verificación explícito --formas de ambigüedad
semántica que requieren juicio de dominio-- frente a los cuantificadores
vagos, que el detector identifica de forma más directa mediante reglas
léxicas.

\subsection{Trabajo futuro declarado}
\label{sec:futuro}

El análisis cuantitativo secundario de RQ2 (comparación del nivel de
confianza entre casos de acuerdo y desacuerdo, con las pruebas y
correcciones descritas en la Sección \ref{sec:plan-analisis}) queda
pendiente de ejecución y se declara explícitamente como línea de trabajo
futuro sobre el mismo conjunto de datos ya depositado.

Todos los valores numéricos de esta sección provienen directamente de
\texttt{06\_Experimento/resultados/tablas/}, generados por los scripts
versionados en \texttt{06\_Experimento/scripts\_analisis/} a partir de
\texttt{datos\_crudos/}. Ningún valor fue calculado, redondeado o
transcrito manualmente para este documento.
	...
	entrega (\hyperref[tab:RF-18]{RF-18} a \hyperref[tab:RF-25]{RF-25}) no surgieron de una revisión de escritorio sino de
	...
	directa de los tres requisitos de explicabilidad (\hyperref[tab:RNF-13]{RNF-13}, \hyperref[tab:RNF-14]{RNF-14} y \hyperref[tab:RNF-15]{RNF-15}),
	...
	(\hyperref[tab:RF-18]{RF-18}, \hyperref[tab:RF-22]{RF-22} y \hyperref[tab:RF-23]{RF-23}) no cuentan con una pantalla de alta fidelidad
	...
	resultados obtenidos. Esta decisión metodológica es la que permite
	reportar los hallazgos sin sospecha de ajuste posterior de las
	hipótesis. El plan de análisis estadístico se ejecuta
	...
	\begin{table}[H]
		...
		\begin{tabularx}{\linewidth}{|C{2.5cm}|X|C{2.8cm}|}
			...
			\hline
			Semana 12 (20--26 jul.) & Redacción de los \hyperref[tab:RF-18]{RF-18} a \hyperref[tab:RF-25]{RF-25} y \hyperref[tab:RNF-09]{RNF-09} a \hyperref[tab:RNF-15]{RNF-15} a partir de la codificación temática, mapeo legal LOPDP, construcción del diagrama de Razón Estratégica (SR) y refinamiento del diagrama de clases. & Completado \\
			\hline
			...
			\hline
			Semanas 14--16 (03--23 ago.) & Ejecución del componente empírico, desarrollo del MVP (27 de 27 RF cubiertos), segunda ronda de \emph{walkthrough} con PROP-12 y VET-07 (EV-23, EV-24), revisión integral del documento. & Completado \\
			\hline
			Semana 17 (24--30 ago.) & Corrección de calidad de los requisitos funcionales conforme a ISO/IEC/IEEE 29148:2018, incorporación de \hyperref[tab:RF-26]{RF-26} y \hyperref[tab:RF-27]{RF-27} a partir de la nueva evidencia de campo, actualización de la matriz de trazabilidad extendida (61 filas, incluidos los 9 requisitos legales) y cierre del documento ERS/SRS. & Completado \\
			\hline
		\end{tabularx}
	\end{table}
	...
	Se mantiene la entrevista estructurada como técnica
	...
	amplía el cuestionario aplicado (versión 2.0) con las preguntas
	...
	(Apéndice~B.8), que es el insumo directo para derivar ocho requisitos
	funcionales (\hyperref[tab:RF-18]{RF-18} a \hyperref[tab:RF-25]{RF-25}). Dos sesiones adicionales combinan entrevista
	y \emph{walkthrough} sobre el mismo participante,
	...
	Apéndice~B.7). Estas dos evidencias son el origen directo de \hyperref[tab:RF-27]{RF-27} y
	\hyperref[tab:RF-26]{RF-26}, respectivamente.
	...
	\hyperref[tab:RD-09]{RD-09} y de la Ley Orgánica de Protección de Datos Personales del Ecuador
	...
	{\small
		\begin{longtable}{@{}>{\raggedright\arraybackslash}p{1.2cm} >{\raggedright\arraybackslash}p{3.1cm} >{\raggedright\arraybackslash}p{3.1cm} >{\raggedright\arraybackslash}p{6.0cm} >{\raggedright\arraybackslash}p{1.4cm}@{}}
			...
			\bottomrule
			\endlastfoot
			EV-01 & Entrevista grabada, audio, video y consentimiento & PROP-01 (propietario de mascota) & \hyperref[tab:RF-01]{RF-01}, \hyperref[tab:RF-03]{RF-03}, \hyperref[tab:RF-05]{RF-05}, \hyperref[tab:RF-07]{RF-07}, \hyperref[tab:RF-08]{RF-08}, \hyperref[tab:RF-12]{RF-12}, \hyperref[tab:RF-13]{RF-13} & Sí \\
			\addlinespace
			EV-02 & Entrevista grabada, audio, video y consentimiento & PROP-02 (propietaria de mascota) & \hyperref[tab:RF-01]{RF-01}, \hyperref[tab:RF-02]{RF-02}, \hyperref[tab:RF-03]{RF-03}, \hyperref[tab:RF-05]{RF-05}, \hyperref[tab:RF-07]{RF-07}, \hyperref[tab:RF-08]{RF-08}, \hyperref[tab:RF-10]{RF-10}, \hyperref[tab:RF-11]{RF-11}, \hyperref[tab:RF-13]{RF-13}, \hyperref[tab:RF-14]{RF-14}, \hyperref[tab:RF-15]{RF-15} & Sí \\
			\addlinespace
			EV-03 & Entrevista grabada, audio, video y consentimiento & VET-01 (médica veterinaria) & \hyperref[tab:RF-01]{RF-01}, \hyperref[tab:RF-02]{RF-02}, \hyperref[tab:RF-05]{RF-05}, \hyperref[tab:RF-06]{RF-06}, \hyperref[tab:RF-08]{RF-08}, \hyperref[tab:RF-09]{RF-09}, \hyperref[tab:RF-12]{RF-12}, \hyperref[tab:RF-14]{RF-14}, \hyperref[tab:RF-16]{RF-16}, \hyperref[tab:RF-17]{RF-17}, \hyperref[tab:RNF-01]{RNF-01}, \hyperref[tab:RNF-05]{RNF-05}, \hyperref[tab:RNF-06]{RNF-06}, \hyperref[tab:RNF-07]{RNF-07}, \hyperref[tab:RD-03]{RD-03}, \hyperref[tab:RD-04]{RD-04} & Sí \\
			\addlinespace
			EV-04 & Entrevista grabada, audio, video y consentimiento & VET-02 (veterinario) & \hyperref[tab:RF-02]{RF-02}, \hyperref[tab:RF-03]{RF-03}, \hyperref[tab:RF-06]{RF-06}, \hyperref[tab:RF-10]{RF-10}, \hyperref[tab:RF-13]{RF-13}, \hyperref[tab:RF-15]{RF-15}, \hyperref[tab:RNF-06]{RNF-06}, \hyperref[tab:RNF-08]{RNF-08}, \hyperref[tab:RD-03]{RD-03} & Sí \\
			\addlinespace
			EV-05 & Entrevista grabada, audio, video y consentimiento & VET-03 (médico veterinario) & \hyperref[tab:RF-01]{RF-01}, \hyperref[tab:RF-02]{RF-02}, \hyperref[tab:RF-03]{RF-03}, \hyperref[tab:RF-05]{RF-05}, \hyperref[tab:RF-06]{RF-06}, \hyperref[tab:RF-08]{RF-08}, \hyperref[tab:RF-09]{RF-09}, \hyperref[tab:RF-11]{RF-11}, \hyperref[tab:RF-13]{RF-13}, \hyperref[tab:RF-14]{RF-14}, \hyperref[tab:RF-16]{RF-16}, \hyperref[tab:RF-17]{RF-17}, \hyperref[tab:RNF-01]{RNF-01}, \hyperref[tab:RNF-05]{RNF-05}, \hyperref[tab:RD-03]{RD-03}, \hyperref[tab:RD-04]{RD-04} & Sí \\
			\addlinespace
			EV-06 & Entrevista grabada, audio, video y consentimiento & PROP-03 (propietaria de mascota) & \hyperref[tab:RF-01]{RF-01}, \hyperref[tab:RF-02]{RF-02}, \hyperref[tab:RF-03]{RF-03}, \hyperref[tab:RF-05]{RF-05}, \hyperref[tab:RF-08]{RF-08}, \hyperref[tab:RF-10]{RF-10}, \hyperref[tab:RF-11]{RF-11}, \hyperref[tab:RF-13]{RF-13}, \hyperref[tab:RF-14]{RF-14}, \hyperref[tab:RF-15]{RF-15} & Sí \\
			\addlinespace
			EV-07 & Cuestionario digital, formularios y resultados exportados & Usuarios potenciales encuestados & \hyperref[tab:RF-01]{RF-01}, \hyperref[tab:RF-03]{RF-03}, \hyperref[tab:RF-04]{RF-04}, \hyperref[tab:RF-06]{RF-06}, \hyperref[tab:RF-07]{RF-07}, \hyperref[tab:RF-08]{RF-08}, \hyperref[tab:RF-11]{RF-11}, \hyperref[tab:RF-12]{RF-12}, \hyperref[tab:RF-13]{RF-13}, \hyperref[tab:RF-17]{RF-17}, \hyperref[tab:RNF-03]{RNF-03}, \hyperref[tab:RNF-04]{RNF-04}, \hyperref[tab:RNF-07]{RNF-07} & No \\
			\addlinespace
			EV-08 & Observación presencial del proceso actual en una clínica veterinaria, con fotografías fechadas del entorno y consentimiento informado firmado por la veterinaria observada & OBS-01 (establecimiento veterinario) & \hyperref[tab:RF-01]{RF-01}, \hyperref[tab:RF-03]{RF-03}, \hyperref[tab:RF-04]{RF-04}, \hyperref[tab:RF-05]{RF-05}, \hyperref[tab:RF-06]{RF-06}, \hyperref[tab:RF-07]{RF-07}, \hyperref[tab:RF-11]{RF-11}, \hyperref[tab:RF-12]{RF-12}, \hyperref[tab:RF-13]{RF-13}, \hyperref[tab:RF-17]{RF-17}, \hyperref[tab:RNF-01]{RNF-01}, \hyperref[tab:RNF-04]{RNF-04} & Si \\
			\addlinespace
			EV-09 & Entrevista grabada, audio, video y consentimiento & PROP-04 (propietario de mascota) & \hyperref[tab:RF-11]{RF-11}, \hyperref[tab:RF-12]{RF-12}, \hyperref[tab:RF-23]{RF-23}, \hyperref[tab:RNF-09]{RNF-09} & Si \\
			\addlinespace
			EV-10 & Entrevista grabada, audio, video y consentimiento & PROP-05 (propietario de mascota) & \hyperref[tab:RF-11]{RF-11}, \hyperref[tab:RF-14]{RF-14}, \hyperref[tab:RF-22]{RF-22} & Si \\
			\addlinespace
			EV-11 & Entrevista grabada, audio, video y consentimiento & PROP-06 (propietaria de mascota) & \hyperref[tab:RF-06]{RF-06}, \hyperref[tab:RF-10]{RF-10}, \hyperref[tab:RF-18]{RF-18}, \hyperref[tab:RF-19]{RF-19}, \hyperref[tab:RNF-13]{RNF-13} & Sí \\
			\addlinespace
			EV-12 & Entrevista grabada, audio, video y consentimiento & PROP-07 (propietaria de mascota) & \hyperref[tab:RF-23]{RF-23}, \hyperref[tab:RF-25]{RF-25} & Sí \\
			\addlinespace
			EV-13 & Entrevista grabada, audio, video y consentimiento & PROP-08 (propietario de mascota) & \hyperref[tab:RF-06]{RF-06}, \hyperref[tab:RF-20]{RF-20}, \hyperref[tab:RNF-13]{RNF-13} & Sí \\
			\addlinespace
			EV-14 & Entrevista grabada, audio, video y consentimiento & PROP-09 (propietario de mascota) & \hyperref[tab:RF-03]{RF-03}, \hyperref[tab:RF-06]{RF-06}, \hyperref[tab:RF-11]{RF-11}, \hyperref[tab:RF-16]{RF-16} & Sí \\
			\addlinespace
			EV-15 & Entrevista grabada, audio, video y consentimiento & VET-04 (veterinaria) & \hyperref[tab:RF-02]{RF-02}, \hyperref[tab:RF-09]{RF-09}, \hyperref[tab:RF-16]{RF-16}, \hyperref[tab:RF-23]{RF-23}, \hyperref[tab:RF-24]{RF-24}, \hyperref[tab:RNF-13]{RNF-13} & Sí \\
			\addlinespace
			EV-16 & Entrevista grabada, audio, video y consentimiento & VET-05 (veterinario) & \hyperref[tab:RF-02]{RF-02}, \hyperref[tab:RF-09]{RF-09}, \hyperref[tab:RF-14]{RF-14}, \hyperref[tab:RF-18]{RF-18}, \hyperref[tab:RF-21]{RF-21}, \hyperref[tab:RF-22]{RF-22}, \hyperref[tab:RF-24]{RF-24}, \hyperref[tab:RNF-13]{RNF-13} & Sí \\
			\addlinespace
			EV-17 & Entrevista grabada, audio, video y consentimiento & PROP-10 (propietaria de mascota) & Refuerza \hyperref[tab:RNF-13]{RNF-13} & Sí \\
			\addlinespace
			EV-18 & Entrevista grabada, audio, video y consentimiento & PROP-11 (propietaria de mascota) & \hyperref[tab:RF-11]{RF-11}, \hyperref[tab:RF-23]{RF-23}, \hyperref[tab:RNF-09]{RNF-09}, \hyperref[tab:RD-04]{RD-04} & Sí \\
			\addlinespace
			EV-19 & Sesión de validación (\emph{walkthrough}) grabada en video & PROP-08 (propietario de mascota, usuario no técnico) & Validación de \hyperref[tab:RF-01]{RF-01} a \hyperref[tab:RF-17]{RF-17} & Sí \\
			\addlinespace
			EV-20 & Sesión de validación (\emph{walkthrough}) grabada en video & PROP-02 (propietaria de mascota, usuaria no técnica) & Validación de \hyperref[tab:RF-01]{RF-01} a \hyperref[tab:RF-08]{RF-08}, \hyperref[tab:RF-11]{RF-11} a \hyperref[tab:RF-17]{RF-17} & Sí \\
			\addlinespace
			EV-21 & Sesión de validación (\emph{walkthrough}) grabada en video, con consentimiento & VET-04 (veterinaria, usuaria técnica) & Validación de \hyperref[tab:RF-01]{RF-01} a \hyperref[tab:RF-08]{RF-08}, \hyperref[tab:RF-11]{RF-11} a \hyperref[tab:RF-17]{RF-17} & Sí \\
			\addlinespace
			EV-22 & Sesión de validación (\emph{walkthrough}) grabada en video, con consentimiento & VET-06 (veterinario, usuario técnico) & Validación de \hyperref[tab:RF-01]{RF-01} a \hyperref[tab:RF-17]{RF-17} & Sí \\
			\addlinespace
			EV-23 & Entrevista grabada, audio, video, consentimiento, y sesión de validación (\emph{walkthrough}) grabada en video & PROP-12 (propietaria de mascota, usuaria no técnica) & Entrevista: refuerza \hyperref[tab:RF-01]{RF-01} a \hyperref[tab:RF-16]{RF-16}; validación de \hyperref[tab:RF-01]{RF-01} a \hyperref[tab:RF-16]{RF-16}; origen directo de \hyperref[tab:RF-27]{RF-27} & Sí \\
			\addlinespace
			EV-24 & Entrevista grabada, audio, video, consentimiento, y sesión de validación (\emph{walkthrough}) grabada en video & VET-07 (veterinaria, usuaria técnica) & Entrevista: refuerza \hyperref[tab:RF-02]{RF-02}, \hyperref[tab:RF-09]{RF-09}, \hyperref[tab:RF-16]{RF-16}, \hyperref[tab:RF-23]{RF-23}, \hyperref[tab:RF-24]{RF-24}; validación de \hyperref[tab:RF-01]{RF-01} a \hyperref[tab:RF-17]{RF-17}; origen directo de \hyperref[tab:RF-26]{RF-26} & Sí \\
			\addlinespace
		\end{longtable}
	}
	...
	Se levantó un acta por cada entrevista semiestructurada realizada. Cada
	...
	\hyperref[tab:RD-09]{RD-09}, las personas entrevistadas se identifican únicamente por su código.
	
	
	\subsubsection*{Acta de entrevista 1 --- PROP-01 (EV-01)}
...
{\small
	\begin{longtable}{@{}p{5.6cm} p{9.4cm}@{}}
		...
		\midrule
		\endfirsthead
		\multicolumn{2}{@{}l}{\small\itshape (continuación del acta 1)}\\
		\toprule
		\textbf{Pregunta} & \textbf{Respuesta resumida} \\
		\midrule
		\endhead
		...
		¿Cómo gestiona actualmente la salud y el historial médico de su mascota, y qué información considera esencial compartir o mantener privada? & Realiza controles veterinarios periódicos y mantiene registro de vacunas, tratamientos y enfermedades. Considera que las vacunas, alergias, enfermedades contagiosas y el comportamiento deberían poder compartirse; no identifica ningún dato que deba mantenerse privado. \\ \addlinespace
		
		¿Cómo suele realizar el seguimiento de vacunas o controles veterinarios? & Lleva seguimiento del estado de salud general, las vacunas al día, el comportamiento y el historial de enfermedades o vacunaciones previas. \\ \addlinespace
		
		¿Qué criterios de salud, comportamiento o historial médico prioriza al momento de adoptar una mascota o permitir su interacción con otras? & Prioriza el seguimiento preventivo mediante controles veterinarios regulares y recordatorios de vacunas y desparasitantes. Señala como principales dificultades olvidar fechas importantes y el costo de algunos controles. Antes de permitir la interacción con otra mascota, solicita su historial médico, ya que no puede saberse si tiene alguna enfermedad contagiosa. \\ \addlinespace
		
		¿Qué factores determina como compatibilidad entre mascotas, ya sea para convivencia, adopción o cruce? & El temperamento, la edad, el tamaño y la socialización previa de la mascota, tanto la propia como la de la mascota con la que interactúa. \\ \addlinespace
		
		¿Cuáles considera los principales riesgos y problemas en el proceso de adopción, y qué información debería estar disponible para reducirlos? & La falta de información confiable sobre la procedencia de la mascota, el abandono posterior, problemas de salud no informados y la adulteración de información por parte de quien entrega la mascota. Considera necesaria información completa sobre historial médico, vacunas, comportamiento y antecedentes. \\ \addlinespace
		
		¿Qué información considera necesaria sobre el historial médico de una mascota? & Las vacunas y los desparasitantes, como datos más importantes. \\ \addlinespace
		
		¿Qué información considera más importante conocer sobre una mascota antes de adoptarla? & El historial médico y el estado general de salud. No tiene preferencia de raza. \\ \addlinespace
		
		¿Qué aspecto considera importante antes de permitir el cruce entre mascotas? & La raza, en función de la compatibilidad con la raza de la mascota con la que se busca realizar el cruce. \\ \addlinespace
		
	\end{longtable}
}


\subsubsection*{Acta de entrevista 2 --- PROP-02 (EV-02)}
...
{\small
	\begin{longtable}{@{}p{5.6cm} p{9.4cm}@{}}
		...
		\midrule
		\endfirsthead
		\multicolumn{2}{@{}l}{\small\itshape (continuación del acta 2)}\\
		\toprule
		\textbf{Pregunta} & \textbf{Respuesta resumida} \\
		\midrule
		\endhead
		...
		¿Qué información considera fundamental para garantizar el cuidado adecuado de su mascota? & Conocer si su mascota tiene alergias, fobias, y qué enfermedades presenta y cómo tratarlas. \\ \addlinespace
		
		¿Cómo suele conocer el estado de salud de su mascota? & Por medio del comportamiento (niveles de energía inusuales o decaimiento) y mediante chequeos preventivos periódicos. \\ \addlinespace
		
		¿Qué aspectos considera importantes al momento de decidir la adopción de una mascota? & Su situación financiera, si el ambiente cumple las condiciones necesarias, y la salud de la mascota para conocer qué enfermedades presenta. \\ \addlinespace
		
		¿Qué problemas considera más comunes en el proceso de adopción? & Problemas de comunicación al intentar obtener información sobre la mascota desde una fundación. \\ \addlinespace
		
		¿Cómo suele realizar el seguimiento de vacunas o controles veterinarios? & Mediante una ficha del veterinario con fecha de cada vacuna aplicada y la próxima, complementada con un calendario propio con las fechas de la siguiente vacuna y del siguiente chequeo. \\ \addlinespace
		
		¿Qué información considera privada o sensible sobre su mascota? & La rutina diaria y la ubicación habitual de la mascota, por el riesgo de robo de mascotas de raza para venta o uso reproductivo. \\ \addlinespace
		
		¿Qué información considera necesaria sobre el historial médico de una mascota? & Si presenta enfermedades, cuáles son, y si presenta alergias o fobias. \\ \addlinespace
		
		¿Qué aspectos considera importantes antes de permitir el cruce entre mascotas? & La salud de la mascota con la que se cruzaría, priorizando conocer enfermedades genéticas o adquiridas que puedan afectar a la propia mascota. \\ \addlinespace
		
		¿Cómo decidiría si una mascota es compatible con la suya? & Verificando que la otra mascota no presente enfermedades, genéticas o adquiridas, que puedan representar un riesgo. \\ \addlinespace
		
	\end{longtable}
}


\subsubsection*{Acta de entrevista 3 --- VET-01 (EV-03)}
...
{\small
	\begin{longtable}{@{}p{5.6cm} p{9.4cm}@{}}
		...
		\midrule
		\endfirsthead
		\multicolumn{2}{@{}l}{\small\itshape (continuación del acta 3)}\\
		\toprule
		\textbf{Pregunta} & \textbf{Respuesta resumida} \\
		\midrule
		\endhead
		...
		¿Qué información considera esencial conocer sobre una mascota para brindar una atención adecuada, y qué riesgos implica no tenerla? & Edad, sexo, estado reproductivo, esterilización, historial médico, vacunas, desparasitaciones, enfermedades previas, comportamiento y tipo de alimentación. Su ausencia puede acarrear un mal diagnóstico e impedir emitir un diagnóstico confiable. \\ \addlinespace
		
		¿Cómo influye la raza en la atención de la mascota? & De forma directa: las necesidades varían considerablemente entre razas de tamaño y complexión distintos. \\ \addlinespace
		
		¿Cómo influyen la edad, la raza o el historial médico en las decisiones de cuidado, y qué datos considera sensibles? & Los cachorros requieren vacunación y socialización; los adultos, prevención y ejercicio; los animales mayores requieren chequeos frecuentes, dietas especiales y manejo de enfermedades degenerativas. \\ \addlinespace
		
		¿Qué elementos definen una adopción responsable, y qué consecuencias observa en adopciones informales? & Responsabilidad afectiva y continuidad en la vacunación y desparasitación. Su ausencia deriva en un mayor número de abandonos. \\ \addlinespace
		
		¿Qué información previa considera indispensable evaluar antes de un cruce, y qué riesgos identifica en la reproducción irresponsable? & Exámenes genéticos para descartar enfermedades hereditarias, salud reproductiva, edad adecuada, pruebas de enfermedades infecciosas (brucelosis), temperamento y conformación física. La ausencia de estas evaluaciones deriva en abandonos, sobrepoblación, sufrimiento por enfermedades y complicaciones en partos. \\ \addlinespace
		
		¿Cómo determinar si dos animales son compatibles? & Mediante observación del comportamiento: postura, ausencia de agresividad extrema y reacción ante recursos compartidos (comida, juguetes). \\ \addlinespace
		
		¿Qué consideraciones éticas deben guiar el manejo de la información de una mascota, y cómo protegerla? & Educación del propietario en cada proceso de cuidado veterinario; la información debería compartirse solo con autorización del dueño. \\ \addlinespace
		
		¿Cómo puede un veterinario verificar si una mascota cuenta con controles médicos adecuados? & Revisando cartillas de salud e historial veterinario, y verificando que las vacunas estén al día según la especie. \\ \addlinespace
		
		¿Cómo se identifican posibles riesgos en procesos de reproducción entre mascotas? & Evaluando compatibilidad de raza, diferencia de tamaño, enfermedades preexistentes y posibles problemas hereditarios que puedan transferirse a la cría. \\ \addlinespace
		
	\end{longtable}
}


\subsubsection*{Acta de entrevista 4 --- VET-02 (EV-04)}
...
{\small
	\begin{longtable}{@{}p{5.6cm} p{9.4cm}@{}}
		...
		\midrule
		\endfirsthead
		\multicolumn{2}{@{}l}{\small\itshape (continuación del acta 4)}\\
		\toprule
		\textbf{Pregunta} & \textbf{Respuesta resumida} \\
		\midrule
		\endhead
		...
		¿Qué elementos definen una adopción responsable, y qué consecuencias observa en adopciones informales? & Responsabilidad afectiva sostenida en el tiempo, incluyendo el cumplimiento de esquemas de vacunación y desparasitación; su ausencia incrementa el abandono. \\ \addlinespace
		
		¿Qué información previa es indispensable antes de un cruce, y qué riesgos identifica en procesos de reproducción irresponsable? & Exámenes genéticos para descartar enfermedades hereditarias, salud reproductiva, edad adecuada y pruebas de enfermedades infecciosas como la brucelosis. Los riesgos de la reproducción irresponsable incluyen abandonos, sobrepoblación y complicaciones de salud tanto en la madre como en las crías. \\ \addlinespace
		
		¿Cómo se determina si dos animales son compatibles? & A través de señales de comportamiento (postura, cola, orejas) y ausencia de agresividad extrema, supervisando también la reacción ante recursos compartidos. \\ \addlinespace
		
		¿Qué consideraciones éticas deben guiar el manejo de la información de una mascota? & Educación continua del propietario en el proceso de cuidado; participación de instituciones educativas y organizaciones en la difusión de información responsable. \\ \addlinespace
		
		¿Qué datos ayudan a entender mejor el estado general de una mascota? & Estado de salud, enfermedades crónicas, historial reproductivo, comportamientos agresivos o miedosos, y alergias a medicamentos o alimentos. \\ \addlinespace
		
		¿Cuál es el riesgo cuando se desconocen estos datos? & Riesgo de enfermedades infecciosas no detectadas, daños metabólicos, dermatológicos, cardíacos o complicaciones graves durante procedimientos quirúrgicos si no se conocen alergias a fármacos. \\ \addlinespace
		
		¿Cómo puede verificarse si una mascota cuenta con controles médicos adecuados? & Revisando cartillas de salud, historial veterinario y verificando que las vacunas estén al día según la especie. \\ \addlinespace
		
		¿Quién puede acceder a la información médica de una mascota? & Solo puede compartirse con autorización del propietario; no está disponible de forma pública para terceros sin relación con la mascota. \\ \addlinespace
		
		¿Cómo se identifican posibles riesgos en procesos de reproducción entre mascotas? & Evaluando compatibilidad de raza, diferencia de tamaño y antecedentes de problemas hereditarios que puedan transferirse a la descendencia. \\ \addlinespace
		
	\end{longtable}
}


\subsubsection*{Acta de entrevista 5 --- VET-03 (EV-05)}
...
{\small
	\begin{longtable}{@{}p{5.6cm} p{9.4cm}@{}}
		...
		\midrule
		\endfirsthead
		\multicolumn{2}{@{}l}{\small\itshape (continuación del acta 5)}\\
		\toprule
		\textbf{Pregunta} & \textbf{Respuesta resumida} \\
		\midrule
		\endhead
		...
		¿Qué información considera más importante conocer sobre una mascota? & La edad, la raza y el control de vacunas o desparasitaciones. \\ \addlinespace
		
		¿Qué datos ayudan a entender mejor el estado general de una mascota? & La alimentación que recibe, y en caso de enfermedad, la presencia de vómitos o diarrea. \\ \addlinespace
		
		¿Qué antecedentes considera importante conocer? & Reacciones alérgicas previas, esterilización y automedicación por parte del propietario. \\ \addlinespace
		
		¿Qué riesgos existen cuando no se conoce suficiente información sobre una mascota? & Riesgos quirúrgicos por falta de exámenes complementarios previos, complicaciones en cicatrización de heridas o en pacientes con problemas cardíacos o respiratorios durante cirugía o postoperatorio. \\ \addlinespace
		
		¿Cómo influye la edad en el cuidado y comportamiento de una mascota? & Las mascotas dóciles son más fáciles de manipular durante una intervención; el temperamento arisco dificulta los cuidados necesarios, sobre todo en adultos. \\ \addlinespace
		
		¿Cómo influye la raza en las necesidades de una mascota? & Las razas braquicefálicas (nariz chata) tienden a problemas respiratorios; ciertas razas grandes, como el pastor alemán, presentan predisposición a displasia de cadera. \\ \addlinespace
		
		¿Qué datos de la mascota considera sensibles? & Principalmente la edad. \\ \addlinespace
		
		¿Qué enfermedades o condiciones considera importante registrar? & Casos de moquillo o parvovirus superados, que suelen dejar secuelas como diarreas o problemas nerviosos. \\ \addlinespace
		
		¿Qué información involucra el perfil médico de una mascota? & Automedicación previa, datos generales (nombre, raza, condición), signos vitales (temperatura, peso). \\ \addlinespace
		
		¿Cómo puede verificarse si una mascota cuenta con controles médicos adecuados? & Mediante el carnet físico o, en su ausencia, registros en plataformas virtuales. \\ \addlinespace
		
		¿Cómo evalúa si una adopción puede considerarse responsable? & Verificando estabilidad económica del adoptante y que la vivienda sea propia, no arrendada. \\ \addlinespace
		
		¿Qué información considera importante antes de entregar una mascota en adopción? & Verificar que el adoptante cuente con un lugar cerrado y propio. \\ \addlinespace
		
		¿Cómo afectan las adopciones informales al bienestar animal? & Las adopciones de animales con discapacidad requieren conocer con qué otros animales convivirá para evitar afectaciones en su entorno. \\ \addlinespace
		
		¿Qué información considera esencial antes de recomendar un cruce entre mascotas? & La raza, el pedigrí, y verificar la ausencia de parentesco entre los ejemplares. \\ \addlinespace
		
		¿Cómo puede identificarse si dos mascotas son compatibles? & Principalmente por raza. \\ \addlinespace
		
		¿Cómo se identifican posibles riesgos en procesos de reproducción? & Los casos más riesgosos corresponden a razas braquicefálicas, por su predisposición genética a problemas respiratorios y cardíacos. \\ \addlinespace
		
		¿Cómo puede afectar un cruce irresponsable entre mascotas? & Deformidades, polidactilia y malformaciones articulares derivadas de parentesco entre los ejemplares cruzados. \\ \addlinespace
		
	\end{longtable}
}


\subsubsection*{Acta de entrevista 6 --- PROP-03 (EV-06)}
...
\begin{table}[H]
	...
	\begin{tabularx}{\textwidth}{@{}l X@{}}
		...
		Cargo/Rol & Usuaria potencial / Propietaria de mascota \\
...
{\small
	\begin{longtable}{@{}p{5.6cm} p{9.4cm}@{}}
		...
		\midrule
		\endfirsthead
		\multicolumn{2}{@{}l}{\small\itshape (continuación del acta 6)}\\
		\toprule
		\textbf{Pregunta} & \textbf{Respuesta resumida} \\
		\midrule
		\endhead
		...
		¿Cómo suele conocer el estado de salud de su mascota? & Observando su comportamiento (alimentación, ánimo, movilidad, cambios físicos); ante cualquier señal atípica, acude al veterinario. \\ \addlinespace
		
		¿Cómo suele realizar el seguimiento de vacunas o controles veterinarios? & Revisa el carnet de vacunación y trata de recordar las fechas de control, apoyándose en el celular o en el aviso de la veterinaria; reconoce haber olvidado alguna fecha importante. \\ \addlinespace
		
		¿Qué información considera necesaria antes de dejar a su mascota interactuar con otras mascotas? & Que la otra mascota tenga las vacunas al día, esté desparasitada, no tenga enfermedades recientes, y conocer si tiene antecedentes de agresividad. \\ \addlinespace
		
		¿Qué información considera necesaria sobre el historial médico de una mascota? & Vacunas, desparasitaciones, enfermedades previas, alergias, cirugías, tratamientos actuales y controles veterinarios recientes. \\ \addlinespace
		
		¿Qué información considera privada o sensible sobre su mascota? & La ubicación exacta de residencia, datos personales, número de contacto directo y documentos privados; considera que ciertos detalles médicos no deberían mostrarse públicamente sin autorización. \\ \addlinespace
		
		¿Qué problema considera más común en los procesos de adopción? & La falta de información completa o real sobre la mascota, poca comunicación entre las partes, y ausencia de seguimiento posterior a la entrega. \\ \addlinespace
		
		¿Qué información considera más importante conocer sobre una mascota antes de adoptar? & Estado de salud, vacunas, comportamiento, temperamento e historial médico, además de si convive bien con otras mascotas, niños o personas. \\ \addlinespace
		
		¿Cómo decide si una mascota es compatible con otra? & Observando el comportamiento al estar cerca de otra mascota (tranquilidad, agresividad o nerviosismo), considerando además edad, tamaño, temperamento y socialización previa. \\ \addlinespace
		
		¿Qué factores influyen en su decisión al momento de elegir una mascota? & Estado de salud, comportamiento, edad, espacio disponible en casa, tiempo disponible y capacidad de asumir gastos de alimentación, vacunas y veterinario. \\ \addlinespace
		
		¿Qué aspectos considera importantes antes de permitir el cruce entre mascotas? & Que ambas mascotas estén sanas, vacunadas, con edad adecuada y sin enfermedades de transmisión; también considera la raza, el comportamiento y la responsabilidad de ambos propietarios. \\ \addlinespace
		
	\end{longtable}
}


\subsubsection*{Acta de entrevista 7 --- PROP-04 (EV-09)}
	...
	{\small
		\begin{longtable}{@{}p{5.6cm} p{9.4cm}@{}}
			...
			\midrule
			\endfirsthead
			\multicolumn{2}{@{}l}{\small\itshape (continuación del acta 7)}\\
			\toprule
			\textbf{Pregunta} & \textbf{Respuesta resumida} \\
			\midrule
			\endhead
	...
	\subsubsection*{Acta de entrevista 8 --- PROP-05 (EV-10)}
	...
	{\small
		\begin{longtable}{@{}p{5.6cm} p{9.4cm}@{}}
			...
			\midrule
			\endfirsthead
			\multicolumn{2}{@{}l}{\small\itshape (continuación del acta 8)}\\
			\toprule
			\textbf{Pregunta} & \textbf{Respuesta resumida} \\
			\midrule
			\endhead
	...
	\subsubsection*{Acta de entrevista 9 --- PROP-06 (EV-11)}
	...
	\subsubsection*{Acta de entrevista 10 --- PROP-07 (EV-12)}
	...
	\subsubsection*{Acta de entrevista 11 --- PROP-08 (EV-13)}
	...
	\subsubsection*{Acta de entrevista 12 --- PROP-09 (EV-14)}
	...
	\subsubsection*{Acta de entrevista 13 --- VET-04 (EV-15)}
	...
	\subsubsection*{Acta de entrevista 14 --- VET-05 (EV-16)}
	...
	\subsubsection*{Acta de entrevista 15 --- PROP-10 (EV-17)}
	...
	\subsubsection*{Acta de entrevista 16 --- PROP-11 (EV-18)}
	...
	\subsubsection*{Acta de entrevista 17 --- PROP-12 (EV-23)}
	...
	\subsubsection*{Acta de entrevista 18 --- VET-07 (EV-24)}
	...
	cruza. El perfil dominante, propietario de mascota, no alcanza el mínimo
	de $n \geq 60$ respuestas por perfil. Como alternativa admitida por la
	guía de esta entrega cuando el mínimo por perfil no se alcanza, el equipo
	calculó la potencia estadística real alcanzada con el $n$ disponible
	($\alpha = 0{,}05$, $1 - \beta = 0{,}80$, $d$ de Cohen $= 0{,}5$),
	mediante una prueba $t$ de una muestra ejecutada con el paquete
	\texttt{statsmodels} de Python. Con $n = 47$, la potencia estadística
	alcanzada es de \textbf{91,86\,\%}, muy por encima del mínimo del
	80\,\% exigido; el tamaño de muestra mínimo teórico requerido para
	alcanzar esa potencia con $d = 0{,}5$ es de 34 respuestas, ampliamente
	superado por el $n$ disponible. El script de cálculo, completamente
	reproducible, y la tabla de resultados se versionan en
	\texttt{02\_Evidencias/Cuestionario/power\_calculation/}. Este cálculo
	es independiente del cálculo de potencia ya reportado para el
	componente empírico del Enfoque 2 (Sección~\ref{sec:plan-analisis}), que
	corresponde a un análisis distinto sobre el corpus de requisitos y no
	sobre las respuestas de campo.
	...
	refuerza los requisitos \hyperref[tab:RF-12]{RF-12} (verificación de identidad) y \hyperref[tab:RF-21]{RF-21} (registro
	...
	los metadatos, conforme a la \hyperref[tab:RD-09]{RD-09} y a la zona pública definida para la
	...
	Se registran 6 sesiones de validación (3 con usuarios
	...
	con la incorporación de PROP-12 y VET-07 (3 con perfil técnico y
	...
	\begin{table}[H]
		...
		\begin{tabularx}{\textwidth}{@{}p{2.6cm} p{3.2cm} p{2.4cm} p{1.8cm} X@{}}
			...
			\midrule
			VET-04 & Técnico (veterinaria) & 12:09 min & 27/07/2026 & \hyperref[tab:RF-01]{RF-01} a \hyperref[tab:RF-06]{RF-06}, \hyperref[tab:RF-08]{RF-08} \\
			PROP-02 & No técnico (propietaria) & 16:11 min & 27/07/2026 & \hyperref[tab:RF-01]{RF-01} a \hyperref[tab:RF-06]{RF-06}, \hyperref[tab:RF-08]{RF-08}, \hyperref[tab:RF-11]{RF-11} \\
			PROP-08 & No técnico (propietario) & 25:40 min & 27/07/2026 & \hyperref[tab:RF-01]{RF-01} a \hyperref[tab:RF-17]{RF-17} (conjunto completo) \\
			VET-06 & Técnico (veterinario) & 13:05 min & 02/08/2026 & \hyperref[tab:RF-01]{RF-01} a \hyperref[tab:RF-17]{RF-17} (conjunto completo) \\
			PROP-12 & No técnico (propietaria) & 22:18 min & 14/08/2026 & \hyperref[tab:RF-01]{RF-01} a \hyperref[tab:RF-16]{RF-16} \\
			VET-07 & Técnico (veterinaria) & 14:17 min & 13/08/2026 & \hyperref[tab:RF-01]{RF-01} a \hyperref[tab:RF-17]{RF-17} (conjunto completo) \\
			\bottomrule
		\end{tabularx}
	\end{table}
	...
	\hyperref[tab:RF-17]{RF-17}; las sesiones de PROP-12 y VET-07 son además el origen directo de
	\hyperref[tab:RF-27]{RF-27} y \hyperref[tab:RF-26]{RF-26}, respectivamente, conforme se detalla en sus actas de
	...
	\begin{table}[H]
		...
		\begin{tabularx}{\textwidth}{@{}X p{3.0cm} p{2.4cm} p{2.2cm} c@{}}
			...
			\midrule
			``El temperamento, la edad, el tamaño'' & Criterios de compatibilidad & Compatibilidad y cruza & Refuerza \hyperref[tab:RF-06]{RF-06} & EV-01 \\
			\addlinespace
			``El acceso a esta información debe estar restringido'' & Control granular de visibilidad médica & Privacidad & Refuerza \hyperref[tab:RF-11]{RF-11} & EV-04 \\
			\addlinespace
			``Se puede sacar de algún registro de alguna plataforma'' & Mención de plataforma virtual como registro & Seguimiento preventivo & Refuerza \hyperref[tab:RF-14]{RF-14}, \hyperref[tab:RNF-08]{RNF-08} & EV-05 \\
			\addlinespace
			``Evitar el uso indebido o repetitivo de una mascota... maltrato animal'' & Riesgo de maltrato animal por cruces repetitivos & Compatibilidad y cruza & \hyperref[tab:RF-27]{RF-27} (origen directo) & EV-23 \\
			\addlinespace
			``Podría funcionar mejor un sistema donde la información sea validada mediante otros colegas'' & Validación médica por pares veterinarios & Confianza y verificación & \hyperref[tab:RF-26]{RF-26} (origen directo) & EV-24 \\
			\bottomrule
		\end{tabularx}
	\end{table}
	...
	La curva evidencia un descubrimiento intenso de códigos en las primeras
	entrevistas del corpus (EV-01 a EV-08), con entre 6 y 9 códigos nuevos
	por evidencia, que desciende progresivamente hasta llegar a \textbf{cero
	códigos nuevos en EV-17}, punto que marca la saturación temática del
	corpus de entrevistas y cuestionario. La incorporación de las cuatro
	sesiones de validación \emph{walkthrough} (EV-19 a EV-22) reactiva
	parcialmente el descubrimiento, con 7 códigos nuevos en EV-19 y 3 en cada
	una de las restantes, hasta alcanzar los 80 códigos acumulados. La
	incorporación de EV-23 (PROP-12) y EV-24 (VET-07) --- ambas de
	naturaleza mixta, entrevista semiestructurada seguida de sesión de
	validación \emph{walkthrough} --- reactiva nuevamente el descubrimiento
	con una intensidad comparable a la de las primeras entrevistas de campo:
	7 códigos nuevos en EV-23 y 8 en EV-24, hasta alcanzar los \textbf{95
	códigos acumulados} con los que cierra el corpus. Este segundo repunte,
	más pronunciado que el de las sesiones de \emph{walkthrough} puras
	(EV-19 a EV-22), es coherente con el carácter combinado de EV-23 y
	EV-24: al reintroducir una fase de entrevista semiestructurada antes de
	la validación, ambas evidencias exploran vocabulario nuevo (por ejemplo,
	el origen directo de \hyperref[tab:RF-26]{RF-26} y \hyperref[tab:RF-27]{RF-27}) además de confirmar códigos ya
	existentes del corpus. Este repunte es esperable y no invalida la
	saturación previa: el \emph{walkthrough} es un tipo de evidencia
	distinto al de la elicitación, orientado a la validación de requisitos
	ya especificados, por lo que introduce un vocabulario propio
	...
	Como evidencia terminal adicional a la ronda de validación \emph{walkthrough}
	(Apéndice~B.7), se realizó una sesión de miembro-verificación (\emph{member
		checking}) en la que al menos tres participantes previos del estudio
	confirman la interpretación de los resultados del análisis cualitativo,
	...
	Se exige una sesión con al menos tres
	...
	a la restricción \hyperref[tab:RD-09]{RD-09}.
...
volumen dividido en partes de 80~MB cada una, alojado en
\texttt{02\_Evidencias/00\_Restringido/evidencias\_restringidas/}, dado
que el proveedor del repositorio limita a 2~GB el tamaño de un archivo
individual. La contraseña del
contenedor se entrega únicamente al docente responsable por el Sistema de
Gestión Académica (SGA). El detalle de cada archivo interno --- nombre, tipo,
...
del volumen \texttt{.7z} los contiene.
	...
	{\small
		\begin{longtable}{@{}>{\raggedright\arraybackslash}p{1.4cm} >{\raggedright\arraybackslash}p{1.6cm} >{\raggedright\arraybackslash}p{3.2cm} >{\raggedright\arraybackslash}p{1.8cm} >{\raggedright\arraybackslash}p{6.5cm}@{}}
			...
			\bottomrule
			\endlastfoot
			EV-01 & PROP-01 & Propietario de mascota & 17/05/2026 & \texttt{2026-05-17\_PropietarioMascota\_PROP-01\_TranscripcionEntrevista.md} \\
			EV-02 & PROP-02 & Propietaria de mascota & 17/05/2026 & \texttt{2026-05-17\_PropietarioMascota\_PROP-02\_TranscripcionEntrevista.md} \\
			EV-03 & VET-01 & Médica veterinaria & 17/05/2026 & \texttt{2026-05-17\_Veterinaria\_VET-01\_TranscripcionEntrevista.md} \\
			EV-04 & VET-02 & Veterinario & 17/05/2026 & \texttt{2026-05-17\_Veterinario\_VET-02\_TranscripcionEntrevista.md} \\
			EV-05 & VET-03 & Médico veterinario & 29/05/2026 & \texttt{2026-05-29\_Veterinario\_VET-03\_TranscripcionEntrevista.md} \\
			EV-06 & PROP-03 & Propietaria de mascota & 14/06/2026 & \texttt{2026-06-14\_PropietarioMascota\_PROP-03\_TranscripcionEntrevista.md} \\
			EV-09 & PROP-04 & Propietario de mascota & 26/07/2026 & \texttt{2026-07-26\_PropietarioMascota\_PROP-04\_TranscripcionEntrevista.md} \\
			EV-10 & PROP-05 & Propietario de mascota & 26/07/2026 & \texttt{2026-07-26\_PropietarioMascota\_PROP-05\_TranscripcionEntrevista.md} \\
			EV-11 & PROP-06 & Propietaria de mascota & 24/07/2026 & \texttt{2026-07-24\_PropietarioMascota\_PROP-06\_TranscripcionEntrevista.md} \\
			EV-12 & PROP-07 & Propietaria de mascota & 26/07/2026 & \texttt{2026-07-26\_PropietarioMascota\_PROP-07\_TranscripcionEntrevista.md} \\
			EV-13 & PROP-08 & Propietario de mascota & 23/07/2026 & \texttt{2026-07-23\_PropietarioMascota\_PROP-08\_TranscripcionEntrevista.md} \\
			EV-14 & PROP-09 & Propietario de mascota & 25/07/2026 & \texttt{2026-07-25\_PropietarioMascota\_PROP-09\_TranscripcionEntrevista.md} \\
			EV-15 & VET-04 & Veterinaria & 27/07/2026 & \texttt{2026-07-27\_Veterinaria\_VET-04\_TranscripcionEntrevista.md} \\
			EV-16 & VET-05 & Veterinario & 25/07/2026 & \texttt{2026-07-25\_Veterinario\_VET-05\_TranscripcionEntrevista.md} \\
			EV-17 & PROP-10 & Propietaria de mascota & 26/07/2026 & \texttt{2026-07-26\_PropietarioMascota\_PROP-10\_TranscripcionEntrevista.md} \\
			EV-18 & PROP-11 & Propietaria de mascota & 26/07/2026 & \texttt{2026-07-26\_PropietarioMascota\_PROP-11\_TranscripcionEntrevista.md} \\
		EV-23 & PROP-12 & Propietaria de mascota (usuaria no técnica) & 14/08/2026 & \texttt{2026-08-14\_PropietarioMascota\_PROP-12\_TranscripcionEntrevista.md} \\
		EV-24 & VET-07 & Veterinaria (usuaria técnica) & 13/08/2026 & \texttt{2026-08-13\_Veterinario\_VET-07\_TranscripcionEntrevista.md} \\
		\end{longtable}
	}
	...
	técnicos, 3 con usuarios no técnicos). El acta enmascarada de cada sesión reside en
	...
	\textbf{Cumplimiento del objetivo declarado.} El objetivo
	declarado por el propio equipo era de
	6 sesiones de \emph{walkthrough} (3 técnicas + 3 no técnicas). Con la
	incorporación de EV-23 y EV-24, las 6 sesiones quedan ejecutadas y el
	objetivo se da por cumplido.
	...
	\texttt{02\_Evidencias/Cuestionario/Fotos\_Aplicacion/}. Se registran
	61 respuestas.
	...
	vacunación, todos con fecha 27/07/2026.
	...
	\begin{table}[H]
		\centering
		\small
		\caption{Resumen del índice de multimedia recolectado.}
		\label{tab:resumen-verificacion-mm}
		\begin{tabularx}{\textwidth}{@{}X l@{}}
			\toprule
			\textbf{Tipo de evidencia} & \textbf{Cantidad} \\
			\midrule
			...
			Sesiones de \emph{walkthrough} & 6 (3 téc. + 3 no téc.) \\
			Sesión de miembro-verificación (\emph{member checking}) & 1 (3 participantes) \\
			\bottomrule
		\end{tabularx}
	\end{table}
	...
	\begin{landscape}
		\begin{longtable}{@{}p{1.2cm} p{5.2cm} p{1.0cm} p{1.3cm} p{1.3cm} p{1.1cm} p{6.5cm}@{}}
			...
			\hyperref[tab:RF-01]{RF-01} & Registrar mascota & RF & Must & Media (4,06) & 9,20 & Es el pilar del sistema; sin este RF no puede existir ningún perfil de mascota ni proceso posterior. \\ \midrule
			\hyperref[tab:RF-02]{RF-02} & Gestionar historial médico & RF & Must & Alta (4,58) & 3,63 & Crítico para la salud animal y para \hyperref[rl:03]{RL-03} (rectificación de datos médicos). \\ \midrule
			\hyperref[tab:RF-03]{RF-03} & Consultar perfil completo & RF & Must & Media (4,06) & 8,67 & Fundamental para que adoptantes e interesados en cruza tomen decisiones informadas; sustenta \hyperref[rl:02]{RL-02}. \\ \midrule
			\hyperref[tab:RF-04]{RF-04} & Buscar y filtrar mascotas & RF & Should & Media (4,16) & 1,80 & Mejora la usabilidad, pero el sistema puede operar inicialmente con listas generales. \\ \midrule
			\hyperref[tab:RF-05]{RF-05} & Gestionar solicitudes de adopción & RF & Must & --- & 3,63 & Núcleo del modelo operativo de la plataforma. \\ \midrule
			\hyperref[tab:RF-06]{RF-06} & Evaluar compatibilidad para cruza & RF & Must & Media (4,45) & 1,65 & Requisito diferenciador que usa IA; motivó \hyperref[tab:RD-01]{RD-01} y exige la explicabilidad de \hyperref[rl:06]{RL-06}. \\ \midrule
			\hyperref[tab:RF-07]{RF-07} & Sistema de mensajería & RF & Could & Baja (3,84) & --- & Deseable para coordinar procesos, pero el contacto inicial puede derivarse a canales externos si hay limitaciones de tiempo. \\ \midrule
			\hyperref[tab:RF-08]{RF-08} & Gestionar solicitudes de cruza & RF & Must & --- & 2,88 & Operación central del modelo de negocio junto a \hyperref[tab:RF-05]{RF-05}. \\ \midrule
			\hyperref[tab:RF-09]{RF-09} & Validar información médica & RF & Must & Alta (4,61) & 4,20 & Imprescindible para la confiabilidad clínica del sistema. \\ \midrule
			\hyperref[tab:RF-10]{RF-10} & Recordatorios de controles preventivos & RF & Should & --- & 2,80 & Aporta valor de bienestar animal, pero no bloquea el flujo principal de adopciones. \\ \midrule
			\hyperref[tab:RF-11]{RF-11} & Administrar privacidad & RF & Must & --- & 5,75 & Mandatorio por \hyperref[rl:02]{RL-02} y \hyperref[rl:05]{RL-05}; es el RF más citado en las entrevistas (9 EV). \\ \midrule
			\hyperref[tab:RF-12]{RF-12} & Verificar identidad de usuarios & RF & Must & Media (4,34) & 5,80 & Indispensable para evitar fraude; sustenta el consentimiento de \hyperref[rl:01]{RL-01}. \\ \midrule
			\hyperref[tab:RF-13]{RF-13} & Registrar publicaciones & RF & Must & --- & 7,00 & Necesario para que un propietario active la disponibilidad de su mascota. \\ \midrule
			\hyperref[tab:RF-14]{RF-14} & Gestionar carnet de vacunación & RF & Must & Alta (4,71) & 4,20 & Crítico para la trazabilidad sanitaria; sustenta \hyperref[rl:03]{RL-03}. \\ \midrule
			\hyperref[tab:RF-15]{RF-15} & Consultar historial de procesos & RF & Could & --- & --- & Aporta trazabilidad al usuario, pero no es indispensable para completar una adopción o cruza. \\ \midrule
			\hyperref[tab:RF-16]{RF-16} & Antecedentes genéticos y parentesco & RF & Must & --- & 2,00 & Necesario para que \hyperref[tab:RF-06]{RF-06} pueda evaluar compatibilidad de forma responsable. \\ \midrule
			\hyperref[tab:RF-17]{RF-17} & Validar imágenes con IA & RF & Should & --- & 1,23 & Refuerza la seguridad del contenido, pero el sistema puede operar con moderación manual inicial. \\ \midrule
			\hyperref[tab:RF-18]{RF-18} & Flujo de adopción por etapas & RF & Must & --- & 1,23 & Formaliza el proceso de adopción responsable identificado en la segunda ronda de campo (EV-11, EV-16). \\ \midrule
			\hyperref[tab:RF-19]{RF-19} & Validar certificados veterinarios antes de cruza & RF & Should & --- & 2,60 & Refuerza la confiabilidad de \hyperref[tab:RF-06]{RF-06}, pero no bloquea su funcionamiento básico. \\ \midrule
			\hyperref[tab:RF-20]{RF-20} & Encuentros de socialización supervisados & RF & Could & --- & --- & Valor deseable de bienestar animal, mencionado por un solo participante (EV-13). \\ \midrule
			\hyperref[tab:RF-21]{RF-21} & Registrar identificador de microchip & RF & Should & --- & 3,00 & Aporta trazabilidad complementaria al carnet de vacunación. \\ \midrule
			\hyperref[tab:RF-22]{RF-22} & Seguimiento post-adopción & RF & Must & --- & 1,25 & Cierra el ciclo de responsabilidad de \hyperref[tab:RF-18]{RF-18}, exigido por participantes de la segunda ronda. \\ \midrule
			\hyperref[tab:RF-23]{RF-23} & Alertas de riesgo sanitario o físico & RF & Must & --- & 2,63 & Mitiga un riesgo real de seguridad y salud animal detectado en varias entrevistas (EV-09, EV-12, EV-15, EV-18). \\ \midrule
			\hyperref[tab:RF-24]{RF-24} & Trazabilidad de cepa, lote y aplicador & RF & Should & Alta (4,71) & 2,67 & Complementa \hyperref[tab:RF-14]{RF-14} con datos adicionales de trazabilidad, no bloqueante. \\ \midrule
			\hyperref[tab:RF-25]{RF-25} & Aviso de mascota extraviada & RF & Could & --- & --- & Funcionalidad de valor social, pero fuera del núcleo de adopción/cruza del sistema. \\ \midrule
			\hyperref[tab:RF-26]{RF-26} & Segunda opinión veterinaria & RF & Should & --- & 1,60 & Refuerza la confiabilidad clínica mediante revisión entre pares; identificado por la veterinaria entrevistada (EV-24) como la mejora que más valor le aportaría. \\ \midrule
			\hyperref[tab:RF-27]{RF-27} & Control de cruzas repetitivas & RF & Should & --- & 3,33 & Mitiga un riesgo de maltrato animal identificado por la propietaria entrevistada (EV-23) al validar \hyperref[tab:RF-14]{RF-14} y en la síntesis final del walkthrough. \\ \midrule
			\hyperref[tab:RNF-01]{RNF-01} & Protección de información de propietarios y mascotas & RNF & Must & --- & --- & Requisito estricto de seguridad; sustenta \hyperref[rl:05]{RL-05} y \hyperref[rl:07]{RL-07}. \\ \midrule
			\hyperref[tab:RNF-02]{RNF-02} & Disponibilidad del sistema & RNF & Must & --- & --- & Crítico para la operatividad continua de la plataforma. \\ \midrule
			\hyperref[tab:RNF-03]{RNF-03} & Tiempo de respuesta del sistema & RNF & Should & --- & --- & Mejora la experiencia, pero el sistema es funcional con tiempos mayores en una primera versión. \\ \midrule
			\hyperref[tab:RNF-04]{RNF-04} & Facilidad de uso de la plataforma & RNF & Should & --- & --- & Relevante para adopción del producto, pero no bloquea sus funciones. \\ \midrule
			\hyperref[tab:RNF-05]{RNF-05} & Integridad de la información médica & RNF & Must & --- & --- & Evita modificaciones no autorizadas del historial clínico; sustenta \hyperref[rl:07]{RL-07}. \\ \midrule
			\hyperref[tab:RNF-06]{RNF-06} & Exactitud del componente de IA & RNF & Must & --- & --- & Sin una precisión mínima, \hyperref[tab:RF-06]{RF-06} pierde su valor diferenciador. \\ \midrule
			\hyperref[tab:RNF-07]{RNF-07} & Precisión en validación de imágenes & RNF & Should & --- & --- & Mejora la calidad del contenido, pero \hyperref[tab:RF-17]{RF-17} puede operar con una tasa de error inicial mayor. \\ \midrule
			\hyperref[tab:RNF-08]{RNF-08} & Actualización de la información registrada & RNF & Should & --- & --- & Mejora la confianza del usuario, no bloquea el flujo principal. \\ \midrule
			\hyperref[tab:RNF-09]{RNF-09} & Interoperabilidad de notificaciones & RNF & Should & --- & --- & Aporta valor (recordatorios por WhatsApp), pero el sistema opera sin ella mediante notificación interna. \\ \midrule
			\hyperref[tab:RNF-10]{RNF-10} & Facilidad de mantenimiento del código & RNF & Should & --- & --- & Importante para sostenibilidad técnica, pero no bloquea la operación funcional del sistema. \\ \midrule
			\hyperref[tab:RNF-11]{RNF-11} & Portabilidad entre navegadores & RNF & Should & --- & --- & Necesaria para alcance de usuarios, pero el sistema puede operar inicialmente en un navegador principal. \\ \midrule
			\hyperref[tab:RNF-12]{RNF-12} & Escalabilidad ante crecimiento de usuarios & RNF & Could & --- & --- & Deseable a mediano plazo; no crítica mientras la base de usuarios sea reducida. \\ \midrule
			\hyperref[tab:RNF-13]{RNF-13} & Explicabilidad de compatibilidad para cruza & RNF & Must & --- & --- & Exigido por el derecho legal a explicación (\hyperref[rl:06]{RL-06}) sobre decisiones automatizadas del \hyperref[tab:RF-06]{RF-06}. \\ \midrule
			\hyperref[tab:RNF-14]{RNF-14} & Explicabilidad de rechazo de imágenes & RNF & Should & --- & --- & Mejora la confianza del usuario, pero el sistema puede operar con un mensaje de rechazo genérico inicialmente. \\ \midrule
			\hyperref[tab:RNF-15]{RNF-15} & Explicabilidad de moderación de mensajes & RNF & Should & --- & --- & Igual que \hyperref[tab:RNF-14]{RNF-14}: deseable para confianza, no bloqueante. \\ \midrule
			\hyperref[tab:RNF-16]{RNF-16} & Confidencialidad de ubicación y contacto & RNF & Must & --- & --- & Obligación legal directa (\hyperref[rl:05]{RL-05}, principio de minimización). \\ \midrule
			\hyperref[tab:RD-01]{RD-01} & Integración de componentes de IA & RD & Must & --- & --- & Restricción ineludible: arquitectura del principal valor agregado de MundiPets. \\ \midrule
			\hyperref[tab:RD-02]{RD-02} & Acceso basado en roles & RD & Must & --- & --- & Mandatorio para diferenciar permisos de propietarios, veterinarias y administradores; sustenta \hyperref[rl:05]{RL-05} y \hyperref[rl:08]{RL-08}. \\ \midrule
			\hyperref[tab:RD-03]{RD-03} & Validación profesional de la información médica & RD & Must & Alta (4,61) & --- & Exigido por los veterinarios entrevistados para garantizar confiabilidad clínica. \\ \midrule
			\hyperref[tab:RD-04]{RD-04} & Protección de datos personales & RD & Must & --- & --- & Implementa directamente \hyperref[rl:05]{RL-05}, \hyperref[rl:07]{RL-07} y \hyperref[rl:08]{RL-08}. \\ \midrule
			\hyperref[tab:RD-05]{RD-05} & Arquitectura modular del sistema & RD & Must & --- & --- & Facilita mantenimiento e incorporación de nuevas funcionalidades; sustenta \hyperref[tab:RNF-10]{RNF-10}. \\ \midrule
			\hyperref[tab:RD-06]{RD-06} & Eliminación de cuenta y datos personales & RD & Must & --- & --- & Obligación legal ineludible (\hyperref[rl:04]{RL-04}, derecho de supresión); sin este mecanismo el sistema incumple la LOPDP. \\ \midrule
			\hyperref[tab:RD-07]{RD-07} & Protocolo de notificación de vulneraciones & RD & Must & --- & --- & Exigido por \hyperref[rl:09]{RL-09} con plazos legales estrictos (5 y 3 días). \\ \midrule
			\hyperref[tab:RD-08]{RD-08} & Dependencia de servicios externos de mensajería & RD & Should & --- & --- & Aporta valor (\hyperref[tab:RF-10]{RF-10}) pero requiere un canal de respaldo ante caídas del proveedor. \\ \midrule
			\hyperref[tab:RD-09]{RD-09} & Anonimización de evidencias para publicación abierta & RD & Must & --- & --- & Exigido por los Art. 30-32 LOPDP para el uso de datos de investigación en el paquete FAIR. \\ \midrule
			\hyperref[rl:01]{RL-01} & Consentimiento libre, específico, informado e inequívoco & RL & Must & --- & --- & Toda obligación legal es no negociable: su incumplimiento expone al proyecto a responsabilidad legal. \\ \midrule
			\hyperref[rl:02]{RL-02} & Información de finalidad y derecho de acceso & RL & Must & --- & --- & Ídem: obligación legal directa (Art. 12-13 LOPDP). \\ \midrule
			\hyperref[rl:03]{RL-03} & Derecho de rectificación y actualización & RL & Must & --- & --- & Ídem: obligación legal directa (Art. 14 LOPDP). \\ \midrule
			\hyperref[rl:04]{RL-04} & Derecho de supresión de datos personales & RL & Must & --- & --- & Ídem: obligación legal directa (Art. 15 LOPDP). \\ \midrule
			\hyperref[rl:05]{RL-05} & Minimización y seguridad de los datos & RL & Must & --- & --- & Ídem: obligación legal directa (Art. 10 LOPDP). \\ \midrule
			\hyperref[rl:06]{RL-06} & Derecho a explicación de decisiones automatizadas & RL & Must & --- & --- & Ídem: obligación legal directa (Art. 20 LOPDP); base de todo el bloque de explicabilidad. \\ \midrule
			\hyperref[rl:07]{RL-07} & Confidencialidad reforzada de datos sensibles y de salud & RL & Must & --- & --- & Ídem: obligación legal directa (Art. 25-30 LOPDP). \\ \midrule
			\hyperref[rl:08]{RL-08} & Protección de datos desde el diseño y por defecto & RL & Must & --- & --- & Ídem: obligación legal directa (Art. 39 LOPDP). \\ \midrule
			\hyperref[rl:09]{RL-09} & Notificación de vulneraciones de seguridad & RL & Must & --- & --- & Ídem: obligación legal directa (Art. 43 y 46 LOPDP), con plazos estrictos. \\
		\end{longtable}
	\end{landscape}
...
final del proyecto MundiPets, en un formato de diez columnas de trazabilidad más el identificador de fila (ID):
\textbf{Fuente, Requisito, Caso
...
funcionales (\hyperref[tr:01]{TR-01} a \hyperref[tr:27]{TR-27}), luego los 16 no funcionales (\hyperref[tr:28]{TR-28} a \hyperref[tr:43]{TR-43}),
las 9 restricciones de diseño (\hyperref[tr:44]{TR-44} a \hyperref[tr:52]{TR-52}) y finalmente los 9 requisitos
legales (\hyperref[tr:53]{TR-53} a \hyperref[tr:61]{TR-61}) --- para facilitar la lectura y la comparación entre
...
El tablero de gestión del proyecto exporta una tarjeta por cada historia
de usuario (\hyperref[tab:HU-01]{HU-01} a \hyperref[tab:HU-27]{HU-27}) y por cada caso de prueba (\hyperref[tab:CP-01]{CP-01} a \hyperref[cp:61]{CP-61}),
enlazada mediante el mismo identificador utilizado en esta matriz, y se
versiona junto con la exportación completa del tablero en
...
\begin{landscape}
	...
	\begin{longtable}{@{}p{0.8cm} p{4.0cm} p{1.3cm} p{1.7cm} p{2.5cm} p{1.0cm} p{1.9cm} p{0.9cm} p{0.9cm} p{1.75cm} p{2.2cm}@{}}
		...
		\endlastfoot
			...
			\hyperref[tr:01]{TR-01}\label{tr:01} & PROP-02, VET-01, VET-03, OBS-01; también \hyperref[tab:RF-12]{RF-12}, \hyperref[tab:RF-13]{RF-13} & \hyperref[tab:RF-01]{RF-01} & \hyperref[tab:CU-01]{CU-01} & Pet & P1 & Drafted & \hyperref[lst:ca01]{CA-01} & \hyperref[tab:CP-01]{CP-01} & \hyperref[tab:HU-01]{HU-01} & Completa \\ \midrule
			\hyperref[tr:02]{TR-02}\label{tr:02} & PROP-01, VET-02; también \hyperref[tab:RF-14]{RF-14}, \hyperref[tab:RF-24]{RF-24} & \hyperref[tab:RF-02]{RF-02} & \hyperref[tab:CU-02]{CU-02} & MedicalHistory & P1 & --- & \hyperref[lst:ca02]{CA-02} & \hyperref[cp:02]{CP-02} & \hyperref[tab:HU-02]{HU-02} & Completa \\ \midrule
			\hyperref[tr:03]{TR-03}\label{tr:03} & VET-02, PROP-03, OBS-01; también \hyperref[tab:RF-04]{RF-04}, \hyperref[tab:RF-11]{RF-11} & \hyperref[tab:RF-03]{RF-03} & \hyperref[tab:CU-10]{CU-10} & Pet & P2 & --- & \hyperref[lst:ca03]{CA-03} & \hyperref[cp:03]{CP-03} & \hyperref[tab:HU-03]{HU-03} & Completa \\ \midrule
			\hyperref[tr:04]{TR-04}\label{tr:04} & Encuesta, OBS-01; también \hyperref[tab:RF-03]{RF-03} & \hyperref[tab:RF-04]{RF-04} & \hyperref[tab:CU-11]{CU-11} & Post & P2 & --- & \hyperref[lst:ca16]{CA-16} & \hyperref[cp:04]{CP-04} & \hyperref[tab:HU-16]{HU-16} & Completa \\ \midrule
			\hyperref[tr:05]{TR-05}\label{tr:05} & PROP-01, PROP-02, VET-01, OBS-01; también \hyperref[tab:RF-12]{RF-12}, \hyperref[tab:RF-13]{RF-13}, \hyperref[tab:RF-18]{RF-18} & \hyperref[tab:RF-05]{RF-05} & \hyperref[tab:CU-04]{CU-04} & AdoptionRequest & P3 & Created & \hyperref[lst:ca04]{CA-04} & \hyperref[cp:05]{CP-05} & \hyperref[tab:HU-04]{HU-04} & Completa \\ \midrule
			\hyperref[tr:06]{TR-06}\label{tr:06} & VET-02, VET-03, PROP-03, OBS-01; también \hyperref[tab:RF-16]{RF-16}, \hyperref[tab:RF-19]{RF-19}, \hyperref[tab:RF-23]{RF-23} & \hyperref[tab:RF-06]{RF-06} & \hyperref[tab:CU-13]{CU-13} & CompatibilityEvaluation & P8 & --- & \hyperref[lst:ca05]{CA-05} & \hyperref[cp:06]{CP-06} & \hyperref[tab:HU-05]{HU-05} & Completa \\ \midrule
			\hyperref[tr:07]{TR-07}\label{tr:07} & PROP-02, Encuesta, OBS-01; también \hyperref[tab:RF-05]{RF-05}, \hyperref[tab:RF-08]{RF-08}, \hyperref[tab:RNF-15]{RNF-15} & \hyperref[tab:RF-07]{RF-07} & \hyperref[tab:CU-08]{CU-08} & Message & P5 & --- & \hyperref[lst:ca17]{CA-17} & \hyperref[cp:07]{CP-07} & \hyperref[tab:HU-17]{HU-17} & Completa \\ \midrule
			\hyperref[tr:08]{TR-08}\label{tr:08} & PROP-01, VET-01, VET-03; también \hyperref[tab:RF-13]{RF-13}, \hyperref[tab:RF-19]{RF-19}, \hyperref[tab:RF-27]{RF-27} & \hyperref[tab:RF-08]{RF-08} & \hyperref[tab:CU-05]{CU-05} & BreedingRequest & P3 & Created & \hyperref[lst:ca06]{CA-06} & \hyperref[cp:08]{CP-08} & \hyperref[tab:HU-06]{HU-06} & Completa \\ \midrule
			\hyperref[tr:09]{TR-09}\label{tr:09} & VET-01, VET-03, Encuesta; también \hyperref[tab:RF-02]{RF-02}, \hyperref[tab:RF-26]{RF-26} & \hyperref[tab:RF-09]{RF-09} & \hyperref[tab:CU-09]{CU-09} & MedicalHistory, Veterinarian & P4 & Verified & \hyperref[lst:ca07]{CA-07} & \hyperref[cp:09]{CP-09} & \hyperref[tab:HU-07]{HU-07} & Completa \\ \midrule
			\hyperref[tr:10]{TR-10}\label{tr:10} & PROP-02, VET-02, PROP-03; también \hyperref[tab:RF-14]{RF-14}, \hyperref[tab:RD-08]{RD-08} & \hyperref[tab:RF-10]{RF-10} & \hyperref[tab:CU-12]{CU-12} & PetVaccine & P1 & --- & \hyperref[lst:ca18]{CA-18} & \hyperref[cp:10]{CP-10} & \hyperref[tab:HU-18]{HU-18} & Completa \\ \midrule
			\hyperref[tr:11]{TR-11}\label{tr:11} & PROP-02, VET-03, PROP-03, Encuesta, OBS-01; también \hyperref[tab:RF-03]{RF-03}, \hyperref[tab:RD-02]{RD-02} & \hyperref[tab:RF-11]{RF-11} & \hyperref[tab:CU-12]{CU-12} & PrivacySettings & P1 & --- & \hyperref[lst:ca08]{CA-08} & \hyperref[cp:11]{CP-11} & \hyperref[tab:HU-08]{HU-08} & Completa \\ \midrule
			\hyperref[tr:12]{TR-12}\label{tr:12} & PROP-01, VET-01, Encuesta, OBS-01; también \hyperref[tab:RF-01]{RF-01}, \hyperref[rl:01]{RL-01} & \hyperref[tab:RF-12]{RF-12} & (Transversal a Onboarding) & User & P1 & --- & \hyperref[lst:ca09]{CA-09} & \hyperref[cp:12]{CP-12} & \hyperref[tab:HU-09]{HU-09} & Completa \\ \midrule
			\hyperref[tr:13]{TR-13}\label{tr:13} & PROP-01, PROP-02, Encuesta, OBS-01; también \hyperref[tab:RF-01]{RF-01}, \hyperref[tab:RF-05]{RF-05}, \hyperref[tab:RF-08]{RF-08} & \hyperref[tab:RF-13]{RF-13} & \hyperref[tab:CU-04]{CU-04} & Post & P3 & Published & \hyperref[lst:ca10]{CA-10} & \hyperref[cp:13]{CP-13} & \hyperref[tab:HU-10]{HU-10} & Completa \\ \midrule
			\hyperref[tr:14]{TR-14}\label{tr:14} & VET-01, VET-03, Encuesta; también \hyperref[tab:RF-02]{RF-02}, \hyperref[tab:RF-24]{RF-24} & \hyperref[tab:RF-14]{RF-14} & \hyperref[tab:CU-03]{CU-03} & PetVaccine, Vaccine & P1 & --- & \hyperref[lst:ca11]{CA-11} & \hyperref[cp:14]{CP-14} & \hyperref[tab:HU-11]{HU-11} & Completa \\ \midrule
			\hyperref[tr:15]{TR-15}\label{tr:15} & PROP-01, PROP-02, VET-02, PROP-03; también \hyperref[tab:RF-05]{RF-05}, \hyperref[tab:RF-08]{RF-08}, \hyperref[tab:RF-27]{RF-27} & \hyperref[tab:RF-15]{RF-15} & \hyperref[tab:CU-07]{CU-07} & AdoptionRequest, BreedingRequest & P2 & --- & \hyperref[lst:ca19]{CA-19} & \hyperref[cp:15]{CP-15} & \hyperref[tab:HU-19]{HU-19} & Completa \\ \midrule
			\hyperref[tr:16]{TR-16}\label{tr:16} & PROP-01, VET-01, VET-03, Encuesta; también \hyperref[tab:RF-06]{RF-06}, \hyperref[tab:RF-19]{RF-19} & \hyperref[tab:RF-16]{RF-16} & \hyperref[tab:CU-02]{CU-02} & MedicalHistory & P1 & --- & \hyperref[lst:ca12]{CA-12} & \hyperref[tab:CP-16]{CP-16} & \hyperref[tab:HU-12]{HU-12} & Completa \\ \midrule
			\hyperref[tr:17]{TR-17}\label{tr:17} & PROP-01, VET-01, Encuesta, OBS-01; también \hyperref[tab:RNF-07]{RNF-07}, \hyperref[tab:RNF-14]{RNF-14} & \hyperref[tab:RF-17]{RF-17} & (Transversal a Publicaciones) & Pet, Post & P8 & PhotosAndSpeciesChecked & \hyperref[lst:ca20]{CA-20} & \hyperref[cp:17]{CP-17} & \hyperref[tab:HU-20]{HU-20} & Completa \\ \midrule
			\hyperref[tr:18]{TR-18}\label{tr:18} & PROP-06, VET-05; también \hyperref[tab:RF-05]{RF-05}, \hyperref[tab:RF-22]{RF-22} & \hyperref[tab:RF-18]{RF-18} & \hyperref[tab:CU-04]{CU-04} & AdoptionRequest & P3 & Under review & \hyperref[lst:ca13]{CA-13} & \hyperref[cp:18]{CP-18} & \hyperref[tab:HU-13]{HU-13} & Completa \\ \midrule
			\hyperref[tr:19]{TR-19}\label{tr:19} & PROP-06; también \hyperref[tab:RF-06]{RF-06}, \hyperref[tab:RF-16]{RF-16} & \hyperref[tab:RF-19]{RF-19} & \hyperref[tab:CU-09]{CU-09} & VetDocument & P4 & Verified & \hyperref[lst:ca21]{CA-21} & \hyperref[cp:19]{CP-19} & \hyperref[tab:HU-21]{HU-21} & Completa \\ \midrule
			\hyperref[tr:20]{TR-20}\label{tr:20} & PROP-08; también \hyperref[tab:RF-07]{RF-07} & \hyperref[tab:RF-20]{RF-20} & \hyperref[tab:CU-08]{CU-08} & Interaction & P5 & --- & \hyperref[lst:ca27]{CA-27} & \hyperref[cp:20]{CP-20} & \hyperref[tab:HU-27]{HU-27} & Completa \\ \midrule
			\hyperref[tr:21]{TR-21}\label{tr:21} & VET-05; también \hyperref[tab:RF-14]{RF-14}, \hyperref[tab:RF-25]{RF-25} & \hyperref[tab:RF-21]{RF-21} & \hyperref[tab:CU-01]{CU-01} & Pet & P1 & --- & \hyperref[lst:ca22]{CA-22} & \hyperref[cp:21]{CP-21} & \hyperref[tab:HU-22]{HU-22} & Completa \\ \midrule
			\hyperref[tr:22]{TR-22}\label{tr:22} & PROP-05, VET-05; también \hyperref[tab:RF-18]{RF-18} & \hyperref[tab:RF-22]{RF-22} & \hyperref[tab:CU-07]{CU-07} & AdoptionRequest & P3 & adopted or matched & \hyperref[lst:ca14]{CA-14} & \hyperref[cp:22]{CP-22} & \hyperref[tab:HU-14]{HU-14} & Completa \\ \midrule
			\hyperref[tr:23]{TR-23}\label{tr:23} & PROP-04, PROP-07, VET-04, PROP-11; también \hyperref[tab:RF-06]{RF-06}, \hyperref[tab:RF-16]{RF-16} & \hyperref[tab:RF-23]{RF-23} & \hyperref[tab:CU-06]{CU-06} & Interaction, BreedingRequest & P7 & --- & \hyperref[lst:ca15]{CA-15} & \hyperref[cp:23]{CP-23} & \hyperref[tab:HU-15]{HU-15} & Completa \\ \midrule
			\hyperref[tr:24]{TR-24}\label{tr:24} & VET-04, VET-05; también \hyperref[tab:RF-02]{RF-02}, \hyperref[tab:RF-14]{RF-14} & \hyperref[tab:RF-24]{RF-24} & \hyperref[tab:CU-03]{CU-03} & PetVaccine & P1 & --- & \hyperref[lst:ca23]{CA-23} & \hyperref[cp:24]{CP-24} & \hyperref[tab:HU-23]{HU-23} & Completa \\ \midrule
			\hyperref[tr:25]{TR-25}\label{tr:25} & PROP-07, VET-05; también \hyperref[tab:RF-21]{RF-21} & \hyperref[tab:RF-25]{RF-25} & \hyperref[tab:CU-04]{CU-04} & LostPetAlert & P3 & --- & \hyperref[lst:ca24]{CA-24} & \hyperref[cp:25]{CP-25} & \hyperref[tab:HU-24]{HU-24} & Completa \\ \midrule
			\hyperref[tr:26]{TR-26}\label{tr:26} & VET-07; también \hyperref[tab:RF-09]{RF-09} & \hyperref[tab:RF-26]{RF-26} & \hyperref[tab:CU-09]{CU-09} & MedicalHistory, VeterinaryEvaluation & P4 & Verified & \hyperref[lst:ca25]{CA-25} & \hyperref[cp:26]{CP-26} & \hyperref[tab:HU-25]{HU-25} & Completa \\ \midrule
			\hyperref[tr:27]{TR-27}\label{tr:27} & PROP-12; también \hyperref[tab:RF-08]{RF-08}, \hyperref[tab:RF-15]{RF-15} & \hyperref[tab:RF-27]{RF-27} & \hyperref[tab:CU-05]{CU-05} & BreedingRequest & P3 & Approved & \hyperref[lst:ca26]{CA-26} & \hyperref[cp:27]{CP-27} & \hyperref[tab:HU-26]{HU-26} & Completa \\ \midrule
			\hyperref[tr:28]{TR-28}\label{tr:28} & VET-03, PROP-03, Encuesta, OBS-01; también \hyperref[tab:RD-04]{RD-04}, \hyperref[rl:05]{RL-05}, \hyperref[rl:07]{RL-07} & \hyperref[tab:RNF-01]{RNF-01} & \hyperref[tab:CU-12]{CU-12} & --- & P1 & --- & --- & \hyperref[cp:28]{CP-28} & --- & Completa (transversal) \\ \midrule
			\hyperref[tr:29]{TR-29}\label{tr:29} & PROP-01, PROP-02, VET-01, VET-02, VET-03, PROP-03, Encuesta; también \hyperref[tab:RNF-03]{RNF-03} & \hyperref[tab:RNF-02]{RNF-02} & (Transversal) & --- & Transversal & --- & --- & \hyperref[cp:29]{CP-29} & --- & Completa (transversal) \\ \midrule
			\hyperref[tr:30]{TR-30}\label{tr:30} & PROP-01, PROP-02, VET-01, VET-02, VET-03, PROP-03, Encuesta; también \hyperref[tab:RNF-02]{RNF-02}, \hyperref[tab:RNF-12]{RNF-12} & \hyperref[tab:RNF-03]{RNF-03} & \hyperref[tab:CU-11]{CU-11} & --- & Transversal & --- & --- & \hyperref[cp:30]{CP-30} & --- & Completa (transversal) \\ \midrule
			\hyperref[tr:31]{TR-31}\label{tr:31} & PROP-01, PROP-02, VET-01, VET-02, VET-03, PROP-03, Encuesta, OBS-01 & \hyperref[tab:RNF-04]{RNF-04} & (Transversal / UI) & --- & Transversal & --- & --- & \hyperref[cp:31]{CP-31} & --- & Completa (transversal) \\ \midrule
			\hyperref[tr:32]{TR-32}\label{tr:32} & VET-01, VET-03, Encuesta; también \hyperref[tab:RD-04]{RD-04} & \hyperref[tab:RNF-05]{RNF-05} & \hyperref[tab:CU-02]{CU-02} & --- & P1 & --- & --- & \hyperref[cp:32]{CP-32} & --- & Completa (transversal) \\ \midrule
			\hyperref[tr:33]{TR-33}\label{tr:33} & PROP-01, VET-01, VET-03, Encuesta; también \hyperref[tab:RF-06]{RF-06}, \hyperref[tab:RNF-13]{RNF-13} & \hyperref[tab:RNF-06]{RNF-06} & \hyperref[tab:CU-13]{CU-13} & CompatibilityEvaluation & P8 & --- & \hyperref[lst:ca05]{CA-05} (heredado de \hyperref[tab:RF-06]{RF-06}) & \hyperref[tab:CP-33]{CP-33} & \hyperref[tab:HU-05]{HU-05} (heredada de \hyperref[tab:RF-06]{RF-06}) & Completa \\ \midrule
			\hyperref[tr:34]{TR-34}\label{tr:34} & PROP-01, VET-01, VET-03, Encuesta; también \hyperref[tab:RF-17]{RF-17}, \hyperref[tab:RNF-14]{RNF-14} & \hyperref[tab:RNF-07]{RNF-07} & (Transversal a Media) & Post & P8 & --- & \hyperref[lst:ca20]{CA-20} (heredado de \hyperref[tab:RF-17]{RF-17}) & \hyperref[cp:34]{CP-34} & \hyperref[tab:HU-20]{HU-20} (heredada de \hyperref[tab:RF-17]{RF-17}) & Completa \\ \midrule
			\hyperref[tr:35]{TR-35}\label{tr:35} & PROP-01, PROP-02, VET-01, VET-02, VET-03, PROP-03, Encuesta & \hyperref[tab:RNF-08]{RNF-08} & \hyperref[tab:CU-01]{CU-01} & --- & P1 & --- & --- & \hyperref[cp:35]{CP-35} & --- & Completa (transversal) \\ \midrule
			\hyperref[tr:36]{TR-36}\label{tr:36} & PROP-04, PROP-11; también \hyperref[tab:RF-10]{RF-10}, \hyperref[tab:RD-08]{RD-08} & \hyperref[tab:RNF-09]{RNF-09} & \hyperref[tab:CU-12]{CU-12} & --- & P1 & --- & \hyperref[lst:ca18]{CA-18} (heredado de \hyperref[tab:RF-10]{RF-10}) & \hyperref[cp:36]{CP-36} & \hyperref[tab:HU-18]{HU-18} (heredada de \hyperref[tab:RF-10]{RF-10}) & Completa \\ \midrule
			\hyperref[tr:37]{TR-37}\label{tr:37} & Equipo desarrollador; también \hyperref[tab:RD-05]{RD-05} & \hyperref[tab:RNF-10]{RNF-10} & (Transversal a Diseño) & --- & Transversal & --- & --- & \hyperref[cp:37]{CP-37} & --- & Completa (transversal) \\ \midrule
			\hyperref[tr:38]{TR-38}\label{tr:38} & Encuesta, OBS-01 & \hyperref[tab:RNF-11]{RNF-11} & (Transversal a UI) & --- & Transversal & --- & --- & \hyperref[cp:38]{CP-38} & --- & Completa (transversal) \\ \midrule
			\hyperref[tr:39]{TR-39}\label{tr:39} & Encuesta, OBS-01; también \hyperref[tab:RNF-03]{RNF-03} & \hyperref[tab:RNF-12]{RNF-12} & (Transversal) & --- & Transversal & --- & --- & \hyperref[cp:39]{CP-39} & --- & Completa (transversal) \\ \midrule
			\hyperref[tr:40]{TR-40}\label{tr:40} & PROP-06, PROP-08, VET-04, VET-05; también \hyperref[tab:RF-06]{RF-06}, \hyperref[tab:RNF-06]{RNF-06}, \hyperref[rl:06]{RL-06} & \hyperref[tab:RNF-13]{RNF-13} & \hyperref[tab:CU-13]{CU-13} & CompatibilityEvaluation & P8 & --- & \hyperref[lst:ca05]{CA-05} & \hyperref[cp:40]{CP-40} & \hyperref[tab:HU-05]{HU-05} (heredada de \hyperref[tab:RF-06]{RF-06}) & Completa \\ \midrule
			\hyperref[tr:41]{TR-41}\label{tr:41} & PROP-06, PROP-08; también \hyperref[tab:RF-17]{RF-17}, \hyperref[tab:RNF-07]{RNF-07}, \hyperref[rl:06]{RL-06} & \hyperref[tab:RNF-14]{RNF-14} & (Transversal a Media) & Post & P8 & --- & \hyperref[lst:ca20]{CA-20} (heredado de \hyperref[tab:RF-17]{RF-17}) & \hyperref[cp:41]{CP-41} & \hyperref[tab:HU-20]{HU-20} (heredada de \hyperref[tab:RF-17]{RF-17}) & Completa \\ \midrule
			\hyperref[tr:42]{TR-42}\label{tr:42} & PROP-08, PROP-11; también \hyperref[tab:RF-07]{RF-07}, \hyperref[rl:06]{RL-06} & \hyperref[tab:RNF-15]{RNF-15} & \hyperref[tab:CU-08]{CU-08} & Message & P5 & --- & \hyperref[lst:ca17]{CA-17} (heredado de \hyperref[tab:RF-07]{RF-07}) & \hyperref[cp:42]{CP-42} & \hyperref[tab:HU-17]{HU-17} (heredada de \hyperref[tab:RF-07]{RF-07}) & Completa \\ \midrule
			\hyperref[tr:43]{TR-43}\label{tr:43} & PROP-07, VET-05; también \hyperref[tab:RF-11]{RF-11}, \hyperref[rl:05]{RL-05}, \hyperref[rl:08]{RL-08} & \hyperref[tab:RNF-16]{RNF-16} & \hyperref[tab:CU-12]{CU-12} & --- & P1 & --- & \hyperref[lst:ca08]{CA-08} & \hyperref[cp:43]{CP-43} & \hyperref[tab:HU-08]{HU-08} (heredada de \hyperref[tab:RF-11]{RF-11}) & Completa \\ \midrule
			\hyperref[tr:44]{TR-44}\label{tr:44} & PROP-01, VET-01, VET-03, Encuesta; también \hyperref[tab:RF-06]{RF-06}, \hyperref[tab:RF-17]{RF-17}, \hyperref[tab:RF-07]{RF-07} & \hyperref[tab:RD-01]{RD-01} & \hyperref[tab:CU-13]{CU-13} & --- & P8 & --- & --- & \hyperref[cp:44]{CP-44} & --- & Completa (transversal) \\ \midrule
			\hyperref[tr:45]{TR-45}\label{tr:45} & PROP-01, PROP-02, VET-01, VET-02, VET-03, PROP-03, Encuesta; también \hyperref[tab:RF-11]{RF-11}, \hyperref[rl:05]{RL-05}, \hyperref[rl:08]{RL-08} & \hyperref[tab:RD-02]{RD-02} & (Transversal a Seguridad) & User, Owner, Shelter, Veterinarian & Transversal & --- & --- & \hyperref[cp:45]{CP-45} & --- & Completa (transversal) \\ \midrule
			\hyperref[tr:46]{TR-46}\label{tr:46} & VET-01, VET-03, Encuesta; también \hyperref[tab:RF-09]{RF-09}, \hyperref[tab:RF-19]{RF-19} & \hyperref[tab:RD-03]{RD-03} & \hyperref[tab:CU-09]{CU-09} & VeterinaryEvaluation, Veterinarian & P4 & --- & --- & \hyperref[cp:46]{CP-46} & --- & Completa (transversal) \\ \midrule
			\hyperref[tr:47]{TR-47}\label{tr:47} & PROP-02, VET-03, PROP-03, Encuesta; también \hyperref[tab:RNF-01]{RNF-01}, \hyperref[rl:05]{RL-05}, \hyperref[rl:07]{RL-07} & \hyperref[tab:RD-04]{RD-04} & \hyperref[tab:CU-12]{CU-12} & PrivacySettings & P1 & --- & --- & \hyperref[cp:47]{CP-47} & --- & Completa (transversal) \\ \midrule
			\hyperref[tr:48]{TR-48}\label{tr:48} & Equipo desarrollador; también \hyperref[tab:RNF-10]{RNF-10} & \hyperref[tab:RD-05]{RD-05} & (Transversal a Diseño) & --- & Transversal & --- & --- & \hyperref[cp:48]{CP-48} & --- & Completa (transversal) \\ \midrule
			\hyperref[tr:49]{TR-49}\label{tr:49} & PROP-11; también \hyperref[rl:04]{RL-04} & \hyperref[tab:RD-06]{RD-06} & (Transversal a Datos) & --- & Transversal & --- & --- & \hyperref[cp:49]{CP-49} & --- & Completa (transversal) \\ \midrule
			\hyperref[tr:50]{TR-50}\label{tr:50} & PROP-07, PROP-11; también \hyperref[rl:09]{RL-09} & \hyperref[tab:RD-07]{RD-07} & (Transversal a Seguridad) & --- & Transversal & --- & --- & \hyperref[cp:50]{CP-50} & --- & Completa (transversal) \\ \midrule
			\hyperref[tr:51]{TR-51}\label{tr:51} & PROP-04, PROP-11; también \hyperref[tab:RF-10]{RF-10}, \hyperref[tab:RNF-09]{RNF-09} & \hyperref[tab:RD-08]{RD-08} & \hyperref[tab:CU-12]{CU-12} & --- & P1 & --- & --- & \hyperref[cp:51]{CP-51} & --- & Completa (transversal) \\ \midrule
			\hyperref[tr:52]{TR-52}\label{tr:52} & PROP-01, PROP-02, VET-01, VET-02, VET-03, PROP-03, Encuesta, OBS-01, PROP-04, PROP-05, PROP-06, PROP-07, PROP-08, PROP-09, VET-04, VET-05, PROP-10, PROP-11, VET-06, PROP-12, VET-07 & \hyperref[tab:RD-09]{RD-09} & (Transversal a Datos) & --- & Transversal & --- & --- & \hyperref[cp:52]{CP-52} & --- & Completa (transversal) \\ \midrule
			\hyperref[tr:53]{TR-53}\label{tr:53} & LOPDP, Art. 7 y 8; también \hyperref[rl:02]{RL-02}, \hyperref[tab:RD-06]{RD-06} & \hyperref[rl:01]{RL-01} & (Onboarding) & --- & P1 & --- & --- & \hyperref[cp:53]{CP-53} & --- & Completa (transversal) \\ \midrule
			\hyperref[tr:54]{TR-54}\label{tr:54} & LOPDP, Art. 12 y 13; también \hyperref[rl:01]{RL-01}, \hyperref[rl:03]{RL-03} & \hyperref[rl:02]{RL-02} & \hyperref[tab:CU-10]{CU-10} & --- & P2 & --- & --- & \hyperref[cp:54]{CP-54} & --- & Completa (transversal) \\ \midrule
			\hyperref[tr:55]{TR-55}\label{tr:55} & LOPDP, Art. 14; también \hyperref[rl:02]{RL-02}, \hyperref[rl:04]{RL-04} & \hyperref[rl:03]{RL-03} & \hyperref[tab:CU-02]{CU-02} & --- & P1 & --- & --- & \hyperref[cp:55]{CP-55} & --- & Completa (transversal) \\ \midrule
			\hyperref[tr:56]{TR-56}\label{tr:56} & LOPDP, Art. 15; también \hyperref[rl:03]{RL-03}, \hyperref[rl:05]{RL-05} & \hyperref[rl:04]{RL-04} & (Transversal a Datos) & --- & Transversal & --- & --- & \hyperref[cp:56]{CP-56} & --- & Completa (transversal) \\ \midrule
			\hyperref[tr:57]{TR-57}\label{tr:57} & LOPDP, Art. 10 lit. e) y j); también \hyperref[rl:04]{RL-04}, \hyperref[rl:07]{RL-07}, \hyperref[rl:08]{RL-08} & \hyperref[rl:05]{RL-05} & \hyperref[tab:CU-12]{CU-12} & --- & P1 & --- & --- & \hyperref[cp:57]{CP-57} & --- & Completa (transversal) \\ \midrule
			\hyperref[tr:58]{TR-58}\label{tr:58} & LOPDP, Art. 20; también \hyperref[tab:RNF-13]{RNF-13}, \hyperref[tab:RNF-14]{RNF-14}, \hyperref[tab:RNF-15]{RNF-15} & \hyperref[rl:06]{RL-06} & \hyperref[tab:CU-13]{CU-13} & --- & P8 & --- & --- & \hyperref[tab:CP-58]{CP-58} & --- & Completa (transversal) \\ \midrule
			\hyperref[tr:59]{TR-59}\label{tr:59} & LOPDP, Art. 25, 26 y 30; también \hyperref[rl:05]{RL-05}, \hyperref[tab:RD-04]{RD-04} & \hyperref[rl:07]{RL-07} & \hyperref[tab:CU-12]{CU-12} & --- & P1 & --- & --- & \hyperref[cp:59]{CP-59} & --- & Completa (transversal) \\ \midrule
			\hyperref[tr:60]{TR-60}\label{tr:60} & LOPDP, Art. 39; también \hyperref[tab:RD-02]{RD-02}, \hyperref[rl:05]{RL-05} & \hyperref[rl:08]{RL-08} & (Transversal a Seguridad) & --- & Transversal & --- & --- & \hyperref[cp:60]{CP-60} & --- & Completa (transversal) \\ \midrule
			\hyperref[tr:61]{TR-61}\label{tr:61} & LOPDP, Art. 43 y 46; también \hyperref[tab:RD-07]{RD-07} & \hyperref[rl:09]{RL-09} & (Transversal a Seguridad) & --- & Transversal & --- & --- & \hyperref[cp:61]{CP-61} & --- & Completa (transversal) \\ \midrule
...
\end{landscape}
	...
	\url{https://github.com/gleiston-guerrero/MundiPets-PFC-IngReq.git}, cuya
	estructura de carpetas se muestra en el Código~\ref{lst:estructura_repo},
	incluyendo la carpeta de defensa (\texttt{09\_Defensa/}), el paquete de
	datos independiente (\texttt{07\_Datos/}), la carpeta de evidencia de
	autoría del equipo (\texttt{10\_Autoria/}), el archivo raíz
	...
	\begin{lstlisting}[style=csharp,caption={Estructura de carpetas del repositorio MundiPets-PFC-IngReq.},label={lst:estructura_repo}]
		MundiPets-PFC-IngReq/
		|-- .gitignore
		|-- .mailmap                       % unifica las identidades de Git
		|-- CHANGELOG.md
		|-- checksums.sha256               % manifiesto de todo el repositorio
		|-- CITATION.cff
		|-- fair_assessment.pdf
		|-- LICENSE
		|-- README.md
		...
		|   `-- figuras/                   % diagramas i*, UML (actividades
		|                                   % CU-01 a CU-13, secuencia CU-01 a
		|                                   % CU-13, clases, componentes,
		|                                   % contexto, despliegue, maquinas
		|                                   % de estado), curva de saturacion,
		|                                   % mockups MU-01 a MU-08 y capturas
		|                                   % del MVP MVP-01 a MVP-04
		|                                   % (temporales de compilacion LaTeX
		|                                   % omitidos: .aux/.log/.toc/etc.)
		|
		|-- 02_Evidencias/
		...
		|   |       `-- evidencias_restringidas.7z.001 ... .286
		|   |-- Codificacion_Tematica/
		|   |   |-- codificacion_tematica.csv
		|   |   `-- curva_saturacion.png
		|   |-- Consentimientos/           % copias enmascaradas [P]
		|   |                              % (18 archivos, uno por participante)
		|   |-- Cuestionario/
		|   |   |-- Fotos_Aplicacion/      % evidencia de aplicacion (6 fotos)
		|   |   |-- power_calculation/     % script, README y tabla de power
		|   |   |                          % calculation del perfil dominante
		|   |   |                          % (n=47, potencia=91,86 %)
		|   |   `-- Respuestas/            % 61 respuestas en PDF
		|   |                              % + Respuestas_Cuestionario_MundiPets.xlsx
		|   |-- Documentos_Organizacion/   % 5 documentos clinicos de referencia
		|   |-- Fotos_Entorno/             % observacion OBS-01 (20 archivos)
		|   |-- Member_Checking/           % acta de miembro-verificacion
		|   |-- Transcripciones/           % transcripciones anonimizadas
		|   |                              % (17 archivos, uno por entrevista)
		|   `-- Validacion_Walkthrough/    % 6 actas (3 tecnicas + 3 no tecnicas)
		|
		|-- 03_Modelado/
		|   |-- Diagramas_UML/             % una subcarpeta por tipo de diagrama
		|   |                              % (Casos de Uso, Componentes, Contexto,
		|   |                              % Dependencia y Razon Estrategica,
		|   |                              % Despliegue, Actividades CU-01 a
		|   |                              % CU-13, Clases, Maquinas de Estado,
		|   |                              % Secuencia CU-01 a CU-13), cada una
		|   |                              % con sus archivos .png/.svg/.vpp y,
		|   |                              % para los diagramas i*, tambien
		|   |                              % .pdf/.drawio
		...
		|   `-- sincronizacion_tablero.csv % exportacion del tablero de gestion
		|
		|-- 05_MVP/
		|   |-- README.md                  % instrucciones de despliegue
		|   |-- video_demo.mp4             % evidencia de funcionamiento
		|   `-- codigo/                    % codigo fuente del MVP (Node.js +
		|       |                          % Express), incorporado con git
		|       |                          % subtree, no es submodulo; conserva
		|       |                          % el historial real de commits del
		|       |                          % desarrollo del MVP
		|       |-- *.html                 % pantallas del MVP (index, dashboard,
		|       |                          % explore, pet-profile, register, etc.)
		|       |-- server.js, package.json
		|       |-- css/, js/, img/
		|       `-- node_modules/          % dependencias de Node.js instaladas
		|                                  % (Express y sus dependencias transitivas;
		|                                  % contenido estandar de npm, omitido en
		|                                  % detalle por no ser artefacto propio)
		|
		|-- 06_Experimento/
		...
		|   |-- instrumentos/              % consentimientos y transcripcion de
		|   |                              % la reunion del panel de expertos
		|   |                              % (EXP-01, EXP-02, EXP-03) y rubrica
		|   |                              % de clasificacion experta
		|   |-- prompts_llm/
		|   |-- datos_crudos/              % evaluacion_expertos.csv,
		|   |                              % salida_detector.csv y su version .xlsx
		|   |-- datos_procesados/          % consenso_experto_vs_detector.csv
		|   |-- resultados/
		|   |   |-- figuras/               % figura_1 a figura_4
		|   |   `-- tablas/                % tabla_1 a tabla_9 (CSV)
		|   `-- scripts_analisis/          % 02, 03, 04, 05 y run_all.py
		|       `-- datos_entrada/
		|
		|-- 07_Datos/                      % paquete de datos independiente y
		|   |                              % autocontenido, reproducible de
		|   |                              % punta a punta desde clon limpio
		|   |-- README_datos.md
		|   |-- LICENSE-DATA.txt           % licencia de los datos, distinta
		|   |                              % de la del codigo
		...
		|   |   `-- tablas/                % tabla_1 a tabla_9 (CSV)
		|   `-- scripts/
		|       `-- run_all.py             % orquestador unico, un solo comando
		|
		|-- 07_Publicacion/
		...
		|   |-- figuras/                   % figura_1 a figura_4
		|   |-- tablas/                    % tabla_1 a tabla_9 (CSV)
		|   |-- manuscrito/                % temporales de compilacion LaTeX
		|   |                              % del manuscrito, omitidos en detalle
		|   `-- dataset_zenodo/
		|       |-- README_dataset.md
		|       |-- ANONYMIZATION.md
		|       |-- ETHICS.md
		|       |-- corpus/                % corpus_RF_MundiPets_2B.json
		|       |-- prompts_llm/
		|       |-- respuestas_cuestionarios/
		|       |-- scripts_analisis/      % misma estructura que 06_Experimento
		|       `-- trazabilidad/          % matriz_trazabilidad.csv
		|
		|-- 08_Etica/
		...
		|
		`-- 10_Autoria/                    % evidencia de autoria y trabajo
		    |                              % propio del equipo (A1 a A12)
		    |-- .mailmap
		    |-- bitacora_sesiones.csv      % A1, una fila por sesion
		    |-- aporte_individual.md       % A10
		    |-- declaracion_uso_ia.md      % A9
		    |-- exif_inventario.csv        % A11
		    |-- retrospectiva_equipo.md
		    |-- verificacion_previa.pdf    % lista de verificacion firmada
		    |-- capturas/                  % A2 (15 archivos)
		    |-- correspondencia/           % A8 (22 archivos)
		    |-- doble_codificacion/        % A7: muestra, hojas de Andy Mendoza
		    |                              % y Edson Fuertes, script de calculo
		    |                              % del coeficiente Kappa y resultado
		    |-- fotos_equipo/              % A6 (3 fotos con EXIF verificado)
		    |-- grabaciones/               % A4 (2 videos de sesion de trabajo)
		    |-- notas_campo/               % A5 (25 archivos)
		    `-- fuente_editables/          % A3, una subcarpeta por diagrama
	\end{lstlisting}
	...
	El archivo \texttt{07\_Publicacion/dataset\_zenodo/corpus/corpus\_RF\_MundiPets\_2B.json}
	...
	equipo. La Figura~\ref{fig:commits}
	muestra una captura del historial al momento de esta revisión.
	
	% --- Captura del historial de commits (agregar manualmente) ---
	\begin{figure}[H]
		\centering
		\includegraphics[width=\textwidth]{figuras/historial_commits.png}
		\caption[Historial de commits del repositorio MundiPets-PFC-IngReq]{Historial de commits del repositorio \texttt{MundiPets-PFC-IngReq}.}
		\label{fig:commits}
	\end{figure}
	
	\subsection*{Línea base del ERS}
	\label{ap:linea-base-ers}

	La última corrección del ERS/SRS (versión 4.0, Tabla~\ref{tab:historial})
	constituye la línea base formal del documento: a partir de este punto,
	cualquier cambio posterior sobre los requisitos, las historias de usuario,
	los diagramas UML o la matriz de trazabilidad se considera una nueva
	revisión y debe incrementar la versión del documento. Esta línea base se
	marca en el repositorio mediante la etiqueta (\emph{tag}) de Git
	\texttt{ers-v4.0-final}, aplicada sobre el commit que incorpora la versión
	corregida de \texttt{01\_ERS/ERS\_SRS\_2B\_v2.0.tex} y su PDF compilado.

	\begin{table}[H]
		\centering
		\caption{Identificación de la línea base del ERS.}
		\label{tab:linea-base-ers}
		\small
		\begin{tabularx}{\textwidth}{@{}p{4cm} X@{}}
			\toprule
			\textbf{Atributo} & \textbf{Valor} \\
			\midrule
			Etiqueta (tag) & \texttt{ers-v4.0-final} \\
			Versión del documento & 4.0 (Tabla~\ref{tab:historial}) \\
			Archivo etiquetado & \texttt{01\_ERS/ERS\_SRS\_2B\_v2.0.tex} y su PDF compilado \\
			...
			Efecto de la etiqueta & Congela el estado del ERS correspondiente a esta corrección como referencia estable para el MVP (Sección~\ref{sec:mvp}) y el componente empírico (Sección~\ref{sec:empirico}); cualquier cambio posterior se realiza sobre una nueva revisión, no sobre la línea base etiquetada. \\
			\bottomrule
		\end{tabularx}
	\end{table}
	...
	\texttt{06\_Experimento/protocolo.pdf}, cuyo contenido corresponde
	literalmente al de la Sección~\ref{sec:empirico} de este documento.
	...
	(\texttt{rubrica\_clasificacion\_experta.xlsx}) y los consentimientos
	firmados por las tres personas evaluadoras
	(\texttt{Consentimiento\_EXP-01\_Experto.png},
	\texttt{Consentimiento\_EXP-02\_Experto.png} y
	\texttt{Consentimiento\_EXP-03\_Experto.png}) --- se anexan al
	protocolo independiente y se versionan en
	\texttt{06\_Experimento/instrumentos/}. Cualquier divergencia entre el
	protocolo registrado en el OSF y el efectivamente ejecutado se declara y
	justifica de forma explícita, junto con los resultados
	...
	El equipo elige \textbf{Requirements Engineering} (Springer Nature) como
	revista objetivo, por su máximo ajuste temático con el proyecto y por no
	exigir cargo de publicación en la modalidad de suscripción. El
	manuscrito se redacta con la plantilla oficial de la revista
	(\texttt{sn-jnl.cls}, Springer Nature LaTeX template), ya presente en
	\texttt{07\_Publicacion/manuscrito/}.
	...
	Dado el volumen de evidencia empírica alcanzado --- un corpus de 61
	requisitos (27 RF, 16 RNF, 9 restricciones de diseño y 9 obligaciones
	legales, vigentes al momento de ejecutar el componente empírico)
	evaluado por un panel de tres personas expertas ---, la modalidad
	realista de envío es la de artículo corto o \emph{tool paper}
	(6 a 8 páginas), y no la de artículo completo.
	...
	\begin{table}[H]
		\centering
	\caption{Checklist de aceptación del documento.}
		\label{tab:checklist}
		\small
		\begin{tabularx}{\textwidth}{@{}c X c@{}}
			...
			6 & Cada consentimiento publicado en la zona pública presenta la información identificativa y la firma debidamente enmascaradas, mientras que el original íntegro se conserva en la zona restringida. & $\blacksquare$ \\
			7 & El cuestionario tiene fotografías de aplicación real y respuestas exportadas con $n \geq 60$ por perfil dominante, o justificación mediante cálculo de potencia estadística. & $\blacksquare$ \\
			...
			12 & La matriz de trazabilidad en CSV tiene 61 filas cerradas y todas las columnas obligatorias. &$\blacksquare$ \\
			13 & Las evidencias declaradas como obligatorias para la entrega se encuentran disponibles en el repositorio, con los materiales sensibles protegidos en la zona restringida y la documentación correspondiente en \texttt{08\_Etica/README\_Etica.md}. & $\blacksquare$ \\
			14 & Se alcanzaron los mínimos comprometidos: 27 RF, 16 RNF, 13 CU detallados y 61 filas de trazabilidad. El cuestionario reúne 61 respuestas: 47 de propietarios de mascotas, 7 de interesados en adopción y 7 de interesados en cruza. La suficiencia de la muestra del perfil dominante se sustenta mediante el cálculo de potencia estadística (91,86\,\% con $n=47$), como alternativa admitida por la guía. & $\blacksquare$ \\
...
\section*{Apéndice I --- Retrospectiva del equipo y declaraciones individuales}
\addcontentsline{toc}{section}{Apéndice I --- Retrospectiva y declaraciones individuales}
\label{ap:retrospectiva}

\subsection*{I.1 Retrospectiva del equipo}
\label{sec:retrospectiva}

El equipo realizó una retrospectiva en formato Start--Stop--Continue,
referida al proceso de trabajo sostenido a lo largo de las cuatro entregas
del proyecto. La Tabla~\ref{tab:retrospectiva} recoge los elementos
identificados por categoría y la persona responsable de la acción de
mejora correspondiente.

\begin{longtable}{@{}p{2.2cm} p{8.0cm} p{3.6cm}@{}}
	...
	\midrule
	\endfirsthead
	\multicolumn{3}{@{}l}{\small\itshape (continuación de la Tabla \ref{tab:retrospectiva})}\\
	\toprule
	\textbf{Categoría} & \textbf{Elemento} & \textbf{Responsable / acción} \\
	\midrule
	\endhead
	...
	\multirow{3}{*}{Start} & Definir con antelación un cronograma interno de commits por hito, en lugar de concentrar la carga de trabajo en los días previos al cierre de cada entrega. & Todo el equipo \\
	\cline{2-3}
	...
	\multirow{3}{*}{Stop} & Dejar de declarar en el texto elementos (nombres de archivo, cifras de cobertura) sin verificarlos primero contra el repositorio real. & Todo el equipo \\
	\cline{2-3}
	...
	\multirow{3}{*}{Continue} & Mantener la distribución de commits repartida entre los cinco integrantes con correo institucional, sin autores externos al equipo. & Todo el equipo \\
	\cline{2-3}
	...
	\cline{2-3}
	& Mantener el inventario técnico de evidencias (\texttt{fichas\_tecnicas.csv}) con hash, duración y códec por archivo, que permite auditar la evidencia sin necesidad de abrir el volumen cifrado. & Mendoza Párraga Andy Johel \\
	
\end{longtable}

\subsection*{I.2 Declaración individual de aporte al proyecto}
\label{sec:aporte-individual}

La Tabla~\ref{tab:aporte-individual} documenta, para cada integrante, sus
aportes concretos al proyecto en términos de artefactos y secciones, con
la evidencia correspondiente verificable en el historial de \emph{commits}
del repositorio.

\begin{longtable}{@{}p{2.6cm} p{7.4cm} p{4.0cm}@{}}
	\caption{Declaración individual de aporte al proyecto.} \label{tab:aporte-individual} \\
	\toprule
	\textbf{Integrante} & \textbf{Aportes concretos (artefactos y secciones)} & \textbf{Evidencia en el repositorio} \\
	\midrule
	\endfirsthead
	\multicolumn{3}{@{}l}{\small\itshape (continuación de la Tabla \ref{tab:aporte-individual})}\\
	\toprule
	\textbf{Integrante} & \textbf{Aportes concretos (artefactos y secciones)} & \textbf{Evidencia en el repositorio} \\
	\midrule
	\endhead
	...
	Fuertes Arraes Edson Daniel (Analista líder) & Coordinación de la elicitación de requisitos, consolidación de la especificación de requisitos funcionales y no funcionales, y verificación de consistencia entre la especificación textual y el modelado. & Commits sobre \texttt{01\_ERS/} y \texttt{04\_Trazabilidad/} \\
	\midrule
	
	Gutiérrez Ortega Génesis Adriana (Verificadora) & Verificación de la matriz de trazabilidad, revisión cruzada de los casos de prueba y consistencia de la priorización MoSCoW, Kano y WSJF. & Commits sobre \texttt{01\_ERS/} (secciones de trazabilidad y priorización) \\
	\midrule
	
	Mendoza Párraga Andy Johel (Modelador) & Modelado UML completo (casos de uso, clases, secuencia, actividad, estados, componentes, despliegue, contexto), especificación de los componentes de inteligencia artificial y su plan de monitoreo. & Commits sobre \texttt{03\_Modelado/} y las figuras asociadas en \texttt{01\_ERS/figuras/} \\
	\midrule
	
	Morales Sánchez Gary Alejandro (Documentador) & Redacción y consolidación del documento ERS/SRS, gestión de la evidencia de campo (transcripciones, actas, anonimización) y estructura del repositorio. & Commits sobre \texttt{02\_Evidencias/} y \texttt{01\_ERS/ERS\_SRS\_2B\_v2.0.tex} \\
	\midrule
	
	Nieves Sánchez Jimmy Samuel (Verificador) & Verificación del componente empírico, ejecución del protocolo experimental registrado en el OSF y verificación del manuscrito derivado. & Commits sobre \texttt{06\_Experimento/} y \texttt{07\_Publicacion/} \\
	
\end{longtable}

\subsection*{I.3 Declaración de uso de herramientas de inteligencia artificial generativa}
\label{sec:declaracion-ia-generativa}

En cumplimiento del principio de transparencia académica, el equipo
declara el uso de herramientas de inteligencia artificial generativa
(modelos de lenguaje de gran escala) durante la elaboración de este
proyecto, conforme al detalle de la Tabla~\ref{tab:uso-ia-generativa}.
Ninguna herramienta de este tipo participó en la recolección de evidencia
de campo (entrevistas, cuestionario, observación), en la toma de
decisiones de diseño del sistema, ni en la generación de los datos
reportados en el componente empírico (Sección~\ref{sec:empirico}); su uso
se limitó a las tareas de apoyo declaradas.

\begin{table}[H]
	\centering
	\small
	\caption{Uso declarado de herramientas de inteligencia artificial generativa.}
	\label{tab:uso-ia-generativa}
	\begin{tabularx}{\textwidth}{@{}p{3.0cm} X@{}}
		\toprule
		\textbf{Tarea de apoyo} & \textbf{Descripción del uso} \\
		\midrule
...
Ninguna captura de pantalla, ninguna evidencia de campo (transcripción,
consentimiento, fotografía) ni ningún resultado del componente empírico
fue generado o alterado por una herramienta de inteligencia artificial
generativa; su origen es exclusivamente el trabajo de campo y el análisis
del equipo, documentado en los Apéndices B y F de este ERS.
